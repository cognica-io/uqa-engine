...
4. **Typed operator joins.** `vector_similarity_join` emits generalized `(left_doc_id, right_doc_id)` tuples, while `cross_paradigm_join` bridges a graph vertex property to a document field and then participates as a costed DPccp source in an ordinary SQL join.
