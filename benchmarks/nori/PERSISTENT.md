...
Seven reports in `persistent-evidence/` were collected on an Apple M1 Ultra with Rust 1.90.0; WASM used Node 22.12.0 and Emscripten 6.0.3. Five additional reports record Linux x86_64 CI measurements: repeated SQLite/redb runs at clean commit `9d4d7830` and a redb commit-order observation at clean commit `1871246b`. Native SQLite reports version 3.50.4 and WASM SQLite reports version 3.51.3, both with WAL, synchronous value 1, and 4,096-byte pages. The original seven reports identify the working tree based on `f7e05ebf` and include every benchmark/runtime source hash; they do not claim collection from a clean commit. The Linux reports preserve the original CI artifact bytes, including their collection-time gate results, and the calibration manifest records the source run, attempt, and artifact ID. Artifact IDs disambiguate repeated uploads with the same name. `persistent-limits.json` pins every report hash and permits no allocation increase beyond the maximum observed value for its provider, target OS, target architecture, and pointer width. SQLite repeated every allocation counter exactly in all three calibrated targets. redb repeated every counter except cumulative allocated bytes, which vary by three bytes in commit workloads.
...
A later Linux run exposed a missing commit-order outcome for `commit_batch_16/256`: 51,574,092 cumulative requested bytes instead of the previously observed 51,574,089. The complete graph and all other five allocation counters were unchanged. In the pinned redb 4.1.0 implementation, `TableTree::flush_table_root_updates` drains a randomized `HashMap`; after the first table update makes the metadata leaf writable, an in-place B-tree replacement copies the previous serialized table definition. The provider data table uses `&[u8]`/`&[u8]` and its metadata table uses `&str`/`u64`. Their serialized type names total ten and seven bytes, so changing which definition is copied changes only cumulative allocation by three bytes. This is an allocation-calibration omission; no index algorithm, stored format, dependency feature, or upstream implementation needs changing.

The provider-owned `commit_allocation_order` benchmark isolates this behavior through public redb APIs. It observes 64 fresh databases for each combination of new/existing tables and identical/provider table types, measuring commit alone and checking both tables after closing and reopening every database. The three controls have one allocation outcome each; only an existing mixed-type pair produces two outcomes exactly three bytes apart, with all other counters identical. The hashed `redb-commit-allocation-probe.json` report retains the exact source, binary, lockfile, compiler, CPU, and dirty-tree provenance based on `1871246b`; it is an allocation experiment, not a timing baseline. Run `cargo bench --locked -p uqa-storage-redb --bench commit_allocation_order` to repeat the experiment without enabling Nori. Native CI executes the same assertions and uploads the result alongside index measurements.

The calibration check now requires both three-byte-separated outcomes for every redb commit workload on each calibrated target, and requires the remaining counters and rollback workloads to stay stable. It rejects the previous incomplete Linux evidence. The new raw CI report preserves its failed collection-time gate and exact artifact identity; only that workload’s Linux cumulative-byte ceiling increases to the newly observed maximum. An additional byte or allocation beyond any reviewed ceiling still fails.
