...
python3 scripts/verify-morphology-browser.py --observe-memory --runs 3 --output output/playwright/nori-enabled/report.json
...
python3 scripts/verify-morphology-browser.py --observe-memory --runs 3 --feature disabled --bundle target/nori-binding-disabled/wasm/index.mjs --output output/playwright/nori-disabled/report.json
...
Chrome and `npx` must be available. The driver defaults to one fresh browser session and functional verification without host memory sampling. It validates every fixture step, page identity, IndexedDB checkpoint and complete diagnostic hash, and rejects browser console errors. The reproduction commands above explicitly select three runs and the optional memory observations; CI uses the functional defaults. It records the browser version, host, toolchain, requested compiler flags, runtime dependency closure, source hashes, and actual JS/WASM artifact hashes. Feature-disabled dependency resolution must omit both `uqa-nori-data` and `uqa-kuromoji-data`. JavaScript CI executes enabled/disabled Nori and Kuromoji browser configurations and retains their reports alongside the existing native/WASM allocation gates. For manual inspection, run `python3 scripts/serve-wasm-tests.py`, open the printed address, and use the reload buttons; the restart button creates an independent test database.
