...
"""Verify real-browser Nori persistence and record post-GC host memory observations."""
...
def verify_run(report: dict, feature: str) -> None:
    ...
    fixture = json.loads((ROOT / "tests/parity/nori/bindings.json").read_text())
    ...
    if report["analyses"] != (contract["checks"] if feature == "enabled" else []):
...
def run_browser(url: str, workdir: pathlib.Path, feature: str) -> tuple[str, dict]:
    session = f"nori-{uuid.uuid4().hex[:10]}"
    config = workdir / "playwright-cli.json"
    config.write_text(json.dumps({"browser": {"launchOptions": {"headless": True, "args": [MEMORY_FLAG]}}}) + "\n")
    ...
    try:
        ...
        fixture = json.loads((ROOT / "tests/parity/nori/bindings.json").read_text())[feature]
        ...
        verify_run(observed["report"], feature)
...
def main() -> int:
    ...
    parser.add_argument("--nori", choices=["enabled", "disabled"], default="enabled")
    ...
    sources = [pathlib.Path(__file__), ROOT / "scripts/serve-wasm-tests.py", ROOT / "scripts/run-nori-benchmark.py", ROOT / "scripts/build-wasm.sh",
               ...
               ROOT / "tests/wasm/browser_nori.html", ROOT / "tests/wasm/browser_nori.mjs", ROOT / "tests/wasm/export_nori_diagnostics.py", ROOT / "tests/parity/nori/bindings.core.mjs",
               ROOT / "tests/parity/nori/bindings.json", ROOT / "examples/javascript/common.mjs", ROOT / "benchmarks/nori/browser-contract.json",
    ...
    if ("uqa-nori-data" in owners) != (args.nori == "enabled"):
        raise RuntimeError("The resolved WASM runtime dependency tree disagrees with the requested Nori feature")
    ...
    try:
        for _ in range(args.runs):
            query = urlencode({"run": uuid.uuid4().hex, "nori": args.nori, "bundle": "/" + relative_bundle.as_posix()})
            url = f"http://127.0.0.1:{server.server_port}/tests/wasm/browser_nori.html?{query}"
            version, report = run_browser(url, workdir, args.nori)
    ...
    if len(set(versions)) != 1:
        raise RuntimeError("Browser version changed during measurement")
    if identities != {path.relative_to(ROOT).as_posix(): benchmark.digest(path) for path in sources}:
        raise RuntimeError("Browser verification sources changed during measurement")
    if runtime_hash != benchmark.runtime_sources_hash(owners):
        raise RuntimeError("Rust runtime sources changed during measurement")
    if artifacts != [{"name": path.relative_to(ROOT).as_posix(), "bytes": path.stat().st_size, "sha256": benchmark.digest(path)} for path in artifact_paths]:
        raise RuntimeError("Browser artifacts changed during measurement")
    summary = {}
    ...
    result = {"schema_version": 1, "feature": args.nori, "runs": runs, "memory_summary": summary,
              "provenance": {"measured_at_utc": datetime.datetime.now(datetime.timezone.utc).isoformat(),
                             ...
                             "browser_flags": [MEMORY_FLAG], "playwright_cli": CLI_VERSION,
                             "rustc": benchmark.command("rustc", "-Vv"), "emcc": benchmark.command("emcc", "--version"),
                             "node": benchmark.command("node", "--version"), "cargo_features": args.nori,
              ...
              "gate": {"passed": True, "scope": "SQL, page reload, IndexedDB, complete diagnostic output, and memory observation completeness; no RSS or continuous-peak claim"}}
