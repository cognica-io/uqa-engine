...
- Advance SQLite main record format to 44, redb main record format to 43 and native SQLite mapping format to 9; catalog format remains 49. Atomic upgrades preserve records, identities, histories and identifier allocations, while retaining existing receipts as manually owned outcomes. Reopened and retained incompatible readers and writers are rejected. Restore a pre-upgrade backup to return to an earlier format. See the [writer compatibility contract](docs/manual/reference/10-upgrading.md#unreleased-mvcc-writer-compatibility).
