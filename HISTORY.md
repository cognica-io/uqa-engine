...
- **Node.js / TypeScript bindings:** the `uqa-node` crate (npm package `uqa`) exposes the full engine to Node and Bun through an async-first Node-API addon: `sql`, `sqlBatch`, every search surface, Cypher, and the calibration workflow return Promises computed on the libuv thread pool (with `sqlSync`-style twins for scripts), TypeScript definitions are generated from the Rust signatures, engine values map to native JavaScript types (`BigInt` beyond `Number.MAX_SAFE_INTEGER`, `Buffer` for bytes, insertion-ordered objects for rows), and `tests/node/test_uqa_node.mjs` mirrors the Python suite. JavaScript-defined SQL functions are deferred until a threadsafe-function design lands.
- **Python bindings catch-up:** the `uqa` package now exposes on-disk format autodetection (`uqa.open_auto(path, key=None)`, `uqa.detect_database_file(path)` returning `missing`/`sqlite`/`compressed`/`compressed_encrypted`/`unrecognized`), the full scoring-calibration workflow (`Engine.estimate_scoring_params`, `learn_scoring_params`, `update_scoring_params`, `calibration_report`, and `save`/`load`/`load_all`/`drop_scoring_params`, exchanging parameters as `dict[str, float]`), the PL/pgSQL notice sink (`Engine.take_sql_notices()`), and the routine recursion cap (`Engine.sql_function_depth_limit()` / `set_sql_function_depth_limit(limit)`), with matching type stubs and pytest coverage.

### Changed

- **Persistent scalar B-tree indexes:** PRIMARY KEY, UNIQUE, and catalog `btree` value-index postings now persist in SQLite schema v11, stay synchronized across insert, update, delete, truncate, DDL rename/drop, and transaction rollback, and hydrate the lazy in-memory B-tree from compact postings after reopen instead of rescanning large document JSON. Existing catalogs backfill each indexed field once on its first lookup.
...
- **Python bayesian search calibration:** `Engine.search(..., scoring="bayesian")` in the Python bindings resolves the persisted or auto-estimated per-field calibration through the now-public `Engine::bayesian_params_for`, matching SQL `text_match` and hybrid search; it previously scored with the identity calibration (`alpha = 1`, `beta = 0`) and ignored saved parameters.
