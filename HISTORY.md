...
- Merge independent SQLite Key/Value and redb writers sharing one IVF index, preserving ordered training changes, tensors, snapshots and savepoint undo. Retain conflicts for overlapping documents and index lifecycle changes, reject mismatched canonical inputs, and resolve publication retries with the original receipt identity. Development MVCC record format 3 atomically upgrades formats 1/2 without changing IVF row format 1; native SQLite IVF, HNSW merging and concurrent Engine SQL remain in progress.
