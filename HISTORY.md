...
- Refresh common Key/Value HNSW handles after rollback, savepoint branching, index recreation and sibling commits while retaining immutable graph snapshots. Compound reads and evaluated mutations now use one fixed view, preserve original write preconditions and discard failed or cancelled batches without replaying evaluation. SQLite Key/Value and redb share this implementation; custom HNSW providers must implement the [compound operation contract](docs/manual/reference/10-upgrading.md#unreleased-rust-keyvalue-compound-operations).
