...
use crate::legacy_vectors::{int2vector, oidvector};
...
fn partition_catalogs_bounds_and_deparsers_match_postgresql_18() {
    ...
    assert_eq!(row["partattrs"], int2vector(vec![Value::Int(1)]));
    assert_eq!(row["partclass"], oidvector(vec![Value::Int(1_978)]));
    assert_eq!(row["partcollation"], oidvector(vec![Value::Int(0)]));
...
fn assert_partition_catalog_routines(engine: &Engine) {
    ...
    assert_eq!(
        ...
        oidvector(vec![Value::Int(194), Value::Int(26)])
    );
    assert_eq!(
        routines.rows[1]["proargtypes"],
        oidvector(vec![Value::Int(194), Value::Int(26), Value::Int(16)])
    );
    assert_eq!(
