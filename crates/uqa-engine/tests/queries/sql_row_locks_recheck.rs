...
fn separate_processes_coordinate_row_locks() {
    ...
    let mut child = spawn_cross_process_child(
        "queries::sql_row_locks_recheck::cross_process_lock_holder_child",
        &database,
        directory.path(),
    );
...
fn stale_cross_process_rewrite_does_not_capture_a_reused_primary_key() {
    ...
    let mut child = std::process::Command::new(std::env::current_exe().unwrap())
        .arg("--exact")
        .arg("queries::sql_row_locks_recheck::cross_process_idle_attachment_child")
        .arg("--nocapture")
...
fn row_lock_rechecks_a_commit_after_the_external_writer_exits() {
    ...
    let mut child = std::process::Command::new(std::env::current_exe().unwrap())
        .arg("--exact")
        .arg("queries::sql_row_locks_recheck::cross_process_exit_writer_child")
        .arg("--nocapture")
...
fn separate_processes_detect_lock_cycles() {
    ...
    let mut child = std::process::Command::new(std::env::current_exe().unwrap())
        .arg("--exact")
        .arg("queries::sql_row_locks_recheck::cross_process_deadlock_child")
        .arg("--nocapture")
...
fn separate_processes_detect_cycles_that_include_a_local_wait_edge() {
    ...
    let mut child = std::process::Command::new(std::env::current_exe().unwrap())
        .arg("--exact")
        .arg("queries::sql_row_locks_recheck::mixed_process_deadlock_child")
        .arg("--nocapture")
    ...
    let mut observer = std::process::Command::new(std::env::current_exe().unwrap())
        .arg("--exact")
        .arg("queries::sql_row_locks_recheck::cross_process_wait_slot_observer_child")
        .arg("--nocapture")
