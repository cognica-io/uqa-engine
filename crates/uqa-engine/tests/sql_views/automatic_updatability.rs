...
fn automatically_updatable_views_match_postgresql_18() {
    ...
    assert_automatic_view_merge_rewrites_all_actions();
    assert_automatic_view_merge_filters_target_rows();
    assert_automatic_view_merge_check_options();
    assert_automatic_view_merge_errors();
...
fn assert_automatic_view_merge_rewrites_all_actions() {
    let engine = Engine::new();
    exec(
        &engine,
...
fn assert_automatic_view_merge_filters_target_rows() {
    let engine = Engine::new();
    exec(
        &engine,
...
fn assert_automatic_view_merge_check_options() {
...
fn assert_automatic_view_merge_errors() {
    let engine = automatic_merge_check_engine();
    let computed = engine
        .sql(
            "MERGE INTO merge_check_view AS target
