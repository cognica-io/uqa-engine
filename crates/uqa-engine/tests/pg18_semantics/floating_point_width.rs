...
//! Floating-point conversion and arithmetic against a live `PostgreSQL` reference.

use uqa_core::Value;
use uqa_engine::sql::{format_postgres_text, postgres_result_type};
use uqa_engine::Engine;

fn verify_floating_point_width(engine: &Engine) {
    let oracle: serde_json::Value = serde_json::from_str(include_str!(
        "../../../../tests/parity/pg18/floating_point_width_oracle.expected.json"
    ))
    .unwrap();
...
#[test]
fn floating_point_width_matches_postgresql_memory() {
    verify_floating_point_width(&Engine::new());
}

#[test]
fn floating_point_width_matches_postgresql_sqlite() {
    let directory = tempfile::tempdir().unwrap();
    verify_floating_point_width(&Engine::open(&directory.path().join("floating.db")).unwrap());
}
