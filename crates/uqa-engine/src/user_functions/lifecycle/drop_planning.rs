...
fn append_schema_function_dependents(
    ...
    for column in columns {
        if let Some(generated) = &column.generated {
            let referenced = generated
                .function_dependencies
                .iter()
                .any(|dependency| crate::session::function_binding_matches(dependency, target))
                || crate::events::expression_references_routine_identity(
        ...
        if let Some(check) = &column.check {
            if crate::events::expression_references_routine_identity(check, target)? {
    ...
    for check in checks {
        if crate::events::expression_references_routine_identity(&check.expr, target)? {
...
impl Engine {
    ...
    pub(in crate::user_functions) fn resolve_sql_routine_alter_target(
        &self,
        registry: &BTreeMap<String, Vec<Arc<SQLUserFunction>>>,
        requested_name: &str,
