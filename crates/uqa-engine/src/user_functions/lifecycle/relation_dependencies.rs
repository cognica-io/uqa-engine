...
impl Engine {
    pub(super) fn sequence_drop_column_names(
        ...
        for sequence in sequences {
            ...
            {
                if let crate::sequences::SequenceSchemaDependent::GeneratedColumn {
                    table,
                    column,
                    ..
                } = dependent
    ...
    pub(super) fn stored_routine_references_relations(
        ...
        {
            crate::events::visit_stored_expression(&mut default.clone(), &mut visit)?;
        }
        if let FunctionBody::Statements(statements) = &definition.body {
            for statement in statements {
                if crate::events::stored_statement_relation_names(statement)?
                    .iter()
                    .any(|name| relations.contains(name))
                {
                    return Ok(true);
                }
                crate::events::visit_stored_statement_expressions(
                    &mut statement.clone(),
                    &mut visit,
                )?;
            }
