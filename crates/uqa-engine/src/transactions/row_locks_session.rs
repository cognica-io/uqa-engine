...
impl uqa_execution::row_locks::retry_cache::CommittedRowSource for crate::Engine {
    fn get_committed_document(
        &self,
        table: &str,
        doc_id: uqa_core::DocId,
    ) -> Result<Option<uqa_storage::StoredDocument>, uqa_sql::SQLError> {
        crate::Engine::get_committed_document(self, table, doc_id)
    }
}
