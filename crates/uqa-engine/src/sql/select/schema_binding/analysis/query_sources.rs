...
use uqa_execution::{ColumnIdentity, RowSchema, ScalarExpr};
use uqa_planner::{ProjectionPlan, QueryBlockPlan};
use uqa_sql::ColumnType;

pub(in crate::sql::select::schema_binding) fn with_query_source_columns(
    schema: &RowSchema,
    ...
    let nullable = crate::sql::select::outer_join_nullable_qualifiers(source);
    let mut scored = BTreeSet::new();
    for part in crate::sql::select::flatten_and_filter_parts(predicate) {
        if !uqa_planner::optimizer::contains_retrieval(part)
            || super::super::expr_contains_subquery(part)
        {
            continue;
        }
        let mut qualifiers = crate::sql::select::expr_qualifiers(part);
...
pub(in crate::sql::select::schema_binding) fn with_projected_open_columns(
    output: &RowSchema,
