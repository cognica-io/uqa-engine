...
use uqa_planner::{AssignmentPlan, CommandPlan, ConflictActionPlan, MergeWhenPlan};
...
impl Preparation<'_> {
    ...
    fn assignment(
        ...
        if let Some(source) = &value.ty {
            if !uqa_execution::assignment_type_compatible(source, ty) {
                return Err(error(
                    "42804",
                    format!(
