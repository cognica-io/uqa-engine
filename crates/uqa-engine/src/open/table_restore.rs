...
fn load_constraint_metadata_migrations(
    ...
    for schema in catalog.load_tables()? {
        ...
        let dispatches_changed = crate::table_storage::upgrade_legacy_schema_function_dispatches(
            &mut columns,
            &mut constraints,
        );
        let metadata_changed = crate::table_storage::materialize_constraint_metadata(
            &schema.relation,
            &mut columns,
            &mut constraints,
        )?;
    ...
    parents.iter().find_map(|parent| {
        parent_foreign_keys.get(parent).and_then(|foreign_keys| {
            foreign_keys
                .iter()
                .find(|foreign_key| {
                    crate::table_storage::foreign_keys_match_without_object_id(
                        foreign_key,
                        inherited,
                    )
                })
...
fn apply_inherited_object_id(
    ...
    if let Some(foreign_key) = migration
        ...
        .iter_mut()
        .find(|foreign_key| {
            crate::table_storage::foreign_keys_match_without_object_id(foreign_key, &inherited)
        })
    {
