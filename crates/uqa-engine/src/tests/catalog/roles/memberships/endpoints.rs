...
fn target_wait_reads_current_member_defaults_and_retains_original_endpoint_identities() {
    for provider in 0..3 {
        ...
        for (isolation_index, isolation) in ["READ COMMITTED", "REPEATABLE READ", "SERIALIZABLE"]
            .into_iter()
            .enumerate()
        {
            for (case, change, explicit, expected, inherit) in [
                ...
                let row = memberships.values().find(|row| {
                    row.role.identity() == target_id && row.member.identity() == original
                });
                if expected == "00000" {
