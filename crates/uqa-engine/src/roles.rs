...
fn table_security_depends_on_role(security: &crate::state::TableSecurity, role: &str) -> bool {
...
fn dependent_sequence_for_role<'a>(
    sequences: &'a BTreeMap<crate::RelationIdentity, crate::state::SequenceSecurity>,
    role: &str,
...
fn dependent_schema_for_role<'a>(
    schemas: &'a BTreeMap<String, crate::state::SchemaSecurity>,
    role: &str,
) -> Option<&'a String> {
...
fn database_depends_on_role(security: &crate::state::DatabaseSecurity, role: &str) -> bool {
    let acl_dependency = security.acl.as_ref().is_some_and(|acl| {
        acl.iter().any(|entry| {
            entry.role == role || entry.grantor.as_deref().unwrap_or(&security.role_owner) == role
        })
