...
impl Engine {
    pub fn automatic_statistics_status(&self) -> AutomaticStatisticsStatus {
        self.row_locks.automatic_statistics.status.lock().clone()
    }
    ...
    pub(crate) fn start_automatic_statistics(&self) {
        ...
        let automatic = &self.row_locks.automatic_statistics;
        ...
        match std::thread::Builder::new()
            ...
            .spawn(move || worker::run(&provider, &manager, &receiver, &worker_cancellation))
        {
            Ok(thread) => {
    ...
    pub(crate) fn wake_automatic_statistics(&self) {
        ...
        if let Some(worker) = self.row_locks.automatic_statistics.control.lock().as_ref() {
