...
//! Query-local corpus statistics and durable scorer identities.

use crate::{BM25Params, ScoringMode};
use std::collections::BTreeSet;
use uqa_core::IndexStats;
use uqa_storage::{InvertedIndex, StorageBackendResult, TokenTermKey, DEFAULT_BLOCK_SIZE};
...
pub(super) fn search_stats_for_terms(
    ...
    let mut seen = BTreeSet::<&TokenTermKey>::new();
    for (term, doc_freq) in terms.iter().zip(doc_freqs) {
        if seen.insert(term) {
            stats.set_doc_freq_utf16(field.to_string(), term.to_term().into_utf16(), *doc_freq);
        }
    }
    Ok(stats)
}
