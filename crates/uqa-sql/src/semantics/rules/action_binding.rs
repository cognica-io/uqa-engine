...
use crate::plpgsql::{bind_statement, ResolvedVariable, VariableResolver};
use crate::SQLError;
...
pub use returning::expand_rule_action_returning_stars;
pub use row_expansion::expand_rule_action_row_stars;
use scope::{
    apply_positional_aliases, collect_visible_scope, cte_output_columns, RuleBindingScope,
};

pub fn action_target_qualifier_referenced(
    catalog: &dyn RuleSourceCatalog,
    action: &Statement,
    qualifier: &str,
) -> bool {
    namespace::action_target_qualifier_referenced(catalog, action, qualifier)
}

mod context;
use context::RuleBindingContext;
pub use context::{rule_action_target_columns, rule_action_target_row_type, RuleSourceCatalog};
...
pub fn bind_rule_action(
    ...
    let context = RuleBindingContext::with_catalog(catalog, true);
...
fn bind_rule_select_scoped(
    ...
    bind_select_with_scope(
        ...
        resolver,
        &RuleBindingScope::default(),
        &RuleBindingContext::with_catalog(catalog, false),
    )
}
...
fn bind_select_with_scope(
    ...
    Ok(SelectStmt {
        ...
        set_op: select
            .set_op
            .as_ref()
            .map(|operation| -> Result<Box<crate::ast::SetOp>, SQLError> {
                Ok(Box::new(crate::ast::SetOp {
                    kind: operation.kind,
                    all: operation.all,
                    left: operation
                        .left
    ...
    for cte in ctes {
        ...
        bound.push(CTE {
            ...
            cycle: cte
                .cycle
                .as_ref()
                .map(|cycle| -> Result<crate::ast::CteCycleClause, SQLError> {
                ...
                .transpose()?,
            body: match &cte.body {
                crate::ast::CteBody::Query(query) => crate::ast::CteBody::Query(Box::new(
