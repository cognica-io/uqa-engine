...
use super::{
    ...
    ColumnIdentity, ConflictActionPlan, ConflictPlan, CteScope, DeletePlan, ExpressionScope,
    InsertPlan, MergePlan, MergeWhenPlan, ProjectionPlan, QueryPlan, RelationalPlan,
    ReturningAliases, RowSchema, SQLError, ScalarExpr, SourcePlan, UpdatePlan, ViewRewriteContext,
};
...
pub(super) fn dml_analysis_scope(
    ...
    let mut scope = services.catalog.binding_scope()?;
...
pub(super) fn collect_expression_subquery_ids<'a>(
    ...
    for expression in expressions {
        crate::semantics::collect_subquery_ids(expression, &mut ids);
    }
    ids
}
...
fn dml_correlated_outer_schema(
    ...
    let target = public_view_row_schema(services, &layer.canonical_name, target_qualifier)?;
...
fn validate_correlated_subquery_ids(
    ...
    let scope = dml_analysis_scope(services, ctes, subqueries, inherited_ctes)?;
    for id in ids {
        let query = subqueries.get(*id).ok_or_else(|| {
            SQLError::Internal(format!("DML scalar subquery slot {id} is out of bounds"))
        })?;
        crate::semantics::view_rewrite::context::analyze_query_plan_schema(
            services,
            query,
            params,
            &scope,
    ...
    match &mut query.root {
        RelationalPlan::QueryBlock(block) => {
            let local_schema = block
                ...
                .as_ref()
                .map(|source| {
                    crate::semantics::view_rewrite::context::analyze_source_plan_schema(
                        context.services,
                        source,
                        context.params,
                        &query_scope,
...
pub(super) fn dml_source_schema(
    ...
    let scope = dml_analysis_scope(services, ctes, subqueries, inherited_ctes)?;
    crate::semantics::view_rewrite::context::analyze_source_plan_schema(
        services, source, params, &scope, None,
    )
    .map(Some)
}
...
pub(super) fn insert_input_width(
    ...
    let scope = dml_analysis_scope(services, &plan.ctes, &plan.subqueries, inherited_ctes)?;
    Ok(
        crate::semantics::view_rewrite::context::analyze_query_plan_schema(
            services, source, params, &scope, None,
        )?
...
pub(super) struct CorrelatedDmlContext<'a> {
    pub(super) services: ViewRewriteContext<'a>,
