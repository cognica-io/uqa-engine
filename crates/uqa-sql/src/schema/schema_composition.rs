...
use super::{
    ColumnIdentity, ColumnType, RowSchema, SchemaBuildMetadata, SchemaLayoutError,
    SchemaLayoutResult, NULL_SLOT,
};
...
impl RowSchema {
    ...
    pub fn append_internal_typed(
        input: &Self,
        columns: &[(crate::ast::InternalColumnRef, Option<ColumnType>)],
    ) -> Self {
        let base = input.physical_width();
    ...
    /// Map logical and hidden identities into `target`'s complete slot layout. Visible columns are matched by logical position; hidden lookup aliases are matched by their structured identity. This is used when two equivalent operator pipelines expose the same logical row through different physical slot arrangements.
    pub fn relayout_slots(&self, target: &Self) -> SchemaLayoutResult<Vec<usize>> {
        fn assign(
            ...
            match source_slots[target_slot] {
                Some(existing) if existing != source_slot => Err(SchemaLayoutError(format!(
