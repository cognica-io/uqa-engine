...
impl RowSchema {
    ...
    pub fn slot(&self, logical: usize) -> Option<usize> {
        self.index
            .slots
            .get(logical)
            .copied()
    ...
    pub fn exact_slot(&self, name: &str) -> Option<usize> {
        self.index
            .exact
            .get(name)
    ...
    pub fn qualified_slot(&self, qualifier: &str, column: &str) -> Option<usize> {
        let identity = ColumnIdentity::qualified(qualifier, column);
        if self.index.ambiguous_qualified.contains(&identity) {
            return None;
        }
...
impl<'a> IntoIterator for &'a RowSchema {
    type Item = &'a String;
