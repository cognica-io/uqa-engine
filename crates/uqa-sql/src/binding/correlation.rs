...
use crate::plan::{ComputePlan, ProjectionPlan, QueryPlan, RelationalPlan, SourcePlan};
use crate::SQLError;
use crate::{ScalarExpr, ScalarFrameBound};

use crate::catalog::{analysis::AnalysisCatalog, resolution::RelationNameResolution};
use crate::semantics::projection_columns;

/// A single immutable namespace and catalog view used throughout correlation analysis.
#[derive(Clone, Copy)]
...
struct QueryScope {
    ...
    internal_relations: BTreeSet<crate::ast::InternalRelationId>,
    columns: RelationColumns,
}

pub struct DecorrelatedExistsPlan {
    pub inner: QueryPlan,
    pub outer_keys: Vec<ScalarExpr>,
}
...
pub fn decorrelate_exists(
    ...
    let scope = source_scope(context, source, &BTreeMap::new())?;
    let mut source_scopes = vec![scope.clone()];
    if source_has_external_reference(context, source, &mut source_scopes)? {
        return Ok(None);
    }
...
pub fn query_depends_on_outer_row(
    context: CorrelationContext<'_>,
    plan: &QueryPlan,
...
fn query_has_external_reference(
    ...
    for cte in &plan.ctes {
        ...
        if cte.body.query().map_or(Ok(true), |query| {
            query_has_external_reference(context, query, scopes)
        })? {
            return Ok(true);
        }
        ctes.insert(cte.name.clone(), columns);
    ...
    match &plan.root {
        RelationalPlan::QueryBlock(block) => {
            let scope = match block.from.as_ref() {
                Some(source) => source_scope(context, source, &ctes)?,
                None => QueryScope {
            ...
            let result = (|| {
                ...
                if let Some(source) = block.from.as_ref() {
                    if source_has_external_reference(context, source, scopes)? {
                        return Ok(true);
                    }
                }
                for subquery in &block.subqueries {
                    if query_has_external_reference(context, subquery, scopes)? {
                        return Ok(true);
                    }
                }
                Ok(false)
        ...
        RelationalPlan::SetOp {
            ...
            if query_has_external_reference(context, left, scopes)?
                || query_has_external_reference(context, right, scopes)?
            {
                return Ok(true);
            }
    ...
    match source {
        SourcePlan::Table {
            ...
            let mut columns = match bound_columns {
                ...
                None => relation_columns(context, name, ctes)?,
            };
        ...
        SourcePlan::Join {
            ...
            let left = source_scope(context, left, ctes)?;
            let right = source_scope(context, right, ctes)?;
...
fn relation_columns(
    ...
    if let Some(columns) = catalog.virtual_relation_schema(resolution, name)? {
        return Ok(RelationColumns::known(
            columns.into_iter().map(|(name, _)| name).collect(),
        ));
    }
    if let Ok(Some(table)) = catalog.table_resolved(resolution, name) {
        return Ok(RelationColumns::known(
            table
                .columns
                .iter()
    ...
    if let Some(view) = catalog.view_resolved(resolution, name)? {
        return Ok(if view.materialized {
            RelationColumns::known(view.output_columns.unwrap_or_default())
        } else {
            query_output_columns(&view.query)
        });
    }
    if let Ok(Some(table)) = catalog.foreign_table_resolved(resolution, name) {
        return Ok(RelationColumns::known(
            table
                .columns
                .iter()
...
fn source_has_external_reference(
    ...
    match source {
        SourcePlan::Join {
            left, right, on, ..
        } => Ok(source_has_external_reference(context, left, scopes)?
            || source_has_external_reference(context, right, scopes)?
            || on
                .as_ref()
                .is_some_and(|expr| expression_has_external_reference(expr, scopes))),
        ...
        SourcePlan::Subquery { body, .. } => query_has_external_reference(context, body, scopes),
...
mod tests {
    ...
    fn table_range_aliases_replace_physical_correlation_names() {
        let crate::ast::Statement::CreateTable(definition) =
            crate::compile("CREATE TABLE correlation_alias_source(id INTEGER, label TEXT)")
                .unwrap()
                .remove(0)
        else {
        ...
        let catalog = crate::binding::fixture::catalog(BTreeMap::from([(
            crate::RelationIdentity::from_legacy_name("public.correlation_alias_source").unwrap(),
            crate::binding::fixture::table_definition(definition.columns),
        )]));
        let resolution =
            crate::binding::fixture::resolution(vec!["public".into()], "pg_temp_1".into());
        ...
        let scope = source_scope(context, &source, &BTreeMap::new()).unwrap();
        assert_eq!(
            scope.columns.names,
