...
use crate::plan::{ProjectionPlan, QueryBlockPlan, QueryPlan};
use crate::RowSchema;
use crate::{SQLError, SQLParam};

use super::{
    bind_query_plan_schema, projection_columns, BindingContext, QueryFunctionTypeResolver,
    ScalarExpr, SchemaScope,
};
...
pub fn validate_query_block_expression_types(
    routines: &dyn RoutineResolution,
    ...
    for expression in statement
        ...
        .chain(statement.distinct_on.iter())
    {
        if let Some(ty) = crate::scalar_type_with_resolver(expression, schema, params, &resolver)? {
            crate::require_equality_operator(&ty)?;
        }
    }
    for order in &statement.order_by {
        if let Some(ty) = crate::scalar_type_with_resolver(&order.expr, schema, params, &resolver)?
        {
...
pub(super) fn rename_schema(
    ...
    for (identity, ty) in schema.typed_virtual_identities() {
        ...
        let mapped = qualifier.map_or_else(
            ...
            |qualifier| {
                vec![
                    crate::ColumnIdentity::unqualified(identity.column()),
                    crate::ColumnIdentity::qualified(qualifier, identity.column()),
                ]
            },
        );
