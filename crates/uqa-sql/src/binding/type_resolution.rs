...
use crate::ast::{ColumnType, SetOpKind};
use crate::{FunctionTypeResolver, ResolvedFunctionOverload, RowSchema};
        ...
        match result {
            Err(error)
                if self.defer_routine_namespace_errors
                    && crate::routines::is_routine_namespace_lookup_error(&error) =>
            {
                Ok(None)
            }
            result => result,
