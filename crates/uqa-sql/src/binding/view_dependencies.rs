...
pub fn bind_query_plan_sequence_references<E>(
    plan: &mut QueryPlan,
...
pub fn bind_query_plan_relations<E>(
    plan: &mut QueryPlan,
...
pub fn bind_source_plan_relations<E>(
...
pub fn relation_reference_matches(reference: &str, target: &RelationIdentity) -> bool {
...
pub fn source_plan_references_relation(
    ...
    match source {
        ...
        crate::plan::SourcePlan::Subquery { body, .. } => {
            query_plan_references_relation(body, target, ctes)
        }
        crate::plan::SourcePlan::Function { relations, .. } => {
        ...
        crate::plan::SourcePlan::Values { .. } => false,
    }
}

pub fn query_plan_references_relation(
    ...
    match &query.root {
        ...
        crate::plan::RelationalPlan::Values { subqueries, .. } => subqueries
            .iter()
            .any(|query| query_plan_references_relation(query, target, &ctes)),
    }
}

pub fn query_plan_references_sequence(plan: &QueryPlan, target: &RelationIdentity) -> bool {
...
pub use crate::routines::function_binding_matches;
...
pub fn query_plan_has_legacy_routine_identity(plan: &QueryPlan) -> bool {
...
pub fn query_plan_references_function(plan: &QueryPlan, target: &FunctionBinding) -> bool {
    let mut scalar_plan = plan.clone();
    let mut referenced = false;
    scalar_plan.rewrite_scalar_expressions(&mut |expression| {
        if let ScalarExpr::Func {
...
pub fn rewrite_query_plan_routine_identity(
    plan: &mut QueryPlan,
    target: &FunctionBinding,
    new_name: &str,
) -> bool {
...
pub fn bind_cte_plan_relations<E>(
...
fn cte_relational_inputs_any(
    ...
    source: &dyn Fn(&crate::plan::SourcePlan) -> bool,
) -> bool {
    match body {
        crate::plan::CtePlanBody::Query(plan) => query(plan),
        crate::plan::CtePlanBody::Command(command) => {
            command
                .ctes()
                .iter()
