...
use crate::ast::{RoleAttribute, TablePrivilege, TablePrivilegeSpec};
use crate::SQLError;

use super::TableSecurity;
use crate::catalog::roles::{role_inherits, RoleDefinition, RoleMembership, RoleMembershipKey};

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub enum TableAclPrivilege {
    Select,
    Insert,
    Update,
    Delete,
...
pub struct TablePrivilegeCheck {
    pub privilege: TableAclPrivilege,
    pub grant_option: bool,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RequestedTablePrivileges {
    pub table: Vec<TableAclPrivilege>,
    pub columns: Vec<(TableAclPrivilege, String)>,
}
...
impl TableAclPrivilege {
    ...
    pub const COLUMN_ALL: [Self; 4] = [Self::Select, Self::Insert, Self::Update, Self::References];

    pub const fn mask(self) -> TablePrivileges {
...
pub fn parse_privilege_checks(value: &str) -> Result<Vec<TablePrivilegeCheck>, SQLError> {
    value
        .split(',')
        .map(|item| {
            let item = item.trim();
...
pub fn parse_column_privilege_checks(value: &str) -> Result<Vec<TablePrivilegeCheck>, SQLError> {
...
pub fn acl_grantor<'a>(entry: &'a TableAclEntry, owner: &'a str) -> &'a str {
...
pub fn rewrite_acl_owner(security: &mut TableSecurity, new_owner: &str) {
...
pub fn role_can_view_table(
    ...
    TableAclPrivilege::ALL.into_iter().any(|privilege| {
        ...
        TableAclPrivilege::COLUMN_ALL.into_iter().any(|privilege| {
            super::columns::role_has_column_privilege(
                security,
                column,
                subject,
                TablePrivilegeCheck {
