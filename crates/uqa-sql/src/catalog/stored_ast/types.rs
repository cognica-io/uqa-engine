...
pub fn stored_statement_relation_names(statement: &Statement) -> Result<Vec<String>, SQLError> {
...
pub fn stored_expression_type_names(expression: &Expr) -> Result<Vec<String>, SQLError> {
    collect_type_names(|visitor| visitor.bind_expr(&mut expression.clone(), &BTreeSet::new()))
}

pub fn stored_statement_type_names(statement: &Statement) -> Result<Vec<String>, SQLError> {
...
type TypeVisitor<'a> = StoredAstVisitor<
    ...
    fn(&mut String, Option<&mut Option<crate::ast::FunctionBinding>>) -> Result<(), SQLError>,
>;
