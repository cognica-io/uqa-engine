...
use crate::canonical_routine_type_name;
use crate::SQLError;
...
pub fn catalog_regtype_name(oid: i64) -> Option<&'static str> {
    match oid {
        24 => Some("regproc"),
...
pub fn routine_type_oid(type_name: &str) -> i64 {
...
pub fn routine_variadic_element_oid(type_name: &str) -> Result<i64, SQLError> {
    let canonical = canonical_routine_type_name(type_name);
...
pub fn pg_type_len(ty: &ColumnType) -> i64 {
    match ty {
        ColumnType::Named(name) => {
            unreachable!("unresolved declaration type {name} reached catalog projection")
        }
...
pub fn pg_type_by_value(ty: &ColumnType) -> bool {
    matches!(
        ty,
...
pub fn pg_type_align(ty: &ColumnType) -> &'static str {
    match ty {
        ColumnType::Named(name) => {
            unreachable!("unresolved declaration type {name} reached catalog projection")
        }
...
pub fn pg_type_storage(ty: &ColumnType) -> &'static str {
    match ty {
        ColumnType::Named(name) => {
            unreachable!("unresolved declaration type {name} reached catalog projection")
        }
...
pub fn pg_type_array_oid(ty: &ColumnType) -> i64 {
    match ty {
        ColumnType::Named(name) => {
            unreachable!("unresolved declaration type {name} reached catalog projection")
        }
...
pub fn pg_type_collation_oid(ty: &ColumnType) -> i64 {
    let scalar = match ty {
        ColumnType::Named(name) => {
            unreachable!("unresolved declaration type {name} reached catalog projection")
        }
...
pub fn pg_type_subscript_handler(ty: &ColumnType) -> i64 {
    match ty {
        ColumnType::Named(name) => {
            unreachable!("unresolved declaration type {name} reached catalog projection")
        }
...
pub fn pg_type_modifier(ty: &ColumnType) -> i64 {
...
pub struct PgTypeRoutineOids {
    pub input: i64,
    pub output: i64,
    pub receive: i64,
    pub send: i64,
    pub modifier_input: i64,
    pub modifier_output: i64,
    pub analyze: i64,
}
...
pub fn pg_type_routine_oids(ty: &ColumnType) -> PgTypeRoutineOids {
