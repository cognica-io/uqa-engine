...
use crate::{expr::cast_value, plpgsql::condition_sqlstates, SQLError, SQLResult};
use uqa_core::Value;

pub fn return_query_context_error() -> SQLError {
    SQLError::Routine {
        sqlstate: "42601".into(),
...
pub fn result_row_values(result: &SQLResult, row: usize) -> Option<Vec<Value>> {
...
pub fn result_row_count(result: &SQLResult) -> Result<i64, SQLError> {
...
pub fn strict_into_check(row_count: i64) -> Result<(), SQLError> {
    if row_count == 0 {
...
pub fn to_i64_value(value: &Value) -> Result<i64, SQLError> {
...
pub fn looks_like_sqlstate(text: &str) -> bool {
...
pub fn sqlstate_matches(condition: &str, state: &str) -> bool {
    condition == state || (condition.ends_with("000") && state.get(..2) == condition.get(..2))
}

/// Substitute `%` placeholders in a RAISE format string.
pub fn format_raise_message(format: &str, args: &[Value]) -> Result<String, SQLError> {
