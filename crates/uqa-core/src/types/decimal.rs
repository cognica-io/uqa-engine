...
impl DecimalValue {
    ...
    /// Sample uniformly from the inclusive finite decimal range using raw
    /// random words supplied by the caller. The result retains the larger
    /// display scale of the two bounds, matching `PostgreSQL`'s
    /// `random(numeric, numeric)` contract. `Ok(None)` reports non-finite,
    /// reversed, or unrepresentable bounds without coupling this value type
    /// to SQL error reporting.
