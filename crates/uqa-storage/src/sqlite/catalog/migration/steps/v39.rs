//
// Unified Query Algebra
//
// Copyright (c) 2023-2026 Cognica, Inc.
//

//! Persist regular-view and materialized-view access-control lists.

use super::super::super::{Catalog, Result};

pub(super) fn migrate(tx: &rusqlite::Transaction<'_>) -> Result<()> {
    let columns = Catalog::table_columns(tx, "_views")?.unwrap_or_default();
    if !columns.contains_key("acl_json") {
        tx.execute_batch("ALTER TABLE _views ADD COLUMN acl_json TEXT;")?;
    }
    if !columns.contains_key("column_acls_json") {
        tx.execute_batch("ALTER TABLE _views ADD COLUMN column_acls_json TEXT;")?;
    }
    Ok(())
}
