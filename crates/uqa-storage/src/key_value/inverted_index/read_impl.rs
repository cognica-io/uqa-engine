...
//! Posting queries evaluated through one fixed read boundary.
...
impl OccurrenceRead<'_> {
    ...
    pub(super) fn get_occurrences_budgeted(
        &self,
        doc_id: DocId,
        field: &str,
        term: &TokenTermKey,
    ...
    pub(super) fn field_stats_scalar_budgeted(
        &self,
        field: &str,
        control: &crate::read_control::StorageReadControl,
    ...
    pub(super) fn source_rebuild_required(&self) -> StorageBackendResult<bool> {
        self.needs_source_rebuild()
    }

    pub(super) fn get_posting_list(
        &self,
        field: &str,
        term: &str,
    ...
    pub(super) fn get_posting_list_key(
        &self,
        field: &str,
        term: &TokenTermKey,
    ) -> StorageBackendResult<PostingList> {
    ...
    pub(super) fn posting_cursor_key(
        &self,
        field: &str,
        term: &TokenTermKey,
    ) -> StorageBackendResult<Box<dyn PostingCursor>> {
    ...
    pub(super) fn get_occurrence_postings(
        &self,
        field: &str,
        term: &TokenTermKey,
    ...
    pub(super) fn indexed_field_metadata(
        &self,
        doc_id: DocId,
        field: &str,
    ...
    pub(super) fn doc_freq(&self, field: &str, term: &str) -> StorageBackendResult<u64> {
        self.doc_freq_key(field, &TokenTermKey::from_text(term))
    }

    pub(super) fn doc_freq_key(
        &self,
        field: &str,
        term: &TokenTermKey,
    ) -> StorageBackendResult<u64> {
        self.require_graph_format()?;
        self.scan_prefix(&keys::term_prefix(self.table, keys::SCORE, field, term)?)?
            .iter()
    ...
    pub(super) fn get_doc_length(&self, doc_id: DocId, field: &str) -> StorageBackendResult<u64> {
        self.document_length(doc_id, field)
    }

    pub(super) fn get_scoring_inputs_bulk(
        &self,
        doc_ids: &[DocId],
        field: &str,
        terms: &[String],
    ...
    pub(super) fn get_scoring_inputs_keys_bulk(
        &self,
        doc_ids: &[DocId],
        field: &str,
        terms: &[TokenTermKey],
    ...
    pub(super) fn get_term_freq_key(
        &self,
        doc_id: DocId,
        field: &str,
        term: &TokenTermKey,
    ...
    pub(super) fn field_doc_count(&self, field: &str) -> StorageBackendResult<u64> {
        self.require_graph_format()?;
        Ok(self
            .stored_field_stats(field)?
    ...
    pub(super) fn vocabulary_terms(&self, field: &str) -> StorageBackendResult<Vec<String>> {
    ...
    pub(super) fn vocabulary_keys(&self, field: &str) -> StorageBackendResult<Vec<TokenTermKey>> {
        self.indexed_terms(Some(field))
    }

    pub(super) fn stats(&self) -> StorageBackendResult<IndexStats> {
        self.index_statistics()
    }

    pub(super) fn posting_count(&self, field: Option<&str>) -> StorageBackendResult<u64> {
        self.scan_prefix(&self.score_prefix(field)?)?
            .iter()
    ...
    pub(super) fn doc_length_count(&self, field: Option<&str>) -> StorageBackendResult<u64> {
        self.require_graph_format()?;
    ...
    pub(super) fn term_count(&self, field: Option<&str>) -> StorageBackendResult<u64> {
        usize_to_u64(self.indexed_terms(field)?.len(), "term count")
    }

    pub(super) fn field_names(&self) -> StorageBackendResult<Vec<FieldName>> {
        self.require_graph_format()?;
