...
fn diskann_canonical_origin_codec_matches_explicit_fields_and_rejects_malformed_records() {
    ...
    assert_eq!(
        Record::decode(&expected, 3).unwrap().version,
        record.version
    );
    for length in 0..56 {
        assert!(Record::decode(&bytes[..length], 3).is_err());
    }
    assert!(Record::decode(&[0; 57], 3).is_err());
    assert!(Record::decode(&bytes, 4).is_err());
    for (offset, value) in [(0, 0), (24, 0), (32, 0), (44, 1), (53, 1)] {
        let mut malformed = bytes;
        malformed[offset] = value;
        assert!(Record::decode(&malformed, 3).is_err());
    }
    for count in [0, 1, 1_u64 << 32] {
        let record = Record { count, ..record };
        assert_eq!(Record::decode(&record.encode(), 3).unwrap().count, count);
    }
}
