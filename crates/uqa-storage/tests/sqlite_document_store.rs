...
fn integer_column() {
    let (_dir, _conn, mut store) = make_store("t1");
    store.put(1, doc([("val", Value::Int(42))]));
...
fn text_column() {
    let (_dir, _conn, mut store) = make_store("t1");
    store.put(1, doc([("val", Value::Str("hello".into()))]));
...
fn boolean_stored_as_integer() {
    let (_dir, _conn, mut store) = make_store("t1");
    store.put(1, doc([("val", Value::Int(1))]));
    store.put(2, doc([("val", Value::Int(0))]));
...
fn blob_column() {
    ...
    store.put(1, doc([("val", Value::Bytes(data.clone()))]));
...
fn get_field_null_returns_none() {
    let (_dir, _conn, mut store) = make_store("t1");
    store.put(1, doc([("id", Value::Int(1))]));
...
fn doc_ids_after_delete() {
    let (_dir, _conn, mut store) = make_store("t1");
    store.put(1, doc([("id", Value::Int(1))]));
    store.put(2, doc([("id", Value::Int(2))]));
    store.delete(1);
...
fn max_doc_id_single_row() {
    let (_dir, _conn, mut store) = make_store("t1");
    store.put(7, doc([("id", Value::Int(7))]));
...
fn max_doc_id_multiple_rows() {
    let (_dir, _conn, mut store) = make_store("t1");
    store.put(3, doc([("id", Value::Int(3))]));
    store.put(10, doc([("id", Value::Int(10))]));
    store.put(5, doc([("id", Value::Int(5))]));
...
fn eval_path_nested_path_falls_back_to_dict_traversal() {
    let (_dir, _conn, mut store) = make_store("t1");
    store.put(1, doc([("name", Value::Str("alice".into()))]));
    assert_eq!(
        store.eval_path(
            1,
            &[
...
fn different_schemas() {
    ...
    store_1.put(1, doc([("val", Value::Int(100))]));
    store_2.put(
        1,
        doc([
            ("a", Value::Str("x".into())),
...
fn idempotent_table_creation() {
    ...
    store1.put(1, doc([("val", Value::Int(42))]));
