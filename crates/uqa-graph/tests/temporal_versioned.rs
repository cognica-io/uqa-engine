...
use uqa_graph::{
    GraphDelta, GraphStore, MemoryGraphStore, TemporalFilter, TemporalTraverse, VersionedGraphStore,
};
...
fn temporal_filter_range_overlap_check() {
    ...
    assert!(overlapping.is_valid(&props));
    let disjoint = TemporalFilter::Range(300.0, 400.0);
    assert!(!disjoint.is_valid(&props));
    let edge_touch = TemporalFilter::Range(200.0, 300.0);
    assert!(edge_touch.is_valid(&props));
}
