...
fn versioned_overwrites_restore_global_records_and_exact_memberships() {
    let mut stores = vec![crate::GraphStoreHandle::default()];
    stores.extend(
        fixtures()
            .into_iter()
            .map(|(catalog, backend)| crate::GraphStoreHandle::from_catalog(catalog, backend)),
    );
    for mut store in stores {
        ...
        {
            let mut versioned = crate::VersionedGraphStore::new(&mut store, "target");
            let mut delta = crate::GraphDelta::new();
...
fn durable_path_index_pages_invalidate_with_shared_graph_writes_and_rollback() {
    ...
    for (catalog, backend) in fixtures() {
        ...
        let first = crate::PathIndex::build_persistent(
            Arc::clone(&catalog),
            Arc::clone(&backend),
            "first::paths",
            "first",
        ...
        .unwrap();
        let second = crate::PathIndex::build_persistent(
            Arc::clone(&catalog),
            Arc::clone(&backend),
            "second::paths",
            "second",
        ...
        let reopened = crate::PathIndex::open_persistent(
            Arc::clone(&catalog),
            Arc::clone(&backend),
            "first::paths",
            "first",
...
fn failed_durable_path_index_rebuild_restores_previous_pages_and_definition() {
    ...
    for (catalog, backend) in fixtures() {
        ...
        graph
            ...
            .unwrap();
        crate::PathIndex::build_persistent(
            Arc::clone(&catalog),
            Arc::clone(&backend),
            "items::paths",
            "items",
        ...
        assert!(crate::PathIndex::build_persistent(
            Arc::clone(&catalog),
            Arc::clone(&backend),
            "items::paths",
            "items",
...
use uqa_core::Value;
use uqa_storage::{
    KeyValueCatalog, KeyValueStorageBackend, KeyValueStore, ManagedConnection, MemoryKeyValueStore,
    SQLiteStorageBackend, StorageSavepointId,
};

fn fixtures() -> Vec<(Arc<dyn CatalogFacade>, Arc<dyn PersistentStorageBackend>)> {
    let connection = ManagedConnection::open_in_memory().unwrap();
    let sqlite: Arc<dyn CatalogFacade> =
        Arc::new(uqa_storage::sqlite::Catalog::open(connection.clone()).unwrap());
...
fn failed_delta_and_panic_roll_back_without_a_graph_snapshot() {
    for (catalog, backend) in fixtures() {
        ...
        {
            let mut versioned = crate::VersionedGraphStore::new(&mut graph, "test_graph");
