...
SQL owns view creation namespaces, persisted view decoding, source binding, dispatch restoration and privilege metadata validation, replacement row types and ownership rules, transaction-block diagnostics, prepared declarations, argument validation and result descriptor identity, mutation and declaration analysis, stored relation and routine binding, overload and CALL rules, stored-object dependencies, routine replacement and privileges, privilege inquiries, and graph and scalar-projection argument rules. Retrieval predicates, FTS and operator joins lower into a declarative IR without runtime scorers, fusers, closures or physical index choices. Shared agtype and retrieval configuration values live in core.

The planner owns source and rewrite-rule pruning, statement statistics, prepared-plan costs and custom/generic selection, parameter specialization, retrieval optimizer setup, index and graph statistics, independent relation and operator-join costs, indexed access selection and physical text top-k planning. Execution owns view creation, materialized refresh and durable restoration, including migration publication and rollback, statement read-only validation and error rollback, prepared-argument evaluation and conversion order, mutation loops, schema and routine lifecycle scheduling, catalog publication, routine invocation and caller restoration, table-function streams, scalar dispatch, score provenance and highlight rendering. It also owns retrieval row dispatch, bounded vector-pool interpretation, physical fusion construction, tree execution, cross-relation joins, score composition and model inference.

Engine supplies retained catalog guards, table generations, storage operations and statement/transaction boundaries through typed adapters. Public retrieval APIs retain their existing entry points. Every commit checks the staged dependency graph, including build and platform-specific edges and transitive ownership boundaries. See [crate ownership](https://github.com/cognica-io/uqa-engine/blob/v0.2.3/docs/manual/internals/01-architecture.md), [planning and execution](https://github.com/cognica-io/uqa-engine/blob/v0.2.3/docs/manual/internals/02-planning-and-execution.md), and the [SQL boundary design](https://github.com/cognica-io/uqa-engine/blob/v0.2.3/docs/design/sql-crate-boundaries.md).
