...
pub fn run_table_merge<S: Clone + Send + Sync + 'static>(
    ...
    let mut execute_read = |read_context: &StatementContext<'_, S>| -> Result<SQLResult, SQLError> {
        ...
        let source_schema = crate::query::binding::analyze_source_plan_schema(
            read_context.source.ctes.routines,
            &stmt.source,
            params,
            &analysis_scope,
            None,
        ...
        let null_target_row = dml_null_target_row(assignment.rows, &target_table, &target_qual)?;
        validate_merge_action_scopes(
        ...
        let source_rows =
            build_join_spill_with_ctes(&read_context.source, &stmt.source, params, &mut ctes)?;
        let mut affected = 0_u64;
        let mut returning_rows = Vec::new();

        let pair_schema = merge_pair_schema(source_rows.row_schema());
        let work_mem =
            crate::query::projection::physical_work_mem_bytes(context.source.relational.runtime)?
                .max(1);
        ...
        for pair in pairing_reader {
            ...
            match select_merge_action(
                ...
                SelectedMergeAction::Update {
                    ...
                    let old_metadata =
                        existing_tuple_metadata(assignment.rows, &prepared.table, prepared.doc_id)?;
                ...
                SelectedMergeAction::Insert { mut document } => {
                    ...
                    refresh_stored_generated_columns(assignment, &storage_table, &mut document)?;
