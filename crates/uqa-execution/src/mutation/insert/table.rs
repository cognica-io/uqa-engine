...
pub fn run_table_insert<S: Clone + Send + Sync + 'static>(
    ...
    let execute_read = |read_context: &StatementContext<'_, S>| -> Result<SQLResult, SQLError> {
        ...
        let required_rule_input_positions = (!view_original_query)
            .then(|| required_view_rule_insert_input_positions(mutation.rules.rules.analysis, stmt))
            .transpose()?
            .flatten();
        ...
        if !stmt.returning.is_empty() {
            ...
            return dml_returning_result(preparation.returning, shape, returning_rows, affected);
        }
