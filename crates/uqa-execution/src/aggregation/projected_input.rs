...
use crate::{RowSchema, ScalarEvalContext, ScalarExpr};
...
impl ProjectedAggregatePlans {
    pub(super) fn compile(
        context: &dyn uqa_sql::plan::AggregateClassifier,
        aggregate_targets: &[ScalarExpr],
        input_schema: &RowSchema,
    ) -> Self {
        let plans = aggregate_targets
            .iter()
            .map(|expression| compile_plan(context, expression, input_schema))
            .collect::<Vec<_>>();
...
fn compile_plan(
    ...
    if context.is_registered_aggregate(name)
        || filter.is_some()
        || *distinct
        || !order_by.is_empty()
