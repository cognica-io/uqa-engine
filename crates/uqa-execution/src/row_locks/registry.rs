...
impl RowLockManager {
    ...
    pub fn allocate_transaction_xid(&self) -> Result<u32, SQLError> {
    ...
    pub fn shared_catalog_key(&self, target: super::shared_objects::SharedCatalogLock<'_>) -> u64 {
        ...
        self.relation_key(match target {
            ...
            SharedCatalogLock::MemberName {
                ...
                owner_object_id,
                name,
            } => LockRelationIdentity::SharedMemberName {
                class_id,
                owner_class_id,
                owner_object_id,
                name: Arc::from(name),
    ...
    pub fn has_cross_process_coordination(&self) -> bool {
        matches!(self.cross.as_ref(), Some(CrossAttachment::Active(_)))
    }
}

pub fn shared_provider_manager(
