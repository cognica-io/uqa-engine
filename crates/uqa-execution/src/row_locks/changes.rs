...
pub struct PendingRowChange {
    pub key: RowLockKey,
    pub kind: PendingRowChangeKind,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum PendingRowChangeKind {
    Insert,
    Update,
    Delete,
    Rewrite(RowLockKey),
...
pub struct RowChangeObservation {
...
impl RowLockManager {
    pub fn begin_change_observation(self: &Arc<Self>) -> RowChangeObservation {
    ...
    pub fn current_change_epoch(&self) -> u64 {
        self.state.lock().change_epoch
    }

    #[cfg(test)]
    pub fn current_row_version(&self, table: &str, doc_id: DocId) -> u64 {
        let key = RowLockKey {
    ...
    pub fn conflicting_change_target_after(
        &self,
        table: &str,
        doc_id: DocId,
