...
use super::{
    ...
    BinaryOp, CteScope, QueryBlockPlan, QueryOutput, QueryOutputMode, SQLError, SQLParam,
    ScalarEvalContext, ScalarExpr, SingleRelation, SourceContext, Value,
};

pub fn run_single_foreign_select_output<'a, S: Clone + Send + Sync + 'static>(
    ...
    let source_schema =
        crate::RowSchema::with_qualified_types(qualifier, source_columns, source_types);
    let source: Box<dyn crate::PhysicalOperator + '_> =
        Box::new(crate::RowIteratorScan::with_row_schema(
            source_schema,
    ...
    let source = crate::query::source_projection::bound_source_operator(source, bound_columns)?;
...
pub fn fdw_predicates_from_where(
...
pub fn fdw_predicate(expr: &ScalarExpr, params: &[SQLParam]) -> Option<uqa_fdw::FDWPredicate> {
    match expr {
...
pub fn fdw_column_name(expr: &ScalarExpr) -> Option<String> {
    match expr {
...
pub fn fdw_const_value(expr: &ScalarExpr, params: &[SQLParam]) -> Option<Value> {
    let ctx = ScalarEvalContext::new(None, params);
    eval_scalar(expr, &ctx).ok()
}

pub fn fdw_binary_op(op: BinaryOp) -> Option<uqa_fdw::PredicateOp> {
    Some(match op {
...
pub fn fdw_reversed_binary_op(op: BinaryOp) -> Option<uqa_fdw::PredicateOp> {
    Some(match op {
        BinaryOp::Equal => uqa_fdw::PredicateOp::Eq,
        BinaryOp::NotEqual => uqa_fdw::PredicateOp::NotEq,
