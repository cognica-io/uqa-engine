...
use super::filter::attach_relational_filter;
...
use crate::aggregation::PhysicalAggregateExecutor;
...
pub fn build_relational_operator<'a, S: Clone + 'static>(
    ...
    match statement.compute {
            ...
            if statement.order_by.is_empty() {
                ...
                if !statement.locking.is_empty() {
                    ...
                    operator = attach_order_limit(
                        ...
                        Arc::clone(&evaluator),
                        Some(
                            crate::query::recheck_source::LockRowsRecheckSource::with_projections(
                                statement,
                                ctes,
                                false,
                                recheck_projections,
                    ...
                    operator = super::attach_lock_rows(
                        ...
                        ctes,
                        None,
                        Some(crate::query::recheck_source::LockRowsRecheckSource::new(
        ...
        ComputePlan::Aggregate => {
            ...
            let output_plan =
                prepare_aggregate_output_projection(context.catalog, statement, internal_targets);
            ...
            let aggregate_row_schema =
                crate::RowSchema::with_types(aggregate_schema.clone(), aggregate_types.clone());
