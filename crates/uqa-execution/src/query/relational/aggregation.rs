...
use crate::{FunctionTypeResolver, ProjectionTarget, RowSchema, ScalarExpr};
...
use crate::query::projection::projection_target_expression;
...
pub fn prepare_order_set_projections(
    ...
    for (index, order) in statement.order_by.iter().enumerate() {
        ...
        if let Some((target, _)) = projections.iter().find(|(_, projected)| {
...
pub fn append_distinct_set_projections(
    statement: &QueryBlockPlan,
    output_columns: &[OutputColumnMapping],
    projections: &mut Vec<PhysicalProjection>,
...
pub struct AggregateKeyStatement {
    pub statement: QueryBlockPlan,
    pub targets: Vec<(usize, InternalColumnRef)>,
    pub distinct_on: Vec<(usize, InternalColumnRef)>,
    pub order_by: Vec<(usize, InternalColumnRef)>,
}

pub fn prepare_aggregate_key_statement(
    statement: &QueryBlockPlan,
