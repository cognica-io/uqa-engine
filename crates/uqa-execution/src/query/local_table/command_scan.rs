...
use super::{LocalTableRowSource, SQLError, Value};
...
impl LocalTableRowSource {
    ...
    pub(super) fn next_command_physical_rows_batch(
        ...
        let has_virtual = crate::query::generated::projection_contains_virtual_generated_column(
        ...
        while rows.len() < max_rows {
            ...
            while candidates.len() < remaining {
                ...
                if self.command_base_ids.is_empty() && !self.command_base_exhausted {
                    let page_size = remaining.max(crate::DEFAULT_BATCH_SIZE);
                    let doc_ids = self
                        .table
                        .read_documents()
                ...
                let (doc_id, physical) = match candidate {
                    CommandScanCandidate::Persisted(doc_id) if needs_stored => {
                        ...
                        let values = self
                            ...
                            .collect();
                        (doc_id, crate::PhysicalRow::from_values(values))
                    }
                    CommandScanCandidate::Persisted(doc_id) => {
                        let physical = if let Some(shared_rows) = persisted_shared.as_mut() {
                            match shared_rows.remove(&doc_id).flatten() {
                                ...
                                None => crate::PhysicalRow::nulls(self.columns.len()),
                            }
                        } else {
