...
use super::{
    ...
    recheck_storage_names_match, LockAcquire, LockRows, LockStrength, LockWait, PhysicalOperator,
    PhysicalRow, ResolvedRowLock, RowLockContext, RowSchema, SQLError,
};
...
impl<S: Clone + Send + Sync + 'static> LockRows<'_, S> {
    ...
    pub(super) fn lock_physical_row(
        ...
        for candidate in &candidates {
            if !self.relation_locked.contains(&candidate.storage_name) {
                self.context.session.lock_relation(
        ...
        for candidate in &mut candidates {
            match self.context.session.lock_row(
                candidate.storage_name.as_ref(),
                candidate.doc_id,
                candidate.strength,
                candidate.wait,
                ...
                Err(error) => {
                    rollback_row_acquisitions(self.context, acquired);
                    return Err(error);
                }
            }
        }
        ...
        let requires_recheck = self.context.session.row_lock_change_requires_recheck()?
            || foreign_waited
    ...
    fn recheck_changed_candidates(
        ...
        cache: &crate::row_locks::retry_cache::RowLockRetryCache,
    ) -> Result<Option<PhysicalRow>, SQLError> {
        let mut overrides: Vec<Option<crate::row_locks::retry_cache::RetryRowOverride>> =
            Vec::new();
        ...
        loop {
                ...
                .zip(&mut visited_doc_ids)
            {
                ...
                if self.context.session.uses_fixed_snapshot()
                    && !matches!(recheck_target, CandidateRecheckTarget::Unchanged)
                {
                    return Err(
                        crate::mutation::locking::concurrent_update_serialization_failure(),
                    );
                }
    ...
    fn apply_candidate_recheck(
        ...
        row_override: &mut Option<crate::row_locks::retry_cache::RetryRowOverride>,
        handled: &mut bool,
        ...
        match committed {
            crate::row_locks::retry_cache::RetryRowOverride::Deleted => {
            ...
            crate::row_locks::retry_cache::RetryRowOverride::Present { doc_id, .. } => {
    ...
    fn cross_process_candidate_changed(
        ...
        let committed_document = match &committed {
            ...
            crate::row_locks::retry_cache::RetryRowOverride::Deleted => return Ok(true),
            crate::row_locks::retry_cache::RetryRowOverride::Present { document, .. } => document,
        };
        ...
        let mutation_strength =
            super::context::update_lock_strength(self.context.catalog, table, &changed_columns);
    ...
    fn run_candidate_recheck(
        ...
        overrides: &[Option<crate::row_locks::retry_cache::RetryRowOverride>],
    ) -> Result<Option<PhysicalRow>, SQLError> {
        ...
        let source_leaves = source
            ...
            .transpose()?
            .unwrap_or_default();
        let mut pins = crate::row_locks::recheck::RowLockRecheckPins::new();
        ...
        for origin in row.lock_origins() {
            ...
            let (doc_id, document) =
                changed_target.map_or((origin.doc_id, None), |(candidate, row_override)| {
                    let document = match row_override {
                        Some(crate::row_locks::retry_cache::RetryRowOverride::Present {
                            document,
                            ..
            ...
            pins.pin_target(
                ...
                identity_source,
                vec![crate::row_locks::recheck::RecheckDoc { doc_id, document }],
            );
        }
        let mut recheck_ctes = source.ctes.clone();
        recheck_ctes.activate_row_lock_recheck(std::sync::Arc::new(pins));
        let mut operator = self.context.recheck.build_recheck(
            &source.statement,
            self.params,
            &mut recheck_ctes,
            source.ordered,
        ...
        let first_row = loop {
            match operator.next() {
                Ok(Some(batch)) => {
                    ...
                    if let Some(row) = batch.rows.into_iter().next() {
                        break Some(match schema.relayout_physical_row(row, &self.schema) {
                            ...
                            Err(error) => {
                                return Err(
                                    crate::query::projection::close_after_physical_failure(
                                        operator.as_mut(),
                                        error,
                ...
                Err(error) => {
                    ...
                    return Err(crate::physical::physical_exec_error(error));
                }
            }
        };
        operator
...
fn rollback_row_acquisitions<S: Clone + Send + Sync + 'static>(
    ...
    for acquisition in acquisitions.into_iter().rev() {
        context.session.rollback_row_lock_acquisition(acquisition);
    }
}
...
fn align_recheck_schema<'a>(
    ...
    Ok(Box::new(crate::ColumnSelection::with_positions(
        operator, positions,
    )))
}

fn lock_origin_matches_target(origin: &crate::RowLockOrigin, target: &ResolvedRowLock) -> bool {
