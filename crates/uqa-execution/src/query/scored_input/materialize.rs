...
impl ScoredDocumentSource {
        ...
        let visited = self
            ...
            .for_each_next_fields(after, max_rows, &fields, &mut |doc_id, values| {
                ...
                let result = (|| -> ExecResult<()> {
                    ...
                    let row = crate::ProjectedRow::new(
                        &self.schema,
    ...
    pub(super) fn next_shared_physical_batch(
        &mut self,
        max_rows: usize,
    ) -> ExecResult<Option<Vec<crate::PhysicalRow>>> {
        if self.lock_origin.is_some()
            || self.recheck_pinned
            || crate::query::generated::projection_contains_virtual_generated_column(
                &self.column_definitions,
    ...
    fn physical_row_from_shared(
    ...
    ) -> ExecResult<crate::PhysicalRow> {
        let (values, projection) = shared.into_parts();
        let source = crate::PhysicalRow::from_shared_values(values, projection);
        if self.appended_doc_id_attribute.is_none()
            ...
            .enumerate()
            .all(|(expected, slot)| {
                matches!(slot, crate::ProjectedValueSlot::Field(actual) if *actual == expected)
            })
        {
            return Ok(source);
        }
        ...
        let row = source.project_with_values(self.projected_slots.iter().map(|slot| {
            match slot {
                crate::ProjectedValueSlot::Field(index) => {
                    crate::RowProjectionValue::InputSlot(*index)
                }
                crate::ProjectedValueSlot::Extra(index) => crate::RowProjectionValue::Owned(
                    extras
                        .get(*index)
                ...
                crate::ProjectedValueSlot::Missing => crate::RowProjectionValue::Owned(Value::Null),
            }
        }));
    ...
    fn materialize_plain_physical_entries(
        ...
        self.for_each_kept_entry(entries, &mut |doc_id, entry, _fields, values| {
            ...
            let row = crate::PhysicalRow::from_values(row.into_values());
        ...
        self.for_each_kept_entry(entries, &mut |doc_id, entry, _fields, values| {
            ...
            let row = self
                .append_metadata_attributes(
                    crate::PhysicalRow::from_values(row.into_values()),
                    doc_id,
                    entry.score,
                )?
                .with_lock_origin(crate::RowLockOrigin::from_shared(
