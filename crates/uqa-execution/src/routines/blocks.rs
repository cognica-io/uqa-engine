...
impl Interpreter<'_> {
    ...
    pub(super) fn exec_exception_block(&mut self, block: &PLpgSQLBlock) -> Result<Flow, SQLError> {
        if self.services.transactions.depth() == 0 {
            return Err(SQLError::Internal(
                "PL/pgSQL exception block executed outside a statement transaction".into(),
            ));
        }
        self.services.transactions.begin()?;
        match self.exec_stmts(&block.body) {
            Ok(flow) => {
                self.services.transactions.commit()?;
                Ok(flow)
            }
            Err(error) => {
                if let Err(rollback_error) = self.services.transactions.rollback() {
