...
impl Interpreter<'_> {
    ...
    pub(super) fn exec_fetch_cursor(
        ...
        let result = self.services.portals.fetch(&FetchCursorStmt {
            name: portal_name,
            direction,
            count,
            move_only: false,
    ...
    pub(super) fn exec_close_cursor(&mut self, cursor_index: usize) -> Result<(), SQLError> {
        ...
        self.services.portals.close(&portal_name)
    }
    ...
    fn exec_pinned_for_portal(
        ...
        if let Err(error) = self.services.portals.pin(portal_name) {
            let _ = self.services.portals.close(portal_name);
            return Err(error);
        }
        let loop_result = self.exec_for_portal_rows(portal_name, label, target, body, prefetch);
        let unpin_result = self.services.portals.unpin(portal_name);
        let close_result = match &unpin_result {
            Ok(()) => self.services.portals.close(portal_name),
            Err(_) => Ok(()),
        };
        match loop_result {
            Err(error) => Err(error),
    ...
    fn portal_name_for_open(
        ...
        match self.values.get(cursor_index) {
            ...
            Some(Value::Null) => Ok(self.services.portals.allocate_name()),
    ...
    fn lower_cursor_plan(&self, statement: Statement) -> Result<UnifiedPlan, SQLError> {
        ...
        self.services.statements.optimize_plan(plan)
    }
    ...
    fn eval_cursor_count(&self, count: &PLpgSQLCursorCount) -> Result<i64, SQLError> {
        ...
        match coerce_routine_value(self.services.expressions, &value, "int4")? {
