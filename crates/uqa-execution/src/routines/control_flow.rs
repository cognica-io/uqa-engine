    ...
    fn foreach_target_is_array(&self, target: usize) -> Result<bool, SQLError> {
        ...
        Ok(match datum {
            PLpgSQLDatum::Var(variable) => {
                foreach_declaration_is_array(self.services, &variable.type_name)
            }
    ...
    pub(super) fn exec_return(
        ...
        if let Some(value) = value {
            ...
            self.ret = match &self.def.returns {
                FunctionReturns::Scalar { type_name } => super::coerce_routine_value_from(
    ...
    pub(super) fn exec_return_next(
        ...
        let value = match &self.def.returns {
            FunctionReturns::SetOf { type_name } => {
                coerce_routine_value(self.services.expressions, &value, type_name)?
            }
            _ => value,
        };
...
fn foreach_declaration_is_array(services: RoutineContext<'_>, type_name: &str) -> bool {
    let canonical = canonical_routine_type_name(type_name);
    if matches!(canonical.as_str(), "anyarray" | "anycompatiblearray") {
        return true;
    }
    if let Some(ty) = services
        .expressions
        .catalog_column_type(&canonical)
