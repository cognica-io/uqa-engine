...
use super::{CatalogContext, ResultRow, SQLError};
    ...
    for name in catalog.table_names() {
        ...
        let tuples = if hierarchy.partition_spec.is_some() {
            ...
            for member in catalog.hierarchy_scan_tables(resolution, &name, true)? {
                total = total
                    .checked_add(context.table_doc_count(&member)?)
                    .ok_or_else(|| {
        ...
        if let Some(bound) = hierarchy.partition_bound.as_ref() {
            row.insert(
                ...
                str_value(partition_bound_node(
                    context, catalog, resolution, &name, bound,
                )?),
            );
        }
        out.push(row);
...
pub fn table_acl_catalog_value(
    owner: &str,
