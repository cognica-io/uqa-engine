...
//! per label. The context stores graphs natively, so these rows are derived
...
pub use crate::catalog::graph::{
    graph_catalog_entries, resolve_age_label_relation, resolve_age_label_relation_name,
};

pub fn build_age_label_relation_rows(
...
pub fn label_relation_regclass(graph: &str, label: &str) -> String {
    format!("{}.{}", quote_ident(graph), quote_ident(label))
}

/// `ag_label.seq_name` of a label.
pub fn label_sequence_name(label: &str) -> String {
...
pub fn build_ag_label(catalog: &CatalogReadView) -> Result<Vec<ResultRow>, SQLError> {
    let mut out = Vec::new();
...
pub fn age_pg_attribute_rows(catalog: &CatalogReadView) -> Result<Vec<ResultRow>, SQLError> {
    let mut out = Vec::new();
    for entry in graph_catalog_entries(catalog)? {
        for label in &entry.labels {
...
pub fn age_pg_sequences_rows(catalog: &CatalogReadView) -> Result<Vec<ResultRow>, SQLError> {
    let mut out = Vec::new();
    for entry in graph_catalog_entries(catalog)? {
...
pub fn age_info_table_rows(catalog: &CatalogReadView) -> Result<Vec<ResultRow>, SQLError> {
    let mut out = Vec::new();
    for entry in graph_catalog_entries(catalog)? {
        for label in &entry.labels {
            out.push(row([
...
fn age_pg_type_row(ty: AgeTypeRow) -> ResultRow {
    ...
    row([
        ...
        ("typsubscript", int_value(ty.subscript)),
