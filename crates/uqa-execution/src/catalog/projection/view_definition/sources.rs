...
use uqa_sql::plan::{QueryPlan, SourcePlan, TableFunctionPlan};

use super::{query_columns, quote_ident, Column, Deparser, RelationIdentity, SQLError, Scope};

impl Deparser<'_> {
    pub fn relation_name(&self, name: &str, scope: &Scope) -> Result<String, SQLError> {
        let (schema, local) =
    ...
    pub fn table_columns(&self, name: &str, scope: &Scope) -> Result<Vec<String>, SQLError> {
    ...
    for name in using {
        ...
        if kind == JoinKind::Full {
            ...
            column.merged_expression = Some(uqa_sql::ir::ScalarExpr::Func {
                ...
                binding: None,
                args: vec![
                    uqa_sql::ir::ScalarExpr::qualified_column(&left.qualifier, name),
                    uqa_sql::ir::ScalarExpr::qualified_column(&right.qualifier, name),
                ],
                distinct: false,
                order_by: Vec::new(),
                filter: None,
...
pub fn identifier_list(names: &[String]) -> String {
    names
        .iter()
