...
pub fn rename_view_column_query(
    context: &crate::catalog::context::CatalogContext<'_>,
    query: &mut QueryPlan,
    table: &str,
    from: &str,
    to: &str,
) -> Result<(), SQLError> {
    change_bound_view_column(context, query, table, from, to).map(|_| ())
}

pub fn view_query_references_column(
    context: &crate::catalog::context::CatalogContext<'_>,
    query: &QueryPlan,
    table: &str,
    column: &str,
) -> Result<bool, SQLError> {
    change_bound_view_column(context, &mut query.clone(), table, column, column)
}
...
fn rename_expression(expression: &mut ScalarExpr, scope: &Scope, rename: &Rename<'_>) {
    uqa_sql::plan::rewrite_scalar_expression(expression, &mut |expression| {
