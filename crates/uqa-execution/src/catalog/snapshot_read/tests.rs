...
#[test]
fn catalog_snapshot_read_releases_its_transaction_after_success_error_and_unwind() {
    let session = session();
...
#[test]
fn catalog_snapshot_read_never_adopts_or_finishes_a_callers_transaction() {
    let session = session();
