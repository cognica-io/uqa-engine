...
use uqa_sql::catalog::roles::{role_inherits, RoleDefinition, RoleMembership, RoleMembershipKey};
...
impl AclPrivilege {
    pub const fn mask(self) -> SequencePrivileges {
        match self {
...
pub fn parse_privilege_checks(value: &str) -> Result<Vec<PrivilegeCheck>, SQLError> {
    value
        .split(',')
        .map(|item| {
            let item = item.trim();
...
pub fn rewrite_acl_owner(security: &mut SequenceSecurity, new_owner: &str) {
