...
//! Offline Kuromoji packing and complete decoded-model verification.

use std::io::Write;
use std::path::{Path, PathBuf};

use uqa_analysis::kuromoji::{pack, DictionaryLimits, KuromojiDictionary};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match arguments.as_slice() {
        ...
        _ => {
            return Err(
                "usage: pack_kuromoji pack <neutral-directory> <bundle> | verify <bundle>".into(),
            )
        }
    }
    Ok(())
