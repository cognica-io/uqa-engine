...
pub struct KuromojiDictionary {
    ...
    pub(super) analysis: AnalysisData,
}

impl std::fmt::Debug for KuromojiDictionary {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("KuromojiDictionary")
            .field("id", &self.id)
...
impl KuromojiDictionary {
    ...
    pub fn from_bytes(bytes: &[u8], limits: DictionaryLimits) -> DictionaryResult<Arc<Self>> {
        ...
        let morphology = take_section(&mut sections, 3, |reader| {
            let morphology = Morphology::decode(reader, limits)?;
            let count = morphology.strings.len();
            Ok((morphology, count))
        })?;
        ...
        let analysis = take_section(&mut sections, 7, |reader| {
            let mut remaining = limits;
            remaining.max_strings = limits
                .max_strings
                .checked_sub(morphology.strings.len())
                .ok_or_else(|| reader.invalid("morphology strings exceed limit"))?;
            let analysis = AnalysisData::decode(reader, remaining)?;
            let count = analysis.completion.len();
            Ok((analysis, count))
        })?;
        let dictionary = Self {
            id,
    ...
    pub fn is_kanji(&self, unit: u16) -> bool {
        self.characters.values[unit as usize][1] != 0
    }
    ...
    pub fn default_stop_words(&self) -> &[String] {
        &self.analysis.stop_words
    }
    pub fn default_stop_tags(&self) -> &[String] {
        &self.analysis.stop_tags
    }
    pub fn completion_mappings(&self) -> &[super::CompletionMapping] {
        &self.analysis.completion
    }
