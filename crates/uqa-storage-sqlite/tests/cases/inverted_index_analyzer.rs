...
use uqa_storage::{KeyValueInvertedIndex, MemoryKeyValueStore, StorageBackendError};
use uqa_storage_sqlite::SQLiteError;
use uqa_storage_sqlite::SQLiteInvertedIndex;
...
fn sqlite_indexing_propagates_analysis_error_and_rolls_back_replacement() {
    ...
    assert!(matches!(
        error,
        StorageBackendError::Backend { source, .. } if matches!(source.downcast_ref::<SQLiteError>(), Some(SQLiteError::Analysis(_)))
    ));
    assert_eq!(index.get_posting_list("title", "old").unwrap().len(), 1);
    assert!(index.get_posting_list("title", "new").unwrap().is_empty());
    assert_eq!(index.doc_count().unwrap(), 1);
