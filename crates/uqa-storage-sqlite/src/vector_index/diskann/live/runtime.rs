...
use super::{
    invalid, DiskANNIndexParams, DiskANNReadLimits, DocId, RetainedDiskANNIndex,
    RetainedSQLiteDiskANNCanonical, SQLiteDiskANNHandle, StorageBackendResult, StorageReadControl,
};
use uqa_storage::diskann_index::{
    build::DiskANNTemporaryBudget, DiskANNIndexOptions, DiskANNPersistentOwner, DiskANNQueryRead,
};

impl DiskANNPersistentOwner for SQLiteDiskANNHandle {
    type Snapshot = RetainedDiskANNIndex<RetainedSQLiteDiskANNCanonical>;
...
impl SQLiteDiskANNHandle {
    fn metadata(&self) -> StorageBackendResult<RetainedSQLiteDiskANNCanonical> {
