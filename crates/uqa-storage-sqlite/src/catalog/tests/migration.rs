...
fn v22_migration_preserves_legacy_postings_in_clustered_rows() {
    ...
    mc.with(|conn| {
        ...
        let postings = uqa_storage::clustered_postings::decode_cluster(
        ...
        assert_eq!(
            uqa_storage::clustered_postings::decode_terms(&terms_blob).unwrap(),
...
fn v22_migration_is_idempotent_when_clustered_tables_precede_the_version_marker() {
    let mc = ManagedConnection::open_in_memory().unwrap();
    let _current = Catalog::open(mc.clone()).unwrap();
    let postings = vec![uqa_storage::clustered_postings::ClusterPosting {
        doc_id: 7,
        term_freq: 1,
        doc_length: 2,
        positions: vec![1],
    ...
    let (score_blob, positions_blob) =
        uqa_storage::clustered_postings::encode_cluster(&postings).unwrap();
    let terms_blob = uqa_storage::clustered_postings::encode_terms(&["rust".to_string()]).unwrap();
    mc.with(|conn| {
        conn.execute(
    ...
    mc.with(|conn| {
        ...
        assert_eq!(
            uqa_storage::clustered_postings::decode_all_scores(0, &stored.1)
                .unwrap()
                .into_iter()
