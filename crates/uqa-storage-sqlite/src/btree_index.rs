    ...
    pub fn fields(&self, table: &str) -> Result<Vec<uqa_storage::ValueIndexKey>> {
        self.conn.with(|conn| {
            ...
            let rows = stmt.query_map([table], |row| {
                row.get::<_, SQLiteValueIndexKey<uqa_storage::ValueIndexKey>>(0)
                    .map(|key| key.0)
            })?;
    ...
    pub fn repairs(&self) -> Result<Vec<(String, uqa_storage::ValueIndexKey)>> {
        self.conn.with(|conn| {
            ...
            let rows = stmt.query_map([], |row| {
                Ok((
                    row.get::<_, String>(0)?,
                    row.get::<_, SQLiteValueIndexKey<uqa_storage::ValueIndexKey>>(1)?
                        .0,
                ))
            })?;
    ...
    pub fn clear_repair(&self, table: &str, field: &uqa_storage::ValueIndexKey) -> Result<()> {
        self.conn.with(|conn| {
            conn.execute(
                ...
                params![table, SQLiteValueIndexKey(field)],
            )?;
            Ok(())
        })
    }
    ...
    pub fn load(
        ...
        self.conn.with(|conn| {
            let exists = conn
                ...
                .query_row(params![table, SQLiteValueIndexKey(field)], |row| {
                    row.get::<_, i64>(0)
                })
                .optional()?
                .is_some();
            ...
            let mut rows = stmt.query(params![table, SQLiteValueIndexKey(field)])?;
        ...
        self.conn.with_mut(|conn| {
            ...
            )?;
            {
                ...
                for doc_id in &stale_doc_ids {
                    delete.execute(params![table, SQLiteValueIndexKey(field), doc_id])?;
                }
            }
            {
        ...
        self.conn.with_mut(|conn| {
            let tx = conn.savepoint()?;
            {
                ...
                for (field, values) in &encoded {
                    delete.execute(params![table, SQLiteValueIndexKey(*field)])?;
                    mark.execute(params![table, SQLiteValueIndexKey(*field)])?;
...
mod tests {
    ...
    fn replace_load_write_delete_and_clear_round_trip() {
        ...
        assert_eq!(
            ...
            vec![uqa_storage::ValueIndexKey::from("public_id")]
        );
