...
fn sqlite_error(error: StorageBackendError) -> SQLiteError {
    let StorageBackendError::Backend { backend, source } = error else {
        panic!("expected a typed SQLite provider error, got {error:?}");
    };
    assert_eq!(backend, "SQLite");
    *source.downcast::<SQLiteError>().unwrap()
}
...
fn document_id_larger_than_sqlite_integer_is_rejected() {
    ...
    assert!(matches!(
        sqlite_error(error),
        SQLiteError::StorageBackend(ref message)
            if message.contains("exceeds SQLite INTEGER")
    ));
    assert_eq!(s.len().unwrap(), 0, "failed write must not insert a row");

    assert!(matches!(
        sqlite_error(s.get(DocId::MAX).unwrap_err()),
        SQLiteError::StorageBackend(ref message)
            if message.contains("exceeds SQLite INTEGER")
    ));
    assert!(matches!(
        sqlite_error(s.get_many(&[DocId::MAX]).unwrap_err()),
        SQLiteError::StorageBackend(ref message)
            if message.contains("exceeds SQLite INTEGER")
    ));
}
...
fn negative_persisted_document_id_is_reported_as_corruption() {
    ...
    assert!(matches!(
        sqlite_error(s.doc_ids().unwrap_err()),
        SQLiteError::StorageBackend(ref message)
            if message.contains("negative SQLite document id -1")
    ));
    assert!(matches!(
        sqlite_error(s.next_doc_id(None).unwrap_err()),
        SQLiteError::StorageBackend(ref message)
            if message.contains("negative SQLite document id -1")
    ));
    assert!(matches!(
        sqlite_error(s.max_doc_id().unwrap_err()),
        SQLiteError::StorageBackend(ref message)
            if message.contains("negative SQLite document id -1")
    ));
}
...
fn missing_or_malformed_blob_rows_are_reported_as_corruption() {
    ...
    s.conn
        ...
        .unwrap();
    assert!(matches!(
        sqlite_error(s.get(31).unwrap_err()),
    ...
    s.conn
        ...
        .unwrap();
    assert!(matches!(
        sqlite_error(s.get_field(32, "embedding").unwrap_err()),
        SQLiteError::CorruptDocumentBlob { .. }
    ));
}
