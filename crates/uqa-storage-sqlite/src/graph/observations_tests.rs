...
fn standalone_graph_definition_names_preserve_scope_and_ignore_registry_only_updates() {
    for route in ["create", "drop", "list", "registry", "other scope"] {
        ...
        let path = directory.path().join("graph-definitions.db");
        ...
        match route {
            "create" => assert!(!a.has_graph("new").unwrap()),
            "list" => assert_eq!(a.graph_names().unwrap(), ["g"]),
            _ => assert!(a.has_graph("g").unwrap()),
        }
        ...
        match route {
            "create" | "list" => b.create_graph("new").unwrap(),
            "registry" => {
                b.allocate_vertex_id("new_label", "g").unwrap();
            }
            _ => b.drop_graph("g").unwrap(),
        }
        backend_a.commit_transaction().unwrap();
        if matches!(route, "registry" | "other scope") {
