...
pub(super) fn downgrade_record_format(store: &SQLiteRecordStore, format: i64) {
    store.with(|connection| {
        ...
        assert!(definition.contains("CHECK(format = 46)"));
        ...
        connection.execute_batch(&definition.replace("CHECK(format = 46)", &format!("CHECK(format = {format})")))?;
...
fn record_format_upgrade_preserves_history_identity_allocations_and_receipts() {
    let control = control();
    for (mode, format) in (0..3).flat_map(|mode| (1..=45).map(move |format| (mode, format))) {
        let connection = ManagedConnection::open_in_memory().unwrap();
        if mode == 2 {
...
fn closed_record_format_files_upgrade_in_every_sqlite_mode() {
    let control = control();
    for (mode, format) in (0..4).flat_map(|mode| (1..=45).map(move |format| (mode, format))) {
        let directory = tempfile::tempdir().unwrap();
