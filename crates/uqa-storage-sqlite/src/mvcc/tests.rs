...
pub(super) fn downgrade_record_format(store: &SQLiteRecordStore, format: i64) {
    store.with(|connection| {
        ...
        assert!(definition.contains("CHECK(format = 53)"));
        ...
        connection.execute_batch(&definition.replace("CHECK(format = 53)", &format!("CHECK(format = {format})")))?;
...
fn record_format_upgrade_preserves_history_identity_allocations_and_receipts() {
    let control = control();
    for (mode, format) in (0..3).flat_map(|mode| (1..=52).map(move |format| (mode, format))) {
        let connection = ManagedConnection::open_in_memory().unwrap();
        if mode == 2 {
...
fn latest_record_format_files_upgrade_in_every_sqlite_mode(
    #[values(0, 1, 2, 3)] mode: usize,
    #[values(46, 47, 48, 49, 50, 51, 52)] format: i64,
) {
    closed_record_formats_upgrade(mode, format);
}
