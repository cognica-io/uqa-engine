...
pub(super) fn initialize_in(transaction: &Connection) -> PhysicalResult<Initialization> {
    ...
    if present == 0 {
        ...
        transaction.execute(
            "INSERT INTO _uqa_mvcc_metadata (singleton, format, database_id, allocated, sequence) VALUES (1, 45, ?1, ?2, ?2)",
            ...
            if !matches {
                if name == TABLES[0].0 {
                    if definition_matches(
                        transaction,
                        name,
                        &expected.replace("CHECK(format = 45)", "CHECK(format = 44)"),
                    )? == Some(true)
                    {
                        predecessor = Some(44);
                    }
...
pub(super) fn initialize_restoration(connection: &Connection) -> PhysicalResult<()> {
    ...
    if present != TABLES.len() || predecessor.is_some_and(|format| !matches!(format, 43 | 44)) {
...
fn upgrade_metadata(transaction: &Connection, format: i64) -> PhysicalResult<()> {
    ...
    transaction.execute_batch(&format!("INSERT INTO _uqa_mvcc_metadata SELECT singleton, 45, database_id, allocated, sequence, mapping, {pending}, {receipt_limit} FROM _uqa_mvcc_previous_metadata; DROP TABLE _uqa_mvcc_previous_metadata;"))?;
    if format >= 44 {
        for action in ["INSERT", "UPDATE", "DELETE"] {
