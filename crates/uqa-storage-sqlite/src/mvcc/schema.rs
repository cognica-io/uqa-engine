...
pub(super) fn initialize_in(transaction: &Connection) -> PhysicalResult<Initialization> {
    ...
    if present == 0 {
        ...
        transaction.execute(
            "INSERT INTO _uqa_mvcc_metadata (singleton, format, database_id, allocated, sequence) VALUES (1, 43, ?1, ?2, ?2)",
...
/// A resumed restore must validate the current physical schema without admitting normal record access through its pending marker.
pub(super) fn validate_restoration(connection: &Connection) -> PhysicalResult<()> {
    let (present, predecessor) = validate_tables(connection)?;
    if present != TABLES.len() || predecessor.is_some() {
        return Err(VersionError::InvalidEncoding("incomplete restoration record schema").into());
    }
    validate_guards(connection, None)
}
...
fn upgrade_metadata(transaction: &Connection, format: i64) -> PhysicalResult<()> {
    ...
    transaction.execute_batch("INSERT INTO _uqa_mvcc_metadata SELECT singleton, 43, database_id, allocated, sequence, mapping, NULL FROM _uqa_mvcc_previous_metadata; DROP TABLE _uqa_mvcc_previous_metadata;")?;
    for action in ["INSERT", "UPDATE", "DELETE"] {
