...
pub(super) fn initialize_in(transaction: &Connection) -> PhysicalResult<Initialization> {
    ...
    for (name, expected) in TABLES {
        if let Some(matches) = definition_matches(transaction, name, expected)? {
            if !matches {
                if name == TABLES[0].0 {
                    for format in [1, 2, 3, 4, 5, 6, 7, 8] {
                        if definition_matches(
                            transaction,
                            name,
                            &expected
                                .replace("CHECK(format = 9)", &format!("CHECK(format = {format})")),
                        )? == Some(true)
                        {
    ...
    if present == 0 {
        ...
        transaction.execute(
            "INSERT INTO _uqa_mvcc_metadata (singleton, format, database_id, allocated, sequence) VALUES (1, 9, ?1, ?2, ?2)",
...
fn upgrade_metadata(transaction: &Connection, format: i64) -> PhysicalResult<()> {
    ...
    transaction.execute_batch("INSERT INTO _uqa_mvcc_metadata SELECT singleton, 9, database_id, allocated, sequence, mapping FROM _uqa_mvcc_previous_metadata; DROP TABLE _uqa_mvcc_previous_metadata;")?;
    for action in ["INSERT", "UPDATE", "DELETE"] {
