...
These changes are not included in the 0.3.8 packages shown below. Opening a supported older database with `main` upgrades its persistent formats; earlier binaries cannot reopen the upgraded database. Keep a pre-upgrade backup made with all database owners closed, and read the [MVCC upgrade requirements](docs/manual/reference/10-upgrading.md#unreleased-mvcc-writer-compatibility), [backup restoration contract](docs/manual/reference/04-storage-and-security.md#backups-and-copies), and [unreleased history](HISTORY.md#unreleased) before upgrading.
...
`uqa-storage` owns provider-independent contracts and shared data structures, while `uqa-storage-sqlite` owns SQLite connections, catalogs, indexes, transactions, graph persistence, and compressed storage. Rust callers using concrete storage types must use the [updated provider imports](docs/manual/reference/10-upgrading.md#sqlite-provider-ownership).
...
| [Concurrent storage transactions](docs/design/concurrent-storage-transactions.md) | Overlapping logical writes, snapshots, conflicts, atomic publication, and provider ownership |
...
| [Upgrade guide](docs/manual/reference/10-upgrading.md) | Package updates, Rust API changes, persistent format upgrades, and backup restoration |
