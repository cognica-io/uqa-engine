...
Every compiled SQL statement now follows `Statement -> UnifiedPlan -> UnifiedPlanExecutor -> EngineUnifiedPlanDriver`. `UnifiedPlan` has exhaustive query and command variants rather than a raw-statement or legacy escape hatch. Its relational query tree owns CTEs, set-operation branches, joins, VALUES/function/subquery sources, filters, arithmetic projections, aggregates, windows, ordering, distinctness, and limits; INSERT/UPDATE/DELETE/MERGE and DDL/session operations are explicit command nodes. Prepared statements, stored views, and the SQL text cache retain compiled plans. SQL generated at runtime by PL/pgSQL, correlation rewrites, or physical CTE/view specialisation is lowered before physical execution. There is no separate top-level row/direct-dispatch executor.

`OperatorTree` remains the specialised child algebra for posting-list, graph, scoring, and fusion access paths; relational and command semantics are represented by the enclosing `UnifiedPlan` instead of being distorted into document-id operators. Every concrete `OperatorTree` variant follows `OperatorTree -> QueryOptimizer (10 passes) -> PlanExecutor -> EngineDriver`. Ordinary, graph, aggregation, fusion, and deep-fusion nodes produce a `PostingList` (graph payloads use the Phi encoding), while join nodes preserve tuples as a `GeneralizedPostingList`. The driver match is exhaustive, an unknown `Opaque` kind is an error, and physical/filter evaluation errors propagate rather than becoming empty results.
