...
- **SQL** (`uqa-sql`, `uqa-engine`): A `libpg_query` backed PostgreSQL-oriented compiler and engine for CREATE/ALTER/DROP, INSERT/UPDATE/DELETE, `RETURNING`, `ON CONFLICT`, CTAS, MERGE, JOINs including LATERAL, recursive CTEs, windows, grouping sets, sequences, views, schemas, `search_path`, `SHOW`, PostgreSQL 17 runtime defaults, `DISCARD ALL/PLANS/SEQUENCES`, `TRUNCATE`, JSON/JSONB, arrays, temporal types, numeric types, `BYTEA`, constraints, referential actions, analyzer DDL, foreign server/table DDL, Rust-backed scalar/table/aggregate function registration, PL/pgSQL and SQL-language `CREATE FUNCTION` / `CREATE PROCEDURE` / `DO` / `CALL`, `information_schema`, and `pg_catalog` virtual views. `DISCARD TEMP` is an explicit unsupported error until temporary tables exist. Scalar predicates on PRIMARY KEY / UNIQUE / btree-indexed columns resolve through lazily built, incrementally maintained value indexes whose posting supports compose as `DocSet` operations.
...
`OperatorTree` remains the specialised child algebra for posting-list, graph, scoring, and fusion access paths; relational and command semantics are represented by the enclosing `UnifiedPlan` instead of being distorted into document-id operators. Every concrete `OperatorTree` variant follows `OperatorTree -> QueryOptimizer (10 passes) -> PlanExecutor -> EngineDriver`. The algebra pass compares membership-only subtrees structurally instead of using Rust object addresses; scored or decorated operands remain distinct because posting-list payload merges add scores and can use right-hand field precedence. Ordinary, aggregation, fusion, and deep-fusion nodes produce a `PostingList`; graph nodes retain `GraphPostingList` through homogeneous graph set algebra, and heterogeneous SQL document predicates insert an explicit Phi codec boundary. Join nodes preserve tuples as a `GeneralizedPostingList`. The driver match is exhaustive, an unknown `Opaque` kind is an error, and physical/filter evaluation errors propagate rather than becoming empty results.

For a score-ordered limit on one text field, the planner attaches an exact WAND or Block-Max WAND physical plan to the term leaf. BMW is selected only for scorer-versioned SQLite block bounds that match the live BM25 parameters and field statistics; writes invalidate those bounds in the posting transaction and execution safely falls back to WAND if validity changes. `Engine::search_profiled` exposes algorithm, candidate/scored counts, skip rate, cursor advances, and latency for reproducible engine-level benchmarks.
