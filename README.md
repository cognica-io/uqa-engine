...
SQL owns mutation analysis, view rewriting, table declarations, stored schema and routine binding, and routine replacement and privilege rules; the planner owns source-output pruning. Execution owns INSERT, UPDATE, DELETE, and MERGE loops, CREATE TABLE, CREATE INDEX, CREATE SEQUENCE, ADD COLUMN, and partition lifecycle scheduling, and schema publication through retained table state handles. It also owns routine registration, ALTER, owner and privilege publication, configuration restoration, and dependency recompilation. Retrieval argument rules live in SQL; physical tree dispatch, cross-relation operator joins, score composition and model inference live in execution. Every commit checks the staged crate dependency graph, including build and platform-specific edges and transitive ownership boundaries. See [crate ownership](docs/manual/internals/01-architecture.md) and the [SQL boundary design](docs/design/sql-crate-boundaries.md).
