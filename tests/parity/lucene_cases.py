...
def provenance(root, stem, entrypoint, cases, output):
    return {
        "format_version": 1,
...
def main(root, prepare_jars, docker_command, stem, entrypoint, fields, description, *, cache_name="uqa-lucene-reference-jars"):
    root = root.resolve()
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument("--cache-dir", type=Path, default=Path(tempfile.gettempdir()) / cache_name)
    ...
    if not args.write:
        ...
        if provenance(root, stem, entrypoint, cases, expected) != json.loads(provenance_path.read_text(encoding="utf-8")):
            raise RuntimeError("Reference input, output, or provenance changed")
    cache = args.cache_dir.resolve()
    manifest = prepare_jars(cache, args.offline)
    with tempfile.TemporaryDirectory(prefix=f"uqa-{root.name}-{stem}-") as temporary:
    ...
    if args.write:
        ...
        provenance_path.write_text(json.dumps(provenance(root, stem, entrypoint, cases, output), indent=2) + "\n", encoding="utf-8")
