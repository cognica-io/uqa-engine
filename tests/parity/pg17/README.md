...
`run_diff.py` executes every probe in `probes.sql` against a real PostgreSQL 17 instance and against the `usql` release binary, then reports divergences in three categories:
...
- A PostgreSQL 17 container named `uqa-pg17-age` with user `postgres`, database `uqa_compat`:
