...
The recorded execution used `linux/arm64` with Temurin 21.0.10+7. The pinned image index also contains `linux/amd64`; select it with `--platform linux/amd64` to verify that platform separately. Regenerate an intentionally changed reference with `--write`, inspect the full output diff, and update the manifest and design evidence together. Do not regenerate expected results merely to make a future Rust mismatch disappear.
...
The larger differential corpus, dictionary exporter, Rust implementation, storage migration, binding execution, and performance measurements described in the design remain implementation work. This directory contains only the executable evidence used while preparing the design.
