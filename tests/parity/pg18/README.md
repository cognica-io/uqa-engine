...
The `--suite triggers` mode executes the 118 cases in `triggers_stateful.sql` and compares them with `triggers_stateful.expected.json`. It covers trigger creation and executable replacement, row and statement execution, `WHEN` validation and timing, generated-row images, zero-row updates, `TRUNCATE`, constraint-trigger immediate and deferred execution, `SET CONSTRAINTS`, captured row images, savepoint and commit lifecycle, trigger-owned constraints, partition clones, catalogs, enable and independent rename lifecycle, dependency drops, queued-event cancellation, transition-relation definition validation, typed setwise execution for multi-row and zero-row mutations, `INSERT SELECT`, `ON CONFLICT`, `UPDATE FROM`, `MERGE`, partition and inheritance descendants, single and multiple foreign-key cascades with cumulative row images and coalesced statement boundaries, persistence guards, and exact SQLSTATEs.
