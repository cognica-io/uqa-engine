...
-- gamma functions
SELECT gamma(5), gamma(.5::float8), gamma(-.5::float8), lgamma(-.5::float8)
SELECT pg_typeof(gamma(5::smallint)), pg_typeof(gamma(5::integer)), pg_typeof(gamma(5::bigint)), pg_typeof(gamma(5::numeric)), pg_typeof(gamma(5::real)), pg_typeof(gamma(5::double precision)), pg_typeof(gamma('5'))
SELECT gamma(NULL), lgamma(NULL), pg_typeof(gamma(NULL)), pg_typeof(lgamma(NULL))
SELECT gamma((SELECT .5::float8)), pg_typeof(gamma((SELECT .5::float8)))
SELECT pg_catalog.gamma(5), pg_catalog.lgamma(-.5::float8)
SELECT gamma('Infinity'::float8), gamma('NaN'::float8), lgamma('Infinity'::float8), lgamma('-Infinity'::float8), lgamma('NaN'::float8)
...
SELECT gamma(ARRAY[1])
SELECT pg_catalog.lgamma('1'::text)
SELECT oid, proname, pronamespace, proowner, prolang, procost, prorows, provariadic, prosupport, prokind, prosecdef, proleakproof, proisstrict, proretset, provolatile, proparallel, pronargs, pronargdefaults, prorettype, proargtypes::text, proallargtypes, proargmodes, proargnames, proargdefaults::text, protrftypes, prosrc, probin, prosqlbody::text, proconfig, proacl FROM pg_catalog.pg_proc WHERE oid IN (6383, 6384) ORDER BY oid
...
SELECT upper('MiXeD')
SELECT lower('MiXeD')
-- casefold fixed signature
SELECT casefold('MiXeD'), casefold(''), pg_typeof(casefold('x'))
SELECT casefold(NULL), pg_typeof(casefold(NULL))
SELECT casefold((SELECT 'AlPhA'::text)), pg_catalog.casefold('AlPhA')
SELECT casefold()
SELECT casefold(1)
SELECT casefold('x', 'y')
SELECT casefold(value => 'x')
SELECT oid, proname, pronamespace, proowner, prolang, procost, prorows, provariadic, prosupport, prokind, prosecdef, proleakproof, proisstrict, proretset, provolatile, proparallel, pronargs, pronargdefaults, prorettype, proargtypes::text, proallargtypes, proargmodes, proargnames, proargdefaults::text, protrftypes, prosrc, probin, prosqlbody::text, proconfig, proacl FROM pg_catalog.pg_proc WHERE oid = 6412
...
SELECT chr(66)
SELECT md5('abc')
SELECT md5(decode('00ff10', 'hex')), pg_typeof(md5(decode('00ff10', 'hex')))
SELECT md5(NULL), pg_typeof(md5(NULL)), md5(NULL::bytea), pg_typeof(md5(NULL::bytea))
SELECT md5('abc'::varchar), md5('abc'::bpchar), md5('abc'::name), md5('a'::"char")
SELECT md5((SELECT decode('00ff10', 'hex'))), pg_typeof(md5((SELECT decode('00ff10', 'hex'))))
SELECT pg_catalog.md5('abc')
SELECT md5(1)
SELECT md5()
SELECT md5('a', 'b')
SELECT md5(value => 'abc')
SELECT md5(ARRAY[1])
SELECT pg_catalog.md5(1)
-- CRC checksums
SELECT crc32('abc'), crc32c('abc'), pg_typeof(crc32('abc')), pg_typeof(crc32c('abc'))
SELECT crc32(decode('00ff10', 'hex')), crc32c(decode('00ff10', 'hex'))
SELECT crc32(NULL), crc32c(NULL::bytea), pg_typeof(crc32(NULL)), pg_typeof(crc32c(NULL::bytea))
SELECT crc32((SELECT decode('00ff10', 'hex'))), pg_typeof(crc32((SELECT decode('00ff10', 'hex'))))
SELECT pg_catalog.crc32('abc'), pg_catalog.crc32c('abc')
SELECT crc32('abc'::text)
SELECT crc32('abc'::varchar)
SELECT crc32(1)
SELECT crc32()
SELECT crc32(decode('00', 'hex'), decode('01', 'hex'))
SELECT crc32(value => decode('00', 'hex'))
SELECT crc32c(ARRAY[1])
SELECT pg_catalog.crc32c(1)
SELECT oid, proname, pronamespace, proowner, prolang, procost, prorows, provariadic, prosupport, prokind, prosecdef, proleakproof, proisstrict, proretset, provolatile, proparallel, pronargs, pronargdefaults, prorettype, proargtypes::text, proallargtypes, proargmodes, proargnames, proargdefaults::text, protrftypes, prosrc, probin, prosqlbody::text, proconfig, proacl FROM pg_catalog.pg_proc WHERE oid IN (6364, 6365) ORDER BY oid
-- one-argument string and binary length overloads
SELECT length('é'), char_length('é'), character_length('é'), pg_typeof(length('é'))
SELECT length(decode('00ff10', 'hex')), octet_length(decode('00ff10', 'hex')), bit_length(decode('00ff10', 'hex')), pg_typeof(length(decode('00ff10', 'hex')))
SELECT length('a'::char(3)), char_length('a'::char(3)), character_length('a'::char(3)), octet_length('a'::char(3)), bit_length('a'::char(3))
SELECT length('é'::char(3)), octet_length('é'::char(3)), bit_length('é'::char(3))
SELECT octet_length('é'), bit_length('é')
SELECT length(NULL), pg_typeof(length(NULL)), length(NULL::bytea), pg_typeof(length(NULL::bytea))
SELECT char_length(NULL), character_length(NULL), octet_length(NULL::bytea), bit_length(NULL::bytea)
SELECT length('abc'::varchar), length('abc'::name), length('a'::"char")
SELECT length((SELECT decode('00ff10', 'hex'))), octet_length((SELECT 'a'::char(3))), bit_length((SELECT 'a'::char(3)))
SELECT pg_catalog.length('é'), pg_catalog.char_length('é'), pg_catalog.character_length('é'), pg_catalog.octet_length('é'), pg_catalog.bit_length('é')
SELECT length(1)
SELECT char_length(decode('00', 'hex'))
SELECT character_length(decode('00', 'hex'))
SELECT octet_length(1)
SELECT bit_length(1)
SELECT length(value => 'a')
SELECT char_length()
SELECT bit_length('a', 'b')
SELECT pg_catalog.bit_length(1)
SELECT oid, proname, pronamespace, proowner, prolang, procost, prorows, provariadic, prosupport, prokind, prosecdef, proleakproof, proisstrict, proretset, provolatile, proparallel, pronargs, pronargdefaults, prorettype, proargtypes::text, proallargtypes, proargmodes, proargnames, proargdefaults::text, protrftypes, prosrc, probin, prosqlbody::text, proconfig, proacl FROM pg_catalog.pg_proc WHERE oid IN (720, 1317, 1318, 1367, 1369, 1372, 1374, 1375, 1381, 1810, 1811, 2010) ORDER BY oid
...
-- JSON null stripping
SELECT json_strip_nulls('{"z":1,"a":null,"z":2,"s":"\u0061","n":1.2300e+02,"x":[null,{"d":null,"k":3}]}'::json)
SELECT json_strip_nulls('{"z":1,"a":null,"z":2,"s":"\u0061","n":1.2300e+02,"x":[null,{"d":null,"k":3}]}'::json, true)
SELECT jsonb_strip_nulls('{"z":1,"a":null,"z":2,"x":[null,{"d":null,"k":3}]}'::jsonb), jsonb_strip_nulls('{"z":1,"a":null,"z":2,"x":[null,{"d":null,"k":3}]}'::jsonb, true)
SELECT json_strip_nulls(target => '{"a":null,"x":[null]}'::json), json_strip_nulls(strip_in_arrays => true, target => '{"a":null,"x":[null]}'::json)
SELECT jsonb_strip_nulls(strip_in_arrays => true, target => '{"a":null,"x":[null]}'::jsonb)
SELECT pg_typeof(json_strip_nulls(NULL)), pg_typeof(json_strip_nulls('{}')), pg_typeof(jsonb_strip_nulls(NULL)), pg_typeof(jsonb_strip_nulls('{}'))
SELECT json_strip_nulls(NULL), json_strip_nulls('{}'::json, NULL), jsonb_strip_nulls(NULL), jsonb_strip_nulls('{}'::jsonb, NULL)
SELECT json_strip_nulls('null'::json), json_strip_nulls('[null]'::json), json_strip_nulls('[null]'::json, true)
SELECT json_strip_nulls((SELECT '{"a":null,"b":1}'::json)), jsonb_strip_nulls((SELECT '{"a":null,"b":1}'::jsonb))
SELECT pg_catalog.json_strip_nulls('{}'::json), pg_catalog.jsonb_strip_nulls('{}'::jsonb)
...
SELECT json_strip_nulls('{"a":}')
SELECT json_strip_nulls()
SELECT json_strip_nulls('{}'::json, false, true)
SELECT json_strip_nulls(bad => '{}'::json)
SELECT oid, proname, pronamespace, proowner, prolang, procost, prorows, provariadic, prosupport, prokind, prosecdef, proleakproof, proisstrict, proretset, provolatile, proparallel, pronargs, pronargdefaults, prorettype, proargtypes::text, proallargtypes, proargmodes, proargnames, proargdefaults::text, protrftypes, prosrc, probin, prosqlbody::text, proconfig, proacl FROM pg_catalog.pg_proc WHERE oid IN (3261, 3262) ORDER BY oid
...
SELECT to_hex(0), to_hex(42), to_hex(-42), to_hex((-42)::bigint)
SELECT to_hex((-2147483648)::integer), to_hex((-9223372036854775807 - 1)::bigint)
SELECT pg_typeof(to_hex(42)), pg_typeof(to_hex(42::bigint)), pg_catalog.to_hex(42)
SELECT to_hex(NULL::integer), to_hex(NULL::bigint), to_hex((SELECT (-1)::bigint))
SELECT to_hex('42')
SELECT to_hex(NULL)
SELECT to_hex(1::smallint)
SELECT to_hex((SELECT 1::smallint))
SELECT to_hex((SELECT '42'))
SELECT to_hex('42'::text)
SELECT to_hex(1::numeric)
SELECT to_hex()
SELECT to_hex(1, 2)
SELECT to_hex(value => 1)
SELECT oid, proname, pronamespace, proowner, prolang, procost, prorows, provariadic, prosupport, prokind, prosecdef, proleakproof, proisstrict, proretset, provolatile, proparallel, pronargs, pronargdefaults, prorettype, proargtypes::text, proallargtypes, proargmodes, proargnames, proargdefaults::text, protrftypes, prosrc, probin, prosqlbody::text, proconfig, proacl FROM pg_catalog.pg_proc WHERE oid IN (2089, 2090) ORDER BY oid
-- random ranges
...
-- UUID generators and fixed signatures
SELECT uuid_extract_version(gen_random_uuid()), uuid_extract_version(uuidv4()), uuid_extract_version(uuidv7()), uuid_extract_version(uuidv7(shift => interval '0'))
SELECT pg_typeof(gen_random_uuid()), pg_typeof(uuidv4()), pg_typeof(uuidv7()), pg_typeof(uuidv7(interval '0'))
SELECT pg_catalog.uuid_extract_version(pg_catalog.gen_random_uuid()), pg_catalog.uuid_extract_version(pg_catalog.uuidv4()), pg_catalog.uuid_extract_version(pg_catalog.uuidv7())
SELECT gen_random_uuid(1)
SELECT uuidv4(1)
SELECT uuidv7(1)
SELECT uuidv7(value => interval '0')
SELECT uuidv7(interval '0', interval '0')
SELECT oid, proname, pronamespace, proowner, prolang, procost, prorows, provariadic, prosupport, prokind, prosecdef, proleakproof, proisstrict, proretset, provolatile, proparallel, pronargs, pronargdefaults, prorettype, proargtypes::text, proallargtypes, proargmodes, proargnames, proargdefaults::text, protrftypes, prosrc, probin, prosqlbody::text, proconfig, proacl FROM pg_catalog.pg_proc WHERE oid IN (3432, 6428, 6429, 6430) ORDER BY oid
-- UUID extraction
