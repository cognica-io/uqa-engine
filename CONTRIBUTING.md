...
4. Add a workspace-internal `[workspace.dependencies]` entry of the form `uqa-foo = { version = "0.2.2", path = "crates/uqa-foo" }` so other crates can depend on it via `workspace = true` without a wildcard pin (cargo deny will reject wildcards).
