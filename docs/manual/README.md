...
This manual targets UQA Engine 0.1.6 and Rust 1.90 or newer. UQA Engine implements a large PostgreSQL-oriented surface, but it is an embedded engine rather than a PostgreSQL server clone. The [compatibility guide](sql/09-compatibility.md) states important differences and unsupported behavior.
