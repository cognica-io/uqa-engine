...
The provider's `mvcc::native` module defines native record codecs and a fixed family/column/primary-key inventory for the 43 current catalog tables. Table-owned data uses the supplied object identity and storage generation instead of its current table name in record keys; table and sequence definitions also validate their persisted identity fields against that owner. Rows preserve SQLite storage classes, including binary document fields and tuple metadata, and decoded payloads borrow their retained record buffers. The codecs enforce layout types, ordered key encoding, allocation limits and cancellation. They prepare records for the common persistence contract; they do not migrate native databases, update native materializations or enable concurrent Engine SQL. Native owner lookup, atomic materialization and complete store/catalog routing remain pending in the implementation plan.
