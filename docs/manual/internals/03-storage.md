...
Common Key/Value HNSW consumers use `KeyValueStore::with_read_view` to read metadata, graph nodes and canonical vectors from one fixed committed/private boundary. `with_mutation` evaluates once against the boundary supplying original write preconditions, then applies the staged canonical values and graph delta atomically. Errors, cancellation and unwinds discard that operation while preserving earlier private writes; publication failures retain the evaluated attempt for resolution. Neither callback may reenter its own store session or perform transaction control. Common logical sessions own this behavior; SQLite forwards the calls and redb uses the common implementation.

HNSW caches retain an immutable graph tagged with the database, committed sequence and relevant non-reused private record identity. Memory reference stores use retained identity tokens that change on writes and undo. Live handles refresh after rollback, savepoint branching, definition recreation and independent commits; retained graph snapshots keep their original results. Warm caches validate canonical-vector drift after the view changes. Third-party stores must implement the two compound operations to support HNSW; unsupported stores return an explicit error. These boundaries do not yet cover every common Key/Value index consumer or provide shared-index mutation merging. Common HNSW graph/cache/candidate/search workspace accounting remains required.
...
The provider's `mvcc::native` module maps the 43 current catalog tables, table-owner bindings and graph lookup entries into 45 fixed record families. Table-owned data uses its object identity and storage generation instead of its current table name in record keys; table and sequence definitions validate their persisted identity fields against that owner. Rows preserve SQLite storage classes, including binary document fields, tuple metadata and the distinct TEXT/BLOB B-tree namespaces. Decoded payloads borrow their retained record buffers. The codecs enforce layout types, ordered key encoding, allocation limits and cancellation, including complete validation of absent tombstone keys.
