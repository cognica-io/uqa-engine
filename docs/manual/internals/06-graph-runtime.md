...
Execution attaches the original query participant to a retained `CatalogReadView` when consuming AGE extension tables or graph entity relations. Graph names, label definitions, label sequences and graph entities use the same Graph-owned observers as direct reads, including empty graph catalogs. `ag_graph` reads graph presence without loading unrelated labels. Persistent handles share their retained resources; primary in-memory stores remain borrowed. Engine supplies the original context and cancellation through its catalog adapter.

PostgreSQL's pinned system catalogs and metadata views do not participate in SSI predicate locking. SQL classifies exact built-in relation identities; Execution projects their graph-derived rows from a metadata view without graph observers. Namespace casts, cached `reg*` output, effective search-path selection and schema privilege inquiries likewise create no graph predicates. Ordinary tables with catalog-like names and AGE extension tables retain their data observations. This follows PostgreSQL 18's [`PredicateLockingNeededForRelation`](https://github.com/postgres/postgres/blob/REL_18_STABLE/src/backend/storage/lmgr/predicate.c#L467-L539) boundary.

Virtual catalog rows are produced on the first row pull through the existing deferred source. Binding and planner snapshots stay unobserved, and `LIMIT 0` or an unused child does not create catalog-read dependencies. Typed cancellation, resource exhaustion and serialization diagnostics survive AGE data projection. Automatic public SQL serializable admission and the remaining transaction acceptance matrix remain incomplete.

Execution selects effective namespaces from the caller's catalog, role and search path. It preserves the first usable namespace, ordered full-path selection, duplicate removal, privilege checks and implicit catalog placement; Engine supplies session settings and retained catalog adapters.
