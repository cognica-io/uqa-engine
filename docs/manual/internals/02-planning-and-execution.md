...
CREATE TABLE inheritance and CHECK merging, CTAS column declarations, index-key analysis, and index naming are implemented under [`uqa-sql/src/schema/`](../../../crates/uqa-sql/src/schema). Unique-index creation validates visible rows through [`uqa-execution/src/schema/indexes.rs`](../../../crates/uqa-execution/src/schema/indexes.rs), reusing the bounded exact-key set and the session memory limit. Its inputs expose catalog reads and expression evaluation without mutation transaction services.
