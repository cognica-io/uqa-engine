...
Session portal dependency discovery is owned by [`session/portals/dependencies.rs`](../../../crates/uqa-engine/src/session/portals/dependencies.rs), which recursively expands CTE, view, routine, hierarchy, and table-function dependencies before a portal is sealed. Rule and trigger definition validation are separate owners under [`events/validation/`](../../../crates/uqa-engine/src/events/validation), so catalog publication does not mix the two event contracts or bypass their relation and row-type checks.
