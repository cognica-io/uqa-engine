...
Quoted phrases share one core `MemoryBudget` through complete query analysis, lossless key encoding, native graph preparation, candidate IDFs and scalar statistics, memoized matching, field-result accumulation, and final posting construction. Each producer retains its allocation reservation until its value is consumed or dropped. `uqa-storage` owns `analyze_query_graph_budgeted`; `uqa-operators` owns `PhraseBudget` and `score_phrase_budgeted`; `uqa-scoring::TextCandidateScorer` sums native contributions in emitted order without a temporary score vector. Execution supplies the current session allowance and cancellation token and maps failures to SQLSTATE `53200` or `57014`, including when analysis removes every query token. `StorageReadControl` carries the same allowance and cancellation token through provider cursor payloads, canonical lookup keys, field scalars, encoded reads, and occurrence decoding. Memory cursors borrow retained postings; persistent cursors retain one encoded score cluster and reuse one decoded block. The occurrence decoder borrows score/position directories, validates the entire target cluster, and allocates only the selected document’s occurrence list. SQLite reserves transient bound values and BLOB payloads before fetching, with size probes and payload reads in one snapshot. Key/Value callbacks borrow memory/redb payloads and use equivalent reserved SQLite reads. Typed provider failures retain their memory/cancellation identity through SQL error mapping.
