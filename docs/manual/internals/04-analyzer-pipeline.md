...
Number composition owns its consumed iterator, saved lookahead tokens, numeric scratch, and output buffer with unique leases. Borrowed tokens are copied through representation-owned helpers; moved tokens transfer their existing reservations. Decimal parsing counts digits before reserving one complete coefficient deque, whose retained capacity stays charged through front insertion, carry, alignment, and trimming. Formatting and malformed-prefix fallback retain their output reservations. Aborted prefixes, stacked-token fallthrough, and exhaustion attributes remain identical to the pinned native/common reference streams.
