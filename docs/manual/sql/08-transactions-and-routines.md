...
`INSTEAD OF` view triggers, direct ALTER or DROP operations on generated partition clones, trigger privileges, exact PostgreSQL `pg_node_tree` serialization, dump and restore, and the complete upstream trigger regression schedule remain compatibility bugs.
