...
On native SQLite, SQLite Key/Value and redb, SERIALIZABLE SQL selects its first data snapshot and registers its original transaction participant before binding the first snapshot-bearing statement. Session defaults also apply to implicit statements. BEGIN, SET TRANSACTION, SHOW and savepoints alone do not select that snapshot. SERIALIZABLE READ ONLY DEFERRABLE waits for the overlapping writers needed to establish a safe snapshot; an unsafe candidate is replaced before the statement executes. Cancellation reports `57014` through normal transaction-error recovery. A safe snapshot retains its selected data even when newer catalog definitions become visible after the wait. Read-only temporary DML and ANALYZE keep their existing permissions. The full concurrent-isolation and direct API acceptance matrix remains incomplete.
...
The command form takes an SQL identifier as its channel. `pg_notify` accepts text expressions; a NULL payload becomes the empty string, while a NULL or empty channel is invalid. Channel names must be shorter than 64 UTF-8 bytes, and payloads must be shorter than 8,000 UTF-8 bytes.
...
File-backed notification effects commit atomically with the main data through a bounded publication record. A surviving listener recovers the original sender identifier, messages and committed subscription changes if the sender disappears before the sidecar publication or wake signal finishes. Recovery does not depend on the listener's SQL snapshot and does not deliver historical messages to a session that only subscribes afterwards. A failure after the main commit reports that the transaction committed and notification completion awaits recovery; it does not undo the data. This is publication recovery, not an application-level acknowledgement or replay guarantee after all listeners disappear.

An indeterminate commit retains its original evaluated notification effects for later COMMIT/ROLLBACK resolution. Other sessions can recover a committed main transaction and receive its notifications before the sender resolves its receipt. An alive, still-uncommitted attempt preserves publication order: later publishers honor cancellation and the existing bounded registry wait, while ordinary listener polling and transaction completion remain available.
...
```sql execute
LISTEN manual_events;
NOTIFY manual_events, 'ready';
UNLISTEN manual_events;
```
...
A NULL argument returns NULL. An unknown setting reports `42704` unless `missing_ok` is true, in which case the result is NULL. Other lookup errors are preserved. An incompatible argument type or arity reports `42883`.

```sql execute
SET application_name TO 'manual-settings';
SELECT current_setting('application_name') AS application;
SELECT current_setting('missing_manual_setting', true) AS absent;
RESET application_name;
```

`SET ROLE name` changes `current_user` for the session while preserving `session_user`; `RESET ROLE`, `SET ROLE NONE`, and `SET ROLE TO DEFAULT` restore the session identity. Quoted or string-literal role names preserve case, so `SET ROLE "NONE"` and `SET ROLE 'default'` select those named roles; a missing role reports `22023`. The embedded connection starts as the durable bootstrap superuser role `uqa`, so it may assume any defined role. A non-superuser session may assume a directly or transitively granted role only through membership edges whose `SET` option is true. A successful `SET ROLE` executed by a `SECURITY INVOKER` routine remains visible to the session, while an error restores the prior identity and any `SET ROLE` attempted inside a `SECURITY DEFINER` context fails with `42501`, including through a nested invoker.

`SET SESSION AUTHORIZATION name` changes both `session_user` and `current_user` and clears an explicit role selection. `RESET SESSION AUTHORIZATION` and `SET SESSION AUTHORIZATION DEFAULT` restore the originally authenticated identity. Selecting a different identity requires the originally authenticated role's current superuser attribute; a later promotion or demotion takes effect without reopening the session. Both authorization parameters support `SET LOCAL`, transaction rollback and savepoint restoration. `SHOW role` reports the explicit selection or `none`, and `SHOW session_authorization` reports the selected session-authorization name.

Sessions retain role OIDs and creation identities independently of names. Dropping a selected role and creating another with the same name does not transfer the replacement's attributes, memberships or ACL rights to the existing session. PUBLIC privileges still apply. `CURRENT_USER` or `SESSION_USER` reports `42704` when its retained role no longer exists; reset operations restore the retained identity without looking up a replacement by name. An explicit role-name privilege inquiry resolves the currently named role.
...
Top-level `DECLARE`, `FETCH`, `MOVE`, and `CLOSE` use session portals with PostgreSQL 18 cursor SQLSTATEs, forward, backward, absolute, and relative positioning, `SCROLL` and `NO SCROLL`, and `WITH HOLD`. When neither scroll option is specified, the query must support native backward scanning and have no row locks to allow backward movement. For example, `SELECT 1` defaults to forward-only, while a plain table scan can move backward. Outer ordering can make window, target-set and `UNION ALL` output scrollable; a target set above sorting or a window that already supplies the requested ordering remains forward-only. Deferred CTEs inherit their query’s traversal support, while materialized CTE scans can move backward. Constant ordering keys do not create backward support. Explicit `SCROLL` enables the existing materialization path when needed. Scrollable scan, `VALUES`, filter, projection, sort, and limit pipelines re-execute volatile expressions in the requested direction when PostgreSQL does; physical plans without backwards execution materialize at their supported semantic boundary so revisits retain the values PostgreSQL freezes. `UNION ALL` keeps backwards-capable child plans live and crosses `Append` boundaries in child order or reverse child order; if any child cannot scan backwards, the complete `Append` output is materialized incrementally. A non-holdable portal closes at transaction end, a holdable portal survives commit, and rolling back to a savepoint closes portals declared after that savepoint without rewinding older portal positions. A PostgreSQL simple-query command string may declare and fetch a cursor inside its shared implicit transaction, after which a non-holdable cursor closes. Cursor behavior for PostgreSQL physical node families not implemented by UQA remains open.

`pg_catalog.pg_cursors` exposes the current session’s SQL and PL/pgSQL portals, including implicit query-loop cursors and a cursor executing its own query. Its columns are `name text`, `statement text`, `is_holdable boolean`, `is_binary boolean`, `is_scrollable boolean`, and `creation_time timestamptz`. SQL DECLARE preserves the complete submitted message; PL/pgSQL preserves the original static query or evaluated dynamic SQL before parameter and variable binding. Creation time is the containing SQL message’s statement timestamp. The flags describe the declaration, so automatically retaining a procedural loop cursor across COMMIT does not change `is_holdable`. Closing, transaction/savepoint rollback and DISCARD remove rows with their corresponding portals; WITH HOLD entries survive commit. The view is read-only and follows ordinary catalog name resolution and SELECT privileges.
...
Creation, replacement, rename, enable state, and removal update the durable trigger catalog transactionally and invalidate affected prepared plans. Trigger DDL has no row result. The trigger relation, trigger function, and an optional constraint-trigger `FROM` relation resolve once through the active role's effective schema `USAGE` boundary; qualified inaccessible schemas report `42501`, missing qualified schemas report `3F000`, and unqualified lookup skips inaccessible search-path entries. Their canonical identities are stored, so catalog restoration and trigger execution do not reinterpret those names through the restoring or executing session's namespace. `DROP TRIGGER IF EXISTS` distinguishes a missing schema from a missing relation in its notice. Invalid timing/event or transition-relation combinations, non-Boolean or out-of-scope `WHEN` expressions, wrong function signatures, dependency violations, and unsupported trigger kinds fail before partial catalog publication with the PostgreSQL-shaped SQLSTATE documented by the compatibility tests.
...
The rule and relation names are identifiers. `OLD` and `NEW` expose the event-row fields valid for the selected event, the optional condition is a Boolean expression over the supported event scope, and each action is an implemented `SELECT`, `INSERT`, `UPDATE`, `DELETE`, or unconditional `NOTIFY` statement, including `INSERT DEFAULT VALUES`. `ALSO` preserves the original command; `INSTEAD` conditionally or unconditionally replaces it; `NOTHING` suppresses it for matching rows.

### Result and effects

Rule DDL has no row result. Definitions, enable state, rename, replacement, and removal are durable and transactional, update `pg_rewrite` and `pg_rules`, participate in relation and column lifecycle, and invalidate affected prepared plans. Active row-bearing actions execute once over the qualified OLD/NEW row set rather than once as an independent SQL statement per event row. An unconditional `NOTIFY` action executes once for its event statement even when the event affects zero or multiple rows, follows outer-transaction commit and rollback, and participates in transaction-wide notification deduplication. `OLD.*` and `NEW.*` list items in action `VALUES` rows, `SELECT` target lists, and `ROW` constructors expand to the event relation's columns in declaration order when the rule is created; the stored expressions remain fixed when columns are added, follow column rename and drop dependencies, and persist across reopen.

### Errors

Creation rejects invalid OLD/NEW scope, result-column or type mismatches for a DML `RETURNING` provider, multiple providers, recursive definitions, reserved `_RETURN` misuse, and unsupported targets before publishing the definition. A conditional `NOTIFY` action reports SQLSTATE `42P17`; active rules reject `ON CONFLICT` and `MERGE`; unsupported conditional set-operation and multi-row rewritten action shapes report their PostgreSQL 18 SQLSTATEs.
...
Durable table rewrite rules execute INSERT VALUES, INSERT DEFAULT VALUES, INSERT SELECT, UPDATE, DELETE, and unconditional `NOTIFY` actions with PostgreSQL 18 cardinality: OLD- or NEW-referencing actions consume the matching event rows set-oriented, row-independent UPDATE and DELETE actions follow the original command's qualification tuples, including no-predicate, false-predicate, empty-target, `UPDATE FROM`, and `DELETE USING` cases, and `NOTIFY` runs once per event statement even for zero or multiple affected rows. Rule definition and removal targets and DML action targets resolve once through the active role's effective schema `USAGE` boundary; missing qualified schemas report `3F000` for the rule relation but the ordinary DML `42P01` for an action target, and `DROP RULE IF EXISTS` distinguishes a missing schema from a missing relation in its notice. Each stored DML action retains its canonical target identity and does not repeat namespace lookup when the rule later executes or the catalog is restored. Each rule also stores creation-bound event, source, and action-target column identities; unique unqualified source references are qualified at creation, projection stars and positional INSERT targets are frozen to the then-visible attributes, scalar whole-row values remain live, and rename, DROP RESTRICT and CASCADE, transaction rollback, and reopen preserve those identities. Ordinary-table, regular-view, materialized-view, and foreign-table renames rewrite creation-bound rule and dependent-view references; regular-view event rules follow the renamed event relation, materialized-view and foreign-table event definitions retain PostgreSQL SQLSTATEs `0A000` and `42809`, and views, materialized views, and foreign tables preserve their relation and row-type OIDs. Each bound action remains one statement, so its statement trigger fires once rather than once per source row. The implemented surface includes collision-free internal row sources, correlated LATERAL action sources, PostgreSQL query-scope rejection for event-row references in CTEs, set-operation members, and `ON CONFLICT DO UPDATE`, unqualified INSERT and DELETE conditions, alphabetical `ALSO` and conditional or unconditional `INSTEAD` ordering, `NOTHING`, recursion protection, replacement with enable-state retention, relation and rule-name lifecycle, view `_RETURN` replacement and protection, and PostgreSQL-shaped `pg_rewrite`, `pg_rules`, rule flags, and `pg_get_ruledef`. Rule `WHERE` conditions accept constant, correlated, scalar, `IN`, external-relation, locally shadowed, and correlated-CTE subqueries over event-appropriate OLD and NEW rows; validate Boolean type, names, routines, and relations before catalog publication; retain bound relation and routine identities across search-path changes and reopen; evaluate INSERT actions against PostgreSQL's action-time relation state; report scalar cardinality as `21000` atomically; authorize condition range-table relations as the live rule-relation owner while routine calls retain the invoker; and preserve exact event-column projection, rename, dependency, and SQL deparsing state. As in PostgreSQL 18, an unconditional `ON INSERT` action containing `UNION`, `INTERSECT`, or `EXCEPT` succeeds for a one-row insert but returns `0A000` when a multi-row insert makes the rewritten action conditional. An unconditional `INSTEAD` rule can provide DML `RETURNING` positionally with PostgreSQL-compatible creation-time row type and type-modifier checks, single-provider restrictions, lazily evaluated provider projections, provider action current/OLD/NEW images, outer row-image aliases, outer `UPDATE FROM` source columns for UPDATE-provider actions, and outer `DELETE USING` source columns for DELETE-provider actions. Scalar OLD and NEW whole-row composites in conditions, actions, and action `RETURNING` use the live event-row shape across add, rename, drop, and durable reopen, preserve set-oriented cardinality and exact event-side errors, and let catalog-backed local relation, column, derived-table, and CTE names shadow the pseudo-rows. Within an action `RETURNING` list, INSERT action default `OLD` and `NEW` names denote action row images, UPDATE and DELETE action defaults denote rule-event rows, and explicit `RETURNING WITH` aliases denote action images; event-row stars expand at rule creation, retain set-oriented cardinality and rename, drop, and reopen dependencies, and report PostgreSQL namespace errors and added-column `XX000` width failure atomically. Action-target `RETURNING` stars expand at rule creation across bare, target-alias, default-image, and explicit-image namespaces, ignore later additions, follow rename and drop dependencies, and survive durable reopen. Active rules reject `ON CONFLICT` and `MERGE` with the PostgreSQL SQLSTATEs. Rule enable modes follow `session_replication_role`: Origin and Always rules run for `origin` and `local`, Replica and Always rules run for `replica`, and Disabled rules never run. Remaining cross-object dependencies, inheritance and partition behavior, remaining privilege object kinds, exact node trees and complete deparsing, dump/restore, and the complete upstream rule schedule remain compatibility bugs.

Relations introduced by a rule action, including a sequence read as a range-table relation, are authorized as the rule relation's live owner. User-defined function `EXECUTE` checks in rule conditions and actions and `nextval`, `currval`, `lastval`, and `setval` sequence-function checks instead use the invoking role, including a sequence function reached through an action target's default expression; SQL-visible `current_user` also remains the invoker. Transferring the rule relation changes only the relation privilege subject, not these expression privilege subjects.
...
Multi-target deletion checks CREATEROLE once at command entry. Each target is resolved and authorized, waits for its shared-object lock, then removes memberships where that identity is the role or member before processing the next target. Later target errors therefore follow earlier waits, and earlier membership removals can remove the caller’s ADMIN path for a later or repeated target. After this ordered traversal, dependency checks and definition-tuple deletion follow the distinct targets in written order. Any failure rolls back the statement’s role and membership effects; transaction and savepoint rollback preserve the same atomic behavior.
...
A missing source reports `42704`. The session user and the outer selected user cannot be renamed (`0A000`), including inside SECURITY DEFINER code; a definer's effective owner can otherwise be renamed. Reserved `pg_` names report `42939`, and an already visible destination reports `42710`, before permission checks. Superusers may rename other roles; renaming a non-superuser otherwise requires CREATEROLE and direct or transitive ADMIN authority over that role (`42501` on denial). Role definition changes and deletion follow membership paths regardless of their INHERIT or SET options; the final edge must carry ADMIN authority. Read-only transactions reject the command (`25006`). Competing definition changes retain the original tuple: a committed update or deletion during its wait reports `XX000`, while rollback permits the rename. Concurrent creators or renamers of its destination wait for the reservation; a committed destination reports `23505`, while rollback permits publication.

```sql execute
CREATE ROLE manual_rename_source;
ALTER ROLE manual_rename_source RENAME TO manual_rename_target;
SELECT rolname FROM pg_roles WHERE rolname = 'manual_rename_target';
DROP ROLE manual_rename_target;
```

Role OIDs are assigned when a role is created and retained through attribute changes, renaming, rollback restoration, catalog refresh and reopen. Dropping and recreating a name creates a new role identity. Owner fields in `pg_class`, `pg_namespace`, `pg_proc` and `pg_database`, membership references in `pg_auth_members`, `pg_roles.oid`, `pg_user.usesysid` and `regrole` conversion use the stored identity. Opening an existing database preserves its role OIDs and rejects invalid or duplicate persisted OIDs; the bootstrap identity remains OID 10 even after renaming. Catalog refresh during `REPEATABLE READ` and `SERIALIZABLE` preserves uncommitted role definitions and memberships, including when a direct Rust API read triggers the refresh; savepoint and full rollback restore the corresponding role state. In the development concurrent providers, independent role creation and attribute changes can commit while another session retains private role changes. Concurrent creators of the same role name wait for the first transaction; its commit produces a unique-constraint error (`23505`) in the waiter, while rollback permits creation. A name already visible before creation still produces `42710`. Name and OID reservations follow transaction and savepoint rollback. Refresh combines the committed and private role records; committing, rolling back or undoing a savepoint retains the other session’s committed roles.
...
A non-superuser may grant or revoke membership only when it has the target role's ADMIN option directly or inherits the privileges of a role that does; SET permission alone does not authorize the change. Only a superuser may change membership of a superuser role. Without `GRANTED BY`, a superuser records the bootstrap role (initially `uqa`) as grantor, while another user records its nearest inherited ADMIN holder. An explicit grantor requires the current user to inherit that grantor's privileges; a GRANT additionally requires the grantor's own direct ADMIN option unless it is the bootstrap role. ADMIN grants cannot depend on the authority being granted back: all recipients in the statement are checked together, and the grantor must retain an independent ADMIN source.
...
Routine ownership and EXECUTE privileges follow transitive membership edges whose `INHERIT` option is true. An inherited owner may alter, replace, grant on, or drop the routine and retains the owner's implicit grant options. EXECUTE itself follows the owner's revocable ACL entry; a `SET`-only member must first assume the owner role. An actual ownership transfer additionally requires the current user to be able to `SET ROLE` to the new owner and requires the new owner to have `CREATE` on the routine's schema; superusers bypass these checks. An unchanged routine owner skips ownership, SET and schema CREATE checks, while ordinary namespace USAGE lookup still applies. Every explicit DROP target is ownership-checked before dependency expansion, so unauthorized multi-target replacement or removal cannot mutate the routine graph.

`has_function_privilege([user,] function, privilege)` accepts an optional role name (`name`) or OID, a function/procedure signature (`text`, such as `app.calculate(integer)`) or OID, and a privilege string. Omitting the user checks `current_user`; the six overloads return `boolean`. `EXECUTE` and `EXECUTE WITH GRANT OPTION` are case-insensitive, and a comma-separated list succeeds when any requested privilege is present. Text signatures resolve through the invoking user's namespace permissions and search path; an OID lookup does not require schema USAGE. The inquiry reads catalog permissions without invoking the routine or changing state.

Any NULL argument produces NULL. A missing named role reports `42704`; lowercase `public` means PUBLIC, and an unknown role OID still receives PUBLIC privileges. Invalid signature syntax reports `22P02`, an unknown argument type reports `42704`, and an unknown named routine reports `42883` before privilege-string validation. Invalid privileges report `22023`. A missing routine OID produces NULL for an ordinary subject and true for a superuser; a numeric text signature with a missing nonzero OID instead reports `XX000` for an ordinary subject. Superusers bypass ACL checks after name and privilege validation. Owners retain grant options even after revoking their own EXECUTE; calls and `pg_proc.proacl` reflect the revocation, including after reopen.

```sql execute
SELECT has_function_privilege('pg_catalog.abs(integer)', 'EXECUTE') AS allowed;
```

`SECURITY INVOKER` runs with the caller's `current_user`; `SECURITY DEFINER` temporarily uses the routine owner while `session_user` remains unchanged. Routine `SET` and `SET ... FROM CURRENT` configure values for the call; routine `RESET` and `RESET ALL` remove those configuration entries. A configured parameter restores its caller value on successful return unless an ordinary `SET` or `RESET` in the body overrides it. An ordinary assignment also supersedes enclosing routine configuration for that parameter. `SET LOCAL` on a configured parameter restores at the corresponding function exit; `SET LOCAL` on an unconfigured parameter lasts until transaction end. Errors restore the preceding values and their pending LOCAL restoration, and definition-time configuration validation preserves that restoration state. Authorization restoration retains the original role identity even when another session deletes and recreates its name. Function and procedure creation enforces schema `CREATE`. Calls and ALTER, DROP, GRANT, and REVOKE routine-target lookup enforce schema `USAGE` for the effective role: qualified inaccessible schemas fail with `42501` before routine existence or `EXECUTE` is considered, unqualified lookup skips inaccessible search-path entries, inherited grants apply, and revocation invalidates a prepared statement before its next execution. Ordinary relation lookup follows the same namespace rule. Views, materialized views, stored generated expressions, SQL-standard routine query bodies, and declared cursors retain exact identities bound by an authorized creator and therefore do not repeat the original schema-name access check; string-literal SQL and PL/pgSQL bodies resolve names under the active invoker or definer identity. Ordinary tables, regular views, materialized views, and foreign tables persist their creating role and enforce their implemented owner, ACL, catalog, and durable lifecycle. Passwords, per-role settings, remaining object schema `USAGE`, database `CONNECT` enforcement at the embedding connection boundary, remaining relation-owner enforcement, default privileges, row-level-security consequences, extension languages, and the remaining PostgreSQL object privilege model remain compatibility bugs. Database and schema ACLs and their six privilege-inquiry overloads are described in [Data Definition Language](02-ddl.md#database-privileges) and [Schemas](02-ddl.md#schemas).

Foreign tables also persist their creating role and relation and column ACLs as one authorization state, enforce inherited owner or containing-schema-owner lifecycle authority, validate owner transfers and rewrite owner-rooted grants before publication, block removal of dependent owners, grantors, or grantees, and restore authorization through transaction and savepoint rollback, cross-engine catalog refresh, explicit migration, and reopen. Direct and nested scans use the active invoker or stored view security subject for exact table or column `SELECT` checks.
