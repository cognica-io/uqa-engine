...
A row-returning command cursor does not execute at `OPEN`. Its complete command runs when the portal is first used, including by `MOVE 0`, and execution errors are reported by that operation; closing the unread cursor has no command side effects. `CALL` returns its output-parameter row, and `MERGE ... RETURNING` completes every selected action before the first row is fetched. Query cursors evaluate target expressions for rows discarded by `OFFSET`, stop before rows excluded by `LIMIT`, and remain one-row incremental during forward movement. An opened cursor is a session portal, so a routine may return its `refcursor` name and a later routine or SQL statement in the same session and transaction may continue fetching. Exact volatile-expression reevaluation during backward movement and scrolling revisits remains open.
