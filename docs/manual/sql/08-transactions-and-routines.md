...
PostgreSQL 18 transition relations are available to ordinary `AFTER` row and statement triggers through `REFERENCING OLD TABLE AS name` and `REFERENCING NEW TABLE AS name`. An `INSERT` trigger may name only the new table, a `DELETE` trigger only the old table, and an `UPDATE` trigger either or both; a transition trigger must name exactly one mutation event and cannot use `UPDATE OF`, `TRUNCATE`, `BEFORE`, constraint-trigger syntax, duplicate transition kinds, identical old and new names, or `OLD ROW` and `NEW ROW`. Each invocation sees the complete typed row set for its statement, including an empty set for a zero-row statement, post-`BEFORE` values and stored generated values, separate action sets for `ON CONFLICT` and `MERGE`, `UPDATE FROM`, recursive foreign-key cascades, and root-shaped rows aggregated across partition and inheritance descendants. Multiple foreign-key actions on the same relation and event preserve cumulative row changes and use PostgreSQL's single statement boundary: `BEFORE` statement triggers use the first action's updated columns, while `AFTER` statement triggers use the union and transition relations contain every action row image. Row-level transition triggers are rejected on a partitioned table, a partition, or an inheritance child. Transition relations are execution-scoped and cannot be captured by a persistent view or materialized view.
...
`INSTEAD OF` view triggers, partition-moving update semantics, direct ALTER or DROP operations on generated partition clones, `session_replication_role`, trigger privileges, exact PostgreSQL `pg_node_tree` serialization, dump and restore, and the complete upstream trigger regression schedule remain compatibility bugs.
