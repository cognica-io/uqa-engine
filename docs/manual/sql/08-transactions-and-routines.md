...
A missing source reports `42704`. The session user and the outer selected user cannot be renamed (`0A000`), including inside SECURITY DEFINER code; a definer's effective owner can otherwise be renamed. Reserved `pg_` names report `42939`, and an already visible destination reports `42710`, before permission checks. Superusers may rename other roles; renaming a non-superuser otherwise requires CREATEROLE and ADMIN authority over that role (`42501` on denial). Read-only transactions reject the command (`25006`). Competing definition changes retain the original tuple: a committed update or deletion during its wait reports `XX000`, while rollback permits the rename. Concurrent creators or renamers of its destination wait for the reservation; a committed destination reports `23505`, while rollback permits publication.
