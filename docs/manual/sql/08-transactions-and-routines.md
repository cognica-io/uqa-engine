...
The rule and relation names are identifiers. `OLD` and `NEW` expose the event-row fields valid for the selected event, the optional condition is a Boolean expression over the supported event scope, and each action is an implemented `INSERT`, `UPDATE`, or `DELETE` statement, including `INSERT DEFAULT VALUES`. `ALSO` preserves the original command; `INSTEAD` conditionally or unconditionally replaces it; `NOTHING` suppresses it for matching rows.

### Result and effects

Rule DDL has no row result. Definitions, enable state, rename, replacement, and removal are durable and transactional, update `pg_rewrite` and `pg_rules`, participate in relation and column lifecycle, and invalidate affected prepared plans. Active actions execute once over the qualified OLD/NEW row set rather than once as an independent SQL statement per event row. `OLD.*` and `NEW.*` list items in action `VALUES` rows, `SELECT` target lists, and `ROW` constructors expand to the event relation's columns in declaration order when the rule is created; the stored expressions remain fixed when columns are added, follow column rename and drop dependencies, and persist across reopen.
