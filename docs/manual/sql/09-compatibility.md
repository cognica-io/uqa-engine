...
The implemented trigger slice provides durable PL/pgSQL `BEFORE` and `AFTER` row and statement execution for `INSERT`, `UPDATE`, `DELETE`, and `TRUNCATE`, including `UPDATE OF`, `WHEN`, PostgreSQL name ordering, zero-row statements, deferred `AFTER ROW` visibility, generated-column `NEW` images, `ON CONFLICT`, `MERGE`, partition clones, transactional DDL, table and column rewrites, exact trigger-function dependencies, `pg_trigger`, `relhastriggers`, `hastriggers`, and `pg_get_triggerdef`. The checked-in 24-case stateful oracle uses PostgreSQL 18.4 with Apache AGE and reopens UQA between every case.
