...
- PostgreSQL-shaped `information_schema` and `pg_catalog` virtual relations carry declared PostgreSQL 18 row types even when empty; implemented `pg_type` rows expose exact scalar, array, information-schema domain, pseudo-type, and `information_schema_catalog_name` metadata. Scalar and array `regproc`, `regclass`, `regnamespace`, and `regtype` values use catalog-aware PostgreSQL text output in explicit casts, CLI rendering, and `COPY TO`, including `0` as `-`, unresolved OIDs as decimal text, visible-name qualification, NULL preservation, and PostgreSQL built-in type aliases.
...
UQA Engine does not yet implement PostgreSQL processes, network protocol semantics as its core API, MVCC storage pages, complete role membership and non-routine object privileges, extensions, background workers, replication, WAL administration, or server configuration. The optional PostgreSQL wire and FDW crates are adapters around the engine. Every externally visible difference caused by this architecture remains an open compatibility bug rather than an accepted alternative behavior.
...
The implemented trigger slice provides durable PL/pgSQL `BEFORE` and `AFTER` row and statement execution for `INSERT`, `UPDATE`, `DELETE`, and `TRUNCATE`, including `UPDATE OF`, create-time boolean `WHEN` validation, row-change-time `AFTER WHEN` selection with deferred function execution, PostgreSQL name ordering, recursive referential-action row and statement triggers, ordered explicit, descendant, and cascading `TRUNCATE` targets, zero-row statements, stored and virtual generated-column images, `ON CONFLICT`, `MERGE`, executable trigger replacement, partition clones, transactional and temporary catalog lifecycle, table and column rewrites, exact trigger-function dependencies, `pg_trigger`, `relhastriggers`, `hastriggers`, and compact and pretty `pg_get_triggerdef`. The checked-in 36-case stateful oracle uses PostgreSQL 18.4 with Apache AGE and reopens UQA between every case.

- Constraint and deferred triggers, transition relations, `INSTEAD OF` view triggers, partition-moving update semantics, direct partition-clone lifecycle, `session_replication_role` execution, trigger privileges, exact `pg_node_tree` serialization, dump and restore, and the complete PostgreSQL trigger regression schedule remain open.
- Durable table rewrite rules support `CREATE RULE`, `CREATE OR REPLACE RULE`, `DROP RULE`, rename and enable modes, INSERT VALUES, INSERT SELECT, UPDATE, and DELETE actions executed once over each qualified OLD/NEW row set, collision-free and correlated LATERAL action sources, PostgreSQL query-scope rejection for CTEs, set-operation members, conditional set-operation actions, the PostgreSQL 18 one-row success and multi-row `0A000` boundary for unconditional `ON INSERT` set-operation actions, and `ON CONFLICT DO UPDATE`, action statement-trigger cardinality, unqualified INSERT and DELETE conditions, alphabetical `ALSO` and conditional or unconditional `INSTEAD` ordering, `NOTHING`, recursion detection, active-rule `ON CONFLICT` and `MERGE` rejection, target-column lifecycle, view `_RETURN` replacement and protection, `pg_rewrite`, `pg_rules`, rule flags, and both `pg_get_ruledef` overloads. Unconditional `INSTEAD` providers implement positional DML `RETURNING` with creation-time row type and type-modifier checks, single-provider restrictions, lazy projection evaluation, provider current/OLD/NEW images, outer aliases, `UPDATE FROM` source-column projection for UPDATE-provider actions, and `DELETE USING` source-column projection for DELETE-provider actions. A 177-case PostgreSQL 18.4 with Apache AGE oracle verifies the implemented slice across durable reopen boundaries. Writable-view DML rules, condition subqueries, OLD/NEW record expansion, `NOTIFY` actions, complete dependencies, inheritance and partition behavior, `session_replication_role`, privileges, exact node trees and deparsing, dump/restore, and the complete upstream rule schedule remain compatibility bugs.
...
`ALTER FUNCTION` and kind-neutral `ALTER ROUTINE` change volatility, null-input behavior, security mode, leakproofness, parallel-safety, support-function identity, and routine-local configuration for exact or uniquely visible function identities while preserving the compiled body and durable catalog state; the three ALTER routine kinds also transfer ownership. Explicit empty and omitted signatures remain distinct, `%TYPE`, search-path, ambiguity, missing-object, wrong-kind, and privilege resolution follow the tested PostgreSQL 18 boundary, and applying function-only attributes to a procedure reports `42P13`. Role membership, non-routine privileges, extension languages, complete grantor chains, and full support-function and planner consequences remain open under `routines.extension-language-privilege-security`.
