...
- Rewrite rules remain unimplemented. `CREATE RULE` execution, OLD and NEW rewrite binding, `DO ALSO`, `DO INSTEAD`, `DO NOTHING`, recursion, view `_RETURN` rules, `pg_rewrite`, `pg_get_ruledef`, enable modes, dependencies, and the official rule regression schedule are tracked separately and are not represented as parser-only support.
