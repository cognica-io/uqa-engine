...
Relation names are resolved through the current role's effective `search_path`. An unqualified lookup skips schemas on which the role lacks `USAGE`, while a qualified name in an inaccessible schema raises `42501` before relation existence or column validation. The same namespace boundary applies to ordinary queries, `INSERT`, `UPDATE`, `DELETE`, `TRUNCATE`, supported relation DDL, trigger and rule targets, rule-action mutation targets, hierarchy parents, foreign-key references, and hard or soft `regclass` input. ACL revocation is observed by the next execution of a prepared statement. A stored view, materialized view, SQL-standard routine query body, declared cursor, or stored rule-action mutation target instead retains the exact relation identity authorized when it was bound, so executing it does not reinterpret the original schema-qualified text under the caller's `search_path`; string SQL remains dynamically resolved under its active invoker or definer identity.

Virtual catalog names use that same namespace order. When `pg_catalog` is absent from `search_path`, it is searched before the listed schemas; when explicitly listed, its position is respected. The session's temporary schema is searched first unless `pg_temp` or its actual schema name is explicitly positioned. `information_schema` and `ag_catalog` are searched for unqualified names only when listed. Quoted identifier case is preserved. Query rows and schemas, stored view and cursor sources, and `regclass` input/output resolve the same selected relation, including a user relation that precedes a virtual catalog relation. These namespace rules follow [PostgreSQL 18 schema search semantics](https://www.postgresql.org/docs/18/ddl-schemas.html#DDL-SCHEMAS-PATH).
