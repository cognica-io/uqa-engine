...
`CREATE MATERIALIZED VIEW` stores a query snapshot, supports `WITH [NO] DATA`, persists its rows and static schema across reopen, and remains stale until owner-authorized `REFRESH MATERIALIZED VIEW [WITH [NO] DATA]`; direct `INSERT`, `UPDATE`, and `DELETE` report PostgreSQL's relation-kind SQLSTATE `42809`. The active creating role owns the materialized view; inherited owner authority protects ALTER and refresh, direct DROP also permits the owner of the containing schema, and `ALTER MATERIALIZED VIEW ... OWNER TO` applies the same target-role rules and durable lifecycle as regular views. `pg_class` exposes relation kind `m`, owner, population state, persistence, and reloptions, while `pg_matviews` exposes the owner and implemented materialized-view metadata. The supported materialized-view option is `fillfactor`, including owner-authorized `ALTER MATERIALIZED VIEW ... SET/RESET`; materialized-view `MAINTAIN` ACL delegation, temporary and unlogged materialized views, concurrent refresh, materialized-view indexes, access methods, tablespaces, and dependencies on temporary relations are not implemented.
...
`TRUNCATE` removes all rows from its listed tables under a transaction boundary. SQL drop supports implemented table, index, view, materialized-view, schema, function, and procedure targets. `DROP TABLE ... CASCADE` removes the complete transitive closure of dependent views without requiring separate ownership of those dependent objects, and relation-kind mismatches return PostgreSQL-compatible errors. Unsupported dependency-sensitive `CASCADE` forms fail rather than partially changing the catalog.
