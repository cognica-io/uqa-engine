...
| Arguments | `name` is a string value naming a graph, not an SQL relation identifier; a valid AGE graph name is 3 to 63 bytes, starts with a Unicode letter or underscore, continues with letters, digits, marks, underscores, dots, or dashes, and ends with a letter, digit, mark, or underscore; `cascade = true` removes the graph with every surviving label, while false or omitted uses AGE's restrictive schema drop |
| Result | SQL `NULL` (the AGE-compatible `void` result) on success |
| Effects | Creates or removes the durable named graph in the current transaction; a created graph reserves a namespace of the same name and initially owns the AGE default labels `_ag_label_vertex` (label id 1) and `_ag_label_edge` (label id 2); a restrictive drop succeeds after every label relation has already been removed, while a cascading drop also removes stored views that depend directly or transitively on its label relations |
| Errors | Apache AGE messages and SQLSTATEs: a SQL `NULL` name is `graph name can not be NULL` (`22023`), an invalid name is `graph name is invalid` (`22023`), a duplicate graph is `graph "name" already exists` (`3F000`), a name that is already a schema is `schema "name" already exists` (`42P06`), a missing drop target is `graph "name" does not exist` (`3F000`), and a false or omitted drop while the namespace still owns a label is `cannot drop schema name because other objects depend on it` (`2BP01`); wrong arity or argument types and persistence failures are also rejected |
...
| Effects | `create_vlabel` / `create_elabel` register an empty vertex or edge label with the next label id, so its graphids are allocated ahead of any entity; `drop_label` follows AGE's `DROP TABLE ... RESTRICT`, removes the target relation, sequence, catalog row, and its own vertex or edge rows, but preserves rows in other label relations, including edges whose endpoint vertex rows were removed; either default label can be removed and stays absent across rename and reopen; `alter_graph ... RENAME` renames the graph, its namespace, and surviving catalog rows while keeping every entity id |
| Errors | Apache AGE messages and SQLSTATEs: `graph name must not be NULL` / `label name must not be NULL` (`22023`), `graph name is invalid` / `label name is invalid` (`22023`), `graph "name" does not exist.` (`3F000`, with the trailing period AGE prints from `create_vlabel` and `create_elabel`), `label "name" already exists` (`3F000`), `graph "name" does not exist` and `label "name" does not exist` for `drop_label` (`3F000` and `42P01`), `force option is not supported yet` (`0A000`), and `cannot drop table graph.label because other objects depend on it` (`2BP01`) while a same-kind user label inherits from a default relation or a stored view references the target; after a default is removed, same-kind label creation and Cypher patterns that require it fail safely as `relation "graph._ag_label_kind" does not exist` (`42P01`); `graph_name must not be NULL` / `operation must not be NULL` / `new_value must not be NULL` (`22023`), `invalid operation "name"` (`22023`), `new graph name is invalid` (`22023`), and `schema "name" already exists` (`42P06`) when the new name is taken; `graph_exists(NULL)` is `graph name can not be NULL` (`22023`) |
...
Removing a vertex label can intentionally leave dangling endpoint ids in surviving edge-label rows, exactly as AGE's table-level DDL does. Removing a default label leaves the graph registered but broken for operations that require that kind; operations confined to the surviving kind, catalog inspection, rename, `drop_graph(..., true)`, and a restrictive graph drop after all labels are gone remain available. The last operation intentionally differs from direct `DROP SCHEMA graph RESTRICT`: AGE's `drop_graph(..., false)` removes the remaining graph catalog sequence as part of its lifecycle, while direct schema removal still reports `2BP01` because `_label_id_seq` depends on the namespace. Apache AGE 1.8.0 can terminate its PostgreSQL backend on some accesses to this broken state; UQA Engine preserves the same durable catalog and row state but reports deterministic `42P01` errors instead of crashing the process.

Call `drop_label` to remove a label. A direct `DROP TABLE graph.label`, including `IF EXISTS` or `CASCADE`, is rejected like AGE's object-access hook with `table "label" is for label "label"` (`2BP01`) and changes nothing.

Stored views bind to label-relation identities. `alter_graph(..., 'RENAME', ...)` rewrites those identities durably so the views remain queryable and continue to block `drop_label` under the new graph name; `drop_graph(..., true)` removes the dependent view closure with the graph, matching `DROP SCHEMA ... CASCADE`.
...
| Result | `ag_graph (graphid oid, name name, namespace regnamespace)` has one row per graph and `ag_label (name name, graph oid, id label_id, kind label_kind, relation regclass, seq_name name)` has one row per surviving label by ascending label id; a new graph starts with the two default labels before user labels, while a dropped label and its relation and sequence disappear; `kind` is `v` or `e`, `relation` is the quoted `graph.label` relation name, and `seq_name` is the label's own `<label>_id_seq` sequence (the graph-level `_label_id_seq` allocator appears only in `pg_sequences`) |
