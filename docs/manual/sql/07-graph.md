...
Removing a vertex label can intentionally leave dangling endpoint ids in surviving edge-label rows, exactly as AGE's table-level DDL does. Removing a default label leaves the graph registered but broken for operations that require that kind; operations confined to the surviving kind, catalog inspection, rename, `drop_graph(..., true)`, and a restrictive graph drop after all labels are gone remain available. Apache AGE 1.8.0 can terminate its PostgreSQL backend on some accesses to this broken state; UQA Engine preserves the same durable catalog and row state but reports deterministic `42P01` errors instead of crashing the process.
