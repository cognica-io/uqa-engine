...
Persistent DiskANN indexes also use the database maintenance worker to prune covered or committed-obsolete change records. `Engine::automatic_diskann_maintenance_status()` returns `DiskANNMaintenanceStatus` with completed index passes, confirmed examined/removed entry counts, pending transaction completion and the latest failure. Each page examines at most 64 keys from a fixed discovery snapshot; later writes wait for another pass and older readers retain their history. These counters do not measure graph rebuilding or reclaimed historical bytes. The worker uses independent provider transactions and releases its retained resources when its final Engine client closes. See the [journal maintenance contract](../../design/diskann-journal-pruning.md#automatic-finite-passes).
