...
Sessions created with `Engine::new_session()` share immutable committed definitions, statistics, and graph snapshots while retaining independent statement gates, transactions, and physical storage handles. Reuse these sessions for independent database operations; cloning an `Arc<Engine>` still shares one SQL session and its statement gate.
...
Graph generations are tracked separately from other registries. A non-graph registry change or statistics publication does not reconstruct unchanged graphs. A changed graph is loaded by name through `CatalogFacade::load_named_graph_snapshot()`, validated, and shared as an immutable version across sessions; concurrent readers decode the same live version once. Vertex and edge changes invalidate every graph containing that entity. Mutations copy only the affected graph, and old transaction snapshots retain their previous version. Uncommitted graph snapshots are never published to this shared cache.

Custom providers can implement this optional method with the same transaction-visible contract. Returning `None` retains conservative full refresh; an empty `Some(CatalogCacheRevisions::default())` asserts that nothing has changed and is not a substitute for unsupported tracking. The optional `graphs` map enables graph-scoped tracking; providers that leave it as `None` must include graph changes in the registry generation. The default named-graph loader filters bulk catalog rows; providers can override it with an indexed lookup. Revisions must not be recycled while cached snapshots can still exist, including graph deletion and recreation. SQLite catalog version 45 upgrades the version-44 revision triggers and adds a graph-membership lookup index; older binaries must not reopen the upgraded database.
