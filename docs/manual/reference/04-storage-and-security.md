...
Storage formats may evolve before a stable release, so application and database rollouts should be coordinated. The [0.4.0 upgrade guide](10-upgrading.md) covers Nori features, SQLite provider imports, durable analyzers, and positional-index migration, alongside the tuple metadata and expression-index changes introduced in the 0.2 series.
...
SQLite native and Key/Value MVCC backups use `ManagedConnection::open_restored(path, request, control)`. The encrypted, compressed and encrypted-compressed counterparts are `open_encrypted_restored`, `open_compressed_restored` and `open_compressed_encrypted_restored`, with the same key/compression arguments as their ordinary constructors. `open_compressed_encrypted_restored_with_anchor` additionally enforces the caller’s trusted current container anchor. These methods return a managed connection for the backup’s original record mapping. Obtain the source identity from `SQLiteRecordStore::database_id()` before closing it, persist a fresh `DatabaseRestore` request outside the database, and close all destination owners before copying or restoring. Missing, empty, uninitialized and unrelated files are rejected. Ordinary opens preserve the existing history.

SQLite restoration retains an exclusive physical owner while validating the existing main mapping and auxiliary SSI checkpoint. It durably records a pending target in the main file, publishes a fresh target coordinator in the auxiliary file, then retires old receipts and publishes the target main identity. Each commit uses FULL synchronization; the separate files do not rely on cross-file WAL atomicity. An error after the pending marker may leave a partially completed restore: ordinary opens return `DatabaseRestoreIncomplete`, and only the original source/target request can resume it. A completed retry validates and preserves later records, receipts and SSI state. Existing connections, physical leases, snapshots and participants prevent restoration with `DatabaseRestoreBusy`. Legacy snapshot and participant leases also exclude the transition.

SQLite keeps the native data-key namespace, committed versions, visibility sequence and allocation watermarks unchanged. Old transaction IDs cannot resolve in the new history. A missing auxiliary file starts a fresh checkpoint; a structurally valid newer checkpoint from the same source may be retired explicitly alongside an older closed main backup. Ordinary admission still rejects incompatible terminal receipts. Unrelated or malformed auxiliary state is rejected before publishing the restore intent. Authenticated compressed-container anchors retain their independent freshness contract: after a successful write, an earlier anchor cannot authorize opening the changed container.
