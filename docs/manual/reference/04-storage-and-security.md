...
Storage formats may evolve before a stable release, so application and database rollouts should be coordinated. The [0.3.8 upgrade guide](10-upgrading.md) covers Nori features, SQLite provider imports, durable analyzers, and positional-index migration, alongside the tuple metadata and expression-index changes introduced in the 0.2 series.
