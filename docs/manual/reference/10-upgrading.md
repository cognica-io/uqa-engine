...
This is a lower record-persistence API for the concurrent-storage implementation. Default `Engine::open` does not select or convert to this format; unbound legacy handles cannot operate on an explicitly converted file. The development `ManagedConnection::bind_native_records` entry point uses the same conversion and attaches its connection clones and native document/B-tree stores to common logical transactions; exact/IVF/HNSW vector APIs and catalog metadata/schema/model/analyzer records, views, foreign servers/tables, ordinary table definitions, catalog indexes, column lifecycle, statistics and sequence catalog/value operations also join that session. `Catalog::open` can attach to an already bound native session, including transaction-visible cache-generation reads. Bind before opening the catalog on an already converted file. `Engine::from_persistent_provider` can restore an explicitly bound `SQLiteStorageProvider`, including reopen and sibling publication in all four file modes. Namespace integrity is checked against the same retained logical view. Default model selection, shared-index merging and complete concurrent SQL transaction integration remain pending. Standalone qualified graph table families retain their separate format. Sequence value operations join the chosen session; Engine integration must still preserve autonomous allocation and transactional definition semantics. Document and exact/IVF vector snapshots retain their read boundary across later writes and rollback, and reject mutation. Prepared native batches must include all rows affected by native cascades or graph invalidation. The adapter rejects omitted effects, merges provider-owned cache increments and commits current rows, history and receipts together. See the [storage contract](../internals/03-storage.md) and the remaining [implementation requirements](../../plans/0008-concurrent-storage-transactions.md).

Native conversion/reopen also validates the catalog cache-invalidation triggers. The known metadata triggers that truncated names at embedded NUL characters are replaced atomically with byte-preserving extraction; catalog open applies the same repair to legacy files. The repair preserves data, revision counters and native histories and does not run again after the canonical triggers are installed. Missing or altered native cache tracking is rejected.


Bound native occurrence APIs now join the logical session, including retained snapshots/cursors and persisted block bounds. Native mapping formats 1–3 upgrade atomically to format 4 while preserving original source history and receipts; earlier adapters reject the new marker. Catalog version 49 and existing occurrence row codecs remain unchanged. Successful initial conversion retires imported dynamic accelerator tables, and later source or column changes invalidate the corresponding fixed-family caches. Continue using a closed-file backup when migrating a development or released file.
