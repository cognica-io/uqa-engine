...
Implement virtual and stored generated columns. Store the generation expression and kind, reject direct writes except PostgreSQL-supported `DEFAULT` forms, evaluate virtual columns on reads, maintain stored columns on writes, order dependencies, reject cycles, expose `attgenerated` and information-schema metadata, and preserve definitions across storage reopen and schema changes.
