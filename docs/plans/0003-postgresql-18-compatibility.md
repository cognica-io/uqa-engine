...
| Qualified joins | `JOIN ... USING`, `USING (...) AS alias`, and `NATURAL JOIN` preserve structural AST metadata, bind against both physical row types, implement merged-column ordering and outer-join value selection, preserve input qualification and duplicate non-key output slots, and report PostgreSQL column SQLSTATEs | Add static equality-operator resolution and common-type coercion for differently declared `USING` columns |
