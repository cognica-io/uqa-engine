...
Status: Active implementation in [PR #108](https://github.com/cognica-io/uqa-engine/pull/108), based on UQA 0.3.0. Shared dictionary primitives are implemented in `uqa-analysis::morphology` and used by Nori. The common `cjk_width` character filter is implemented and verified against pinned Lucene text and offsets. The pinned Japanese source rebuild and complete streamed model export are implemented and verified in Docker. Japanese tokenization, dictionary packaging, analyzer integration, and delivery are incomplete. This plan does not certify Japanese tokenizer/analyzer parity or performance. The tagged 0.3.0 release is independent of this PR.
...
| Japanese bundle, loader, and resources | `uqa-analysis::kuromoji`, `uqa-kuromoji-data`; complete model and common codecs | Separate schema and identity; exhaustive reconstructed model equality; malformed/truncated/oversized/hash-invalid inputs rejected before publication; bounded resource caches and complete licenses | Next implementation unit; source/model inputs are verified, while the native bundle, loader and data packaging remain unimplemented |
