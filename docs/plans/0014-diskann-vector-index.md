...
Status: Implementation in progress. Configuration/reference fixtures merged in [PR #157](https://github.com/cognica-io/uqa-engine/pull/157) as `95f278f0`, navigation metrics/PQ in [PR #158](https://github.com/cognica-io/uqa-engine/pull/158) as `4b14fb74`, node/page encodings in [PR #159](https://github.com/cognica-io/uqa-engine/pull/159) as `d9d78aad`, generation metadata/codecs in [PR #160](https://github.com/cognica-io/uqa-engine/pull/160) as `3d96d662`, and controlled readers/cache/sealing in [PR #161](https://github.com/cognica-io/uqa-engine/pull/161) as `2fd47f4c`; their feature branches are removed. Key/Value provider generations are in progress on `feature/diskann-key-value-generations`. The completion ledger remains twelve units: three complete, one in progress, eight pending. Implementation baseline: main `7faeabe3`, inspected on 2026-09-25. Physical foundations do not add SQL support or establish runtime acceptance.
...
| Provider records and generation leases | Page format and controlled readers | Storage Key/Value, SQLite, redb | In progress: explicit encoded-value caps through retained MVCC reads, shared generation records and actual Key/Value provider acceptance. |
...
The first implementation change adds explicit encoded-value caps through the existing Key/Value and committed/private MVCC point-read boundaries. SQLite checks length before materialization, including compacted records; redb and memory validate borrowed data; retained wrappers forward the cap. Five focused tests passed in Linux Docker across common memory sessions and actual SQLite/redb file owners, including unsupported-provider rejection, original parent accounting, exact-size acceptance, private rollback retention and cancellation. Generation namespaces, persistent reader binding and complete provider-unit acceptance remain in progress.
