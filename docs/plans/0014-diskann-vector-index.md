...
Status: Implementation in progress. Configuration/reference fixtures merged in [PR #157](https://github.com/cognica-io/uqa-engine/pull/157) as `95f278f0`, and navigation metrics/PQ merged in [PR #158](https://github.com/cognica-io/uqa-engine/pull/158) as `4b14fb74`; both feature branches are removed. Page-format work begins with node/page encodings on `feature/diskann-node-pages`. The completion ledger remains twelve units: two complete, one in progress, nine pending. Implementation baseline: main `7faeabe3`, inspected on 2026-09-25. Numerical primitives and codecs do not add SQL support or establish runtime acceptance.
...
| Page format and controlled readers | Configuration and independent fixtures | Storage | In progress: node/page encodings have passed 549 Storage library tests, strict Clippy and independent Python 3.8 byte fixtures; review/merge pending. Manifest/PQ codecs and controlled readers/cache remain pending, so the unit is not complete. |
