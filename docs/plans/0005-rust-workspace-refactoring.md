...
Current implementation position: Phase 0 through Phase 3 are complete. The checked-in transition ratchet now inventories 58 files at or above 1,000 physical lines, immutable statement catalog snapshots and four narrow capabilities govern the read path, catalog projection, schema binding, evaluation scopes, and physical construction have responsibility-owned modules, every DML command and action family uses one typed mutation protocol, the facade roots and sole dispatcher remain reduced without adding an execution path, and Phase 4 transaction-and-lock migration is next.
...
The three broad lint boundaries also remain. Phase 0 temporarily removed each root `clippy::too_many_lines` allowance and ran the owning library with `-D warnings`: `uqa-engine/src/sql.rs` exposed 115 existing long-function diagnostics, `uqa-execution/src/lib.rs` exposed 16, and `uqa-sql/src/lib.rs` exposed 40. Phase 1 reduced `sql.rs` from 1,115 to 337 physical lines and Phase 2 reduced it to 334. Phase 3 removed three DML roots from the transition inventory and lowered four other DML baselines, but the SQL descendant-wide allowance still covers long functions in later transaction, lock, DDL, query, and command-policy owners; it and the two lower-crate allowances remain explicit Phase 6 completion debt, and none has been replaced by unreviewed local waivers.
...
### 3.5 Phase status at the current implementation

| Phase | Status | Current evidence |
| --- | --- | --- |
| Phase 0: structural ratchet | Complete | The exact transition ratchet inventories 58 current files, rejects growth and new 1,000-line files, all six inherited 1,500-line violations are responsibility-split, the root lint debt is explicitly counted, and the fixed runner has clean and warm timing baselines. |
...
The implementation sequence now moves to Phase 4. The completed mutation bundle leaves CI topology unchanged: the harness checker still reports 218 direct integration sources in 18 targets across 18 crates, `UnifiedPlanExecutor` remains the only dispatcher, and the full `uqa-engine` integration target plus focused catalog, binding, physical-construction, scalar-traversal, DML protocol, codec, INSERT/ON CONFLICT, UPDATE/DELETE, referential-action, partition-movement, MERGE, trigger, rule, and view tests pass.

## 4. Findings and root problems

### 4.1 The composition root is also an implementation container

`uqa-engine` is correctly the only crate that can compose storage, planner, execution, graph, scoring, model, FDW, session, transaction, and extension behavior. That does not require every algorithm to accept `&Engine`. The selected catalog, session, runtime, schema-mutation, and DML protocol paths now expose their data through narrow capability values and typed carriers; command workflows that genuinely coordinate several engine-owned domains are declared adapters, while engine-free protocol and codec leaves fail the policy check on any facade access. The remaining transaction, locking, DDL, query, and lower-crate method surface still lets many modules acquire unrelated capabilities transitively, so later domain bundles must continue the migration.
...
`uqa-engine/src/sql.rs` is now 334 physical lines after its tests and cursor-worker implementation moved to responsibility-owned modules, so it is no longer an oversized implementation container. It still grants a broad lint exception to descendants, and of 232 SQL files, 203 import their parent module while eight use a parent glob import; the transaction, locking, remaining SQL-owner, and final-ceiling bundles must reduce that ambient namespace without manipulating the count as a proxy for ownership.
...
- `sql/dml/constraints.rs` remains 1,466 lines because immediate checks, key policy, deferred constraints, period behavior, and staging are still substantial orchestration; referential delete actions moved to a 446-line owner. `insert.rs`, `merge.rs`, `conflict.rs`, and `view_triggers.rs` remain transition-inventory entries with lower baselines, so later ceiling work must split their remaining policy without recreating lifecycle paths.
- `engine_transactions.rs` and `row_locks.rs` each contain several independent state machines whose ordering must remain explicit but whose implementation does not need to occupy one file; these are the next coherent ownership bundle.
...
- Maintain the checked-in machine-readable inventory that began with 69 non-imported Rust files at or above 1,000 physical lines and now contains 61, recording each audited size, owner, responsibility groups, target modules, and migration state.
