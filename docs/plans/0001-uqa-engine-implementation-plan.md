...
- **Operator rewrites.** Filter pushdown, facet additivity, join distribution, and aggregation decomposition require equivalence at the parent's observed carrier. Vector-threshold leaves remain distinct because posting intersection adds their scores; a maximum-threshold leaf preserves only support for identical query vectors with valid thresholds.
