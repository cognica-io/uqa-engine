...
Status: Implementation in progress. Common storage now has committed record histories, immutable prepared changes, conditional revision validation, private mutation journals with retained command views/savepoints and an in-memory reference owner with retained snapshots. Persistent provider/session integration has not started, and existing Engine SQL writes remain serialized. Baseline: UQA 0.3.6, commit `24e464f7709cd5d942df8d0aeef5d522716b1bfb`, inspected on 2026-09-16. The [design](../design/concurrent-storage-transactions.md) defines the intended architecture; this plan distinguishes implemented building blocks from the complete public capability.
...
The implemented [`mvcc`](../../crates/uqa-storage/src/mvcc/mod.rs) module owns record histories and private changes. `PreparedRecordCommit` owns evaluated bytes before commit admission and validates every original revision, including tombstones. `MemoryVersionStore` publishes complete prepared histories atomically and registers snapshots under the same gate as publication and reclamation. `PrivateRecordChanges` stages atomic batches, retains fixed private command views across undo and later writes, restores nested/repeated savepoints without allocating, and prepares final replacements without copying payloads. Intermediate private history stays in its charged journal until the owner and retained views are dropped; spill and intermediate reclamation remain open. Tests include 25 focused cases, with generated savepoint histories compared against an independent copied-map model, plus the existing owner suite (191 unit, 60 integration and one documentation test). This remains a storage building block: Engine SQL command/savepoint integration, serializable isolation and provider durability are not established.
