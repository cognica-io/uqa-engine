...
The common storage library implements the binary term keys, occurrence values, field staging, and version 2 codecs described here. Memory, Key/Value, and native SQLite providers store exact term keys and complete occurrence graphs with revision and original source-end metadata. Persistent initial Engine open rebuilds old positional formats from original sources under restored analyzer descriptors. The [Nori plan](../plans/0006-nori-analyzer.md) tracks graph query consumers and complete Nori retrieval.
...
Schema 48 keeps occurrence data separate from legacy positional tables. `_occurrence_clusters` stores one score/graph pair per `(table_name, field, term BLOB, cluster_id)` and a checked redundant posting count. `_occurrence_documents` stores version 2 reverse keys and the 84-byte field metadata value per document/field; `_occurrence_lengths` stores its checked normalization length. `_occurrence_fields` stores the 40-byte revision with document count and total length, including fields whose documents emitted no tokens. SQLite document IDs and numeric SQL columns retain checked INTEGER bounds; binary metadata retains the common unsigned representation.

`_occurrence_formats` records `source-rebuild` for catalog migrations and `occurrences-v2` only when a complete source rebuild or native write commits. Existing legacy rows require reconstruction even when the analyzer catalog is already current. Missing or unknown graph format markers are errors. Catalog schema preparation and Engine source reconstruction share the initial restore transaction; standalone catalog preparation preserves the source-rebuild obligation. Source staging never fabricates offsets or graph edges from version 1 positions.

Point and batch replacement stage exact compiled analysis, coalesce changes to each cluster, and publish reverse terms, metadata, lengths, counts, and revisions in one savepoint. Rebuild retires legacy rows in that same transaction. Column/table rename and deletion own all corresponding occurrence rows; table format markers follow table identity. Mutations invalidate persisted skip offsets and score bounds. Score cursors read version 2 score payloads without decoding graph edges, while graph readers validate original-source metadata and span bounds.
