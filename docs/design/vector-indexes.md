...
HNSW follows the layered navigable-small-world construction described by Malkov and Yashunin: deterministic seeded level selection, greedy descent through upper layers, bounded `ef_construction` search at insertion, heuristic neighbor selection, reciprocal edges, and degree pruning. Layer zero permits `2m` neighbors and upper layers permit `m`; graph validation checks these bounds, edge reciprocity, node levels, entry-point metadata, counters, and references before a restored graph is published.
