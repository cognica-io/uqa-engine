...
[Physical provider measurements](../../benchmarks/nori/PERSISTENT.md) now cover native SQLite/redb and WASM SQLite, with provider-owned benchmark entrypoints and no new runtime dependency. Each of four commit/rollback workloads verifies all nine live and reopened results against Memory and the existing reviewed graph hashes. Seven complete reports and an allocation-only reference record allocation maxima, timing samples, compiler and source identities, actual SQLite settings, and file lengths. Provider/target ceilings and same-environment timing comparisons are enforced by the existing native/WASM jobs. Rust allocation figures exclude SQLite C allocations, and Emscripten's virtual filesystem does not measure host durability. SQL/session transaction overhead, phrase-query cost, browser host memory, and complete binding/release acceptance remain separate work.
