...
[Physical provider measurements](../../benchmarks/nori/PERSISTENT.md) now cover native SQLite/redb and WASM SQLite, with provider-owned benchmark entrypoints and no new runtime dependency. Each of four commit/rollback workloads verifies all nine live and reopened results against Memory and the existing reviewed graph hashes. Eleven complete reports and an allocation-only reference record allocation maxima, timing samples, compiler and source identities, actual SQLite settings, and file lengths. Repeated Linux x86_64 provider runs complement the macOS aarch64 and WASM evidence; allocation limits identify the exact target OS, architecture, and pointer width, preserving each earlier ceiling. Provider/target ceilings and same-environment timing comparisons are enforced by the existing native/WASM jobs. Rust allocation figures exclude SQLite C allocations, and Emscripten's virtual filesystem does not measure host durability. SQL/session transaction overhead, phrase-query cost, browser host memory, and complete binding/release acceptance remain separate work.

[Native graph phrase measurements](../../benchmarks/nori/PHRASES.md) now cover six corpora in all three decompound modes, with separate pre-analyzed matching and complete analysis/matching scopes. The operator-owned benchmark validates every scored document across nine executions per workload and shares one allowance across query analysis and matching. Four native/WASM reports reproduce all six allocation counters for all 36 workloads and agree exactly on analyzer identities, graph sizes, document support, and scores. Reviewed pointer-width allocation ceilings and explicit same-environment timing comparisons are enforced in native/WASM CI. SQL/session costs, physical-provider phrase I/O, browser host memory, and complete binding/release acceptance remain separate work.
