...
Nori uses the existing crate boundaries and provider interfaces. The original term-only pipeline and whitespace phrase lowering have been replaced by the rich contracts below; the [implementation plan](../plans/0006-nori-analyzer.md) records the corresponding tests and required acceptance gates.
...
The existing generic stages support `Analyzer::compile()` and `compile_with_resources(&AnalyzerResources)` through immutable `CompiledAnalyzer` handles. `AnalyzerDescriptor` persists explicit resolved generic inputs, compatibility revisions, Unicode/regex profiles, and declared length policy under a canonical fingerprint. `AnalyzerResources` interns those revisions with bounded ownership and restores descriptor JSON without reopening synonym files. `AnalyzerResources::with_nori_resources` resolves Korean tokenizer/filter configurations with exact artifact identities and preserves the tokenizer profile for separate normalization. Memory, Key/Value, and native SQLite indexes store immutable graph revisions and original stream-end metadata through the rich pipeline. Persistent providers rebuild incompatible positional formats from original source text under exact restored descriptors. Prepared and uncompiled calls share the same token/source algorithms, and uncompiled synonym stages continue to reload files.

Memory, Key/Value, and SQLite providers now retain compiled handles through shared `AnalyzerBindings`; the default resolves on first successful use, while explicit field assignments resolve before publication. Index and search sides remain independent during the current index lifetime. Query consumers and detached portal indexes retain those exact handles. Provider revision rebuilds and the Engine field-assignment path publish replacement postings with the candidate binding only after success. Exact descriptors and independent phases survive catalog restoration; registry refresh installs committed bindings before reopening changed physical stores.
...
The tokenizer operates in filtered-input coordinates. Convert its coordinates through the composed maps exactly once to produce original-source offsets. Maintain an explicit UTF-16 boundary to UTF-8 byte-boundary map; a Hangul syllable usually consumes one UTF-16 unit and three UTF-8 bytes, while a supplementary character consumes two UTF-16 units and four bytes. Token filters retain source spans when rewriting text, and composed tokens cover their contributing spans. A common stream retains shared original-text and edit-map provenance after the input borrow ends. Composition combines the original filtered ranges and projects that new range once; combining previously corrected boundaries would mis-handle empty spans at deletions.

The Rust input domain is valid UTF-8. Preserve exact UTF-16 coordinates in diagnostics even for a reference corner case that cannot be projected to a Unicode scalar boundary. Strict scalar-boundary projection returns a typed error for a split pair. The explicitly named covering projection retains the exact original UTF-16 range while exposing the smallest safe UTF-8 scalar range; it maps UTF-16 coordinates through every edit before computing the final UTF-8 cover. Highlighting consumes that cover without changing the diagnostic coordinates. Java strings containing isolated surrogates have no direct `&str` input equivalent and must be rejected at a binding's string-conversion boundary.
...
The native tokenizer now reserves input, lattice capacities, pending/output buffers, readings, and morphemes through the shared `uqa-core::memory` allowance before allocation. Buffer replacement charges both allocations, pruning preserves reservations for retained capacity, and the budgeted result carries its output reservation across the return boundary. Cancellation callbacks also cover attribute encoding, candidate expansion, backtrace, pruning, and output transfer. Budgeted character-stage entry points now stream replacements into reserved text/maps, reserve scalar indexes and capture slots, and retain leases across shared source views and Korean token contexts. Literal mappings, built-in Standard/Letter word scans, the HTML `<[^>]+>` scan, and every prepared regex range/capture search traverse their input under callback control. Analysis uses the existing dependency’s public Thompson states and Unicode assertions for capture resolution and patterns without a bounded DFA. Runtime frontiers, capture rows, and the explicit traversal stack reserve before allocation; capture offsets have an owned representation independent of private dependency layouts. Immutable automata remain preparation resources. The common tokenizer bridge now reserves generic token and gram buffers, transfers native morphology leases, counts both UTF-16 and UTF-8 during canonical conversion, and reserves retained source copies and projection payloads. Failed calls do not publish coordinate caches into a borrowed source view. Native/common Korean filters and public analyzer entry points now retain one caller allowance through the complete configured pipeline. Native phrase analysis, occurrence cursors, SQL, and highlighting propagate caller runtime control through their owning crates. The implementation plan records completed configured-stage parity and the required benchmark and delivery gates.

Word transformation helpers also use the shared allowance. Porter stemming retains character and consonant buffers, classifies repeated `y` without recursive prefix walks, polls bounded scans, and reserves scalar or lossless output before releasing scratch. ASCII folding uses public single-scalar compatibility decomposition to emit directly into a reserved term buffer. It preserves the per-character fallback when no ASCII decomposition exists and carries isolated UTF-16 units unchanged. These helpers remove per-suffix, per-character, and raw-segment temporary allocations; common filters retain these leases across Korean stage composition and public analyzer execution.

Full lowercase now uses reserved scalar/raw output and two borrowed forward traversals for original-input sigma context. Prepared Unicode property ranges require no per-call regex workspace; isolated surrogate units delimit context without intermediate scalar strings. Traversal and encoding remain cancellable, and failure drops partial output reservations. Complete scalar/context differentials match Rust lowercasing, while canonical descriptors retain the existing Unicode 16 identity only when both context-class hashes match those verified tables. Other table/version combinations carry explicit class hashes. Common filters now retain batch, term, morphology, and terminal leases through replacement, removal, and expansion. Public budgeted token-filter and analyzer methods consume or return reserved analyzed results and propagate the caller allowance through common and Korean stages. Immutable preparation remains separately owned; configured runtime searches share the caller allowance and cancellation callback. Prepared stop/synonym comparisons and term-length scans poll cancellation. Expansion reserves its complete output vector and transfers unchanged tokens, while removal frees discarded attributes after destruction and preserves upstream terminal precedence and zero-increment trailing removals.

Terms, tokens and complete analyzed results now have reserved copy entry points. Morphology copies include readings, morpheme vectors and lossless surfaces, and complete results retain hidden terminal attributes and final gaps. Copies own their new buffers and share existing source projection/map leases. Synonym expansion avoids copying a term that will immediately be replaced. Gram expansion counts characters and reserves its complete coordinate table before filling it, then preserves exact source spans without repeated prefix scans or replacement-buffer peaks. Limit and cancellation failures release partial copies while preserving borrowed input and independent allocation owners. Generic and Korean filters connect these helpers to retained batch input/output/removal ownership. Numeric composition retains saved lookahead tokens and intermediate numeral buffers, and exact decimal coefficients use reserved deques with retained capacity. Scalar/raw normalization retains encoding and output reservations through replacement. Native and common analyzer entry points propagate the caller budget and callback across every configured stage.
...
The [CSV regeneration tool](../../tests/parity/nori/MODEL.md#csv-source-regeneration) now rebuilds all nine release dictionary resources from the checksum-pinned MeCab source through the public builder in Docker. Two arm64 runs and one amd64 run reproduce every resource byte and hash with normalization disabled. The reviewed manifest covers all 43 CSV/definition inputs, builder settings, JVM identity, and output files. Failures publish neither generated resources nor new expected provenance. A dedicated pre-merge job reproduces the CSV resources, full neutral model, and reference corpora; this gate does not change the production bundle.

The implemented [neutral exporter](../../tests/parity/nori/MODEL.md) now covers the complete jar model and pinned Java Unicode profile. It checks every enumerated surface against runtime FST lookup, every ordered word mapping, all morphology fields, the full matrix, and character definitions, then reopens its output for exhaustive comparison. Independent arm64 and amd64 Docker runs reproduce the five hashes in its [manifest](../../tests/parity/nori/model_manifest.json). The neutral files total 82,581,342 bytes. The implemented [Rust bundle codec](nori-bundle-format.md) packs this model into 9,829,534 bytes and reconstructs every neutral hash after loading. Native tokenizer/filter parity, common pipeline compilation, persisted graph retrieval, and the complete enabled/disabled persistent binding contracts are verified in the implementation plan. The benchmark documents distinguish passing analysis, indexing, phrase, and cancellation gates from the separate SQL timing acceptance.
...
Expose a provider-neutral analysis resource resolver accepting immutable bytes by content identity. The native `NoriResources` owner implements explicit name or SHA-256 requests, validates resolver-supplied artifact identities, and interns dictionary handles and exact user-rule snapshots under count and encoded/source-byte budgets. Alias changes resolve new content; installed handles and source revisions remain immutable after cache eviction. The default `nori` feature uses the bundled data crate. Native hosts may explicitly register another validated bundle; browser hosts may supply bytes before opening a catalog that requires them. The resolver has no implicit filesystem or network fallback. Custom system models use the same validated bundle format and carry their own identity and compatibility evidence.
...
Port `KoreanNumberFilter` as an optional compositional filter, including keyword protection, stacked-token handling, offsets, and decimal arithmetic. Use exact decimal representation with reference-compatible scale and formatting, never binary floating-point. Preserve decimal/grouping punctuation until number composition; removing only `SP` before composition follows the pinned upstream number-test chain. The executed `numbers` case turns `３．２천` into `3200` when punctuation is retained; the same reference chain with punctuation discarded produces `32000`. The `number_comma` case turns `15,7` into `157`. These demonstrate why the default analyzer excludes number normalization and why examples must name the complete chain. See the pinned [number filter](https://github.com/apache/lucene/blob/64ce863a2bea79c69c19c4d56268c26710ff0ff9/lucene/analysis/nori/src/java/org/apache/lucene/analysis/ko/KoreanNumberFilter.java).

Filter order also determines inherited metadata. Number composition changes the term and offsets while retaining attributes observed after lookahead, including a nonnumeric next token or attributes changed during upstream exhaustion. A subsequent POS filter can remove a number that inherited a space tag, and subsequent reading conversion can replace it with the next word's reading. Documented number recipes retain tokenizer punctuation, remove only `SP`, and then apply `nori_number`; complete configurations and all token/end attributes are verified in the expanded number corpus. Preserve keyword checks, stacked fallthrough, aborted prefixes, and terminal-attribute effects for every other accepted chain. The standalone direct helper separately follows numeric-prefix parsing and malformed-decimal fallback; cancellation and resource errors never become verbatim success.
...
Term support, exhaustive and WAND scoring, persisted block bounds, corpus calibration, typed multi-field search, and attention query features now use lossless query identities. Scalar entry points remain available; internal term arrays use canonical keys or explicit UTF-16 values. Regression tests cover repeated raw keys and replacement characters separately. WAND cursor exhaustion no longer reserves a document-ID value. Physical scoring, block-bound validation and rebuilding, result ordering, and calibration-query sampling belong to `uqa-scoring`; all-field costing belongs to `uqa-planner`. Engine supplies retained resources and transaction boundaries through those native APIs.

Quoted `fts_match` phrases now retain a typed expression through SQL lowering and optimizer rewrites. Storage projects the whole search analysis into ordered exact-term occurrences; `uqa-operators` intersects first-edge and terminal-edge cursor support, then traverses memoized query/document states with position lengths and internal holes. `uqa-scoring` scores only accepted candidates and preserves emitted-term multiplicity. Execution retains the query snapshot and field descriptor, forwards cancellation and `work_mem`, and carries the analyzed phrase length into complete Boolean-query calibration. Engine supplies retained index and runtime adapters. Memory candidate cursors borrow the retained posting maps instead of copying complete term support.

Query analysis and key projection now return unique allocation guards, and native phrase matching uses the same core allowance for graph buffers, IDFs, scalar statistics, occurrence results, memoized states, field accumulation, and final postings. Cancellable in-place ordering is shared with highlighting through core; candidate scoring preserves the native scalar sum and calibration without a temporary score vector. Prepared SQL reads current session limits even when complete analysis removes all tokens. Producer-side provider cursor payloads, cluster reads, and occurrence decode scratch now retain their own reservations. The existing SQL integration harness verifies phrase allowance failures and cancellation recovery across Memory, SQLite, and redb in addition to the owner-level cursor tests.

The graph matcher is also checked against an independent recursive path reference over 2,000 generated query/document graphs, including repeated terms, alternative edge lengths, empty graphs, and internal holes. These operator-owned differentials complement the provider and SQL integration tests above.

### Source highlighting

The explicit-analyzer highlighter analyzes the whole source and each complete query input with the selected revision, then selects original-source spans for matching lossless terms. SQL owns argument validation, execution retains the named revision, and the analysis crate owns matching and presentation. The Engine adapter lends the existing compiled resource and runtime view. A NULL or omitted SQL analyzer name retains the English word-scanning contract. Merge overlapping spans only at presentation time. Reading conversion highlights the Hanja source; inflection decomposition highlights the shared original inflection; user-defined punctuation-bearing nouns such as `c++` retain their exact span. Character-filter maps preserve the original string in HTML and replacement examples. Scalar-covering UTF-8 spans retain the exact UTF-16 reference coordinates for split surrogate terms, and fragment windows keep a complete selected match. The implementation does not rescan isolated words on this explicit-analyzer path. Budgeted highlighters now retain one allowance through complete analysis, moved query-term ownership, cancellable lookup and span merging, reserved fragment selection, and direct output encoding. Forward cursors avoid a full-source renderer coordinate table and separate fragment copies while preserving density/focus ties and snapping. The legacy Unicode word scanner polls inside long words and retains its pinned character class and uncompiled resource reload behavior. SQL reads live session `work_mem` and cancellation after argument short circuits and maps failures to `53200` or `57014`. Configured pattern searches and capture resolution share the runtime allowance and poll during execution; immutable preparation resources remain separately owned.
...
Memory indexing now has [separate native/WASM measurements](../../benchmarks/nori/INDEXING.md). The measured whole-index copy in atomic batches was replaced by affected-document staging inside `uqa-storage`, reusing ordered point-replacement validation. Before/after complete graph and metadata hashes agree; repeated allocation counters define native and WASM ceilings enforced alongside the analysis gates.

[Physical provider measurements](../../benchmarks/nori/PERSISTENT.md) now cover native SQLite/redb and WASM SQLite, with provider-owned benchmark entrypoints and no new runtime dependency. Each of four commit/rollback workloads verifies all nine live and reopened results against Memory and the existing reviewed graph hashes. Twelve complete reports and an allocation-only reference record allocation maxima, timing samples, compiler and source identities, actual SQLite settings, and file lengths. Repeated Linux x86_64 provider runs complement the macOS aarch64 and WASM evidence; allocation limits identify the exact target OS, architecture, and pointer width, preserving each earlier ceiling. Provider/target ceilings and same-environment timing comparisons are enforced by the existing native/WASM jobs. Rust allocation figures exclude SQLite C allocations, and Emscripten's virtual filesystem does not measure host durability. SQL/session transaction overhead, physical-provider phrase-query cost, and release acceptance remain separate work; actual browser memory is recorded independently.

[Native graph phrase measurements](../../benchmarks/nori/PHRASES.md) now cover six corpora in all three decompound modes, with separate pre-analyzed matching and complete analysis/matching scopes. The operator-owned benchmark validates every scored document across nine executions per workload and shares one allowance across query analysis and matching. Four native/WASM reports reproduce all six allocation counters for all 36 workloads and agree exactly on analyzer identities, graph sizes, document support, and scores. Reviewed pointer-width allocation ceilings and explicit same-environment timing comparisons are enforced in native/WASM CI. The [public SQL/session benchmark](../../benchmarks/nori/SQL.md) adds complete application-path and physical-provider query measurements; its repeated candidates remain under calibration. Release acceptance remains separate work.
...
[Package acceptance evidence](../../benchmarks/nori/DELIVERY.md) records the corrected full Python and JavaScript matrices, every binding archive, Rust/source archives, feature/dependency metadata checks, and actual installed-package execution at their pinned source checkpoints. The inventory distinguishes compiled tests from packaging-only checks and does not certify later source changes. Public SQL/session gates and final remote-head CI remain acceptance requirements.
...
The implemented portable bundle uses compact UTF-16 transducer and metadata sections with per-section zlib compression, selected after complete model reconstruction and artifact-size comparison. Native and WASM share the validated decoded model representation; measured decode/cache limits are recorded with the analysis baselines. Future compression, lookup, mapping, or threshold changes require measured evidence and unchanged differential results, preserving complete morphology and graph consumers.

The physical-provider calibration also covers both redb table-update orders. A provider-owned allocation probe isolates the three-byte cumulative difference caused by copying differently sized table definitions; existing/new-table and identical-type controls verify the cause. Every native target must record both outcomes for every commit workload, while all other counters and rollback remain stable. The [persistent evidence](../../benchmarks/nori/PERSISTENT.md) pins the source experiment and the original failed CI artifact; only the missing observed Linux maximum changes, with no runtime or dependency changes.
