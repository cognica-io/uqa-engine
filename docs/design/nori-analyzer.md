...
Graph state, per-candidate occurrence buffers, and result accumulation use the normal query memory allowance and cancellation token; exceeding that allowance fails without partial output. The analyzer exposes a complete configured-stage budgeted entry point, but query callers and provider-internal cursor allocations still require integration before the end-to-end bounded execution gate is complete. The remaining differential graph cases also remain open.
