...
The existing generic stages support `Analyzer::compile()` and `compile_with_resources(&AnalyzerResources)` through immutable `CompiledAnalyzer` handles. `AnalyzerDescriptor` persists explicit resolved generic inputs, compatibility revisions, Unicode/regex profiles, and declared length policy under a canonical fingerprint. `AnalyzerResources` interns those revisions with bounded ownership and restores descriptor JSON without reopening synonym files. `AnalyzerResources::with_nori_resources` resolves Korean tokenizer/filter configurations with exact artifact identities and preserves the tokenizer profile for separate normalization. Memory indexes now store immutable graph revisions and original stream-end metadata through the rich pipeline. Key/Value and SQLite reject those pipelines until their representation supports the same contract and atomic source migrations. Prepared and uncompiled calls share the same token/source algorithms, and uncompiled synonym stages continue to reload files.

Memory, Key/Value, and SQLite providers now retain compiled handles through shared `AnalyzerBindings`; the default resolves on first successful use, while explicit field assignments resolve before publication. Index and search sides remain independent during the current index lifetime. Query consumers and detached portal indexes retain those exact handles. Provider revision rebuilds and the Engine field-assignment path publish replacement postings with the candidate binding only after success. Persisting exact descriptors and independent phases across catalog restoration remains required.
