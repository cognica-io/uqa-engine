...
The existing generic stages support `Analyzer::compile()` and `compile_with_resources(&AnalyzerResources)` through immutable `CompiledAnalyzer` handles. `AnalyzerDescriptor` persists explicit resolved generic inputs, compatibility revisions, Unicode/regex profiles, and declared length policy under a canonical fingerprint. `AnalyzerResources` interns those revisions with bounded ownership and restores descriptor JSON without reopening synonym files. `AnalyzerResources::with_nori_resources` resolves Korean tokenizer/filter configurations with exact artifact identities and preserves the tokenizer profile for separate normalization. Current linear indexes reject those pipelines until their representation supports immutable graph revisions. Prepared and uncompiled calls share the same token/source algorithms, and uncompiled synonym stages continue to reload files.
