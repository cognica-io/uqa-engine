...
Graph state, per-candidate occurrence buffers, and result accumulation use the normal query memory allowance and cancellation token; exceeding that allowance fails without partial output. Complete analyzer-stage and provider-internal cursor allocation accounting still requires integration before the end-to-end bounded execution gate is complete. Source highlighting and the remaining differential graph cases also remain open.
