...
The existing generic stages now support standalone `Analyzer::compile()` snapshots through `CompiledAnalyzer`; explicit resource resolution and canonical descriptors extend that entry point as Nori compilation is integrated. Prepared and uncompiled calls share the same token/source algorithms, and uncompiled synonym stages continue to reload files.
