...
The explicit-analyzer highlighter analyzes the whole source and each complete query input with the selected revision, then selects original-source spans for matching lossless terms. SQL owns argument validation, execution retains the named revision, and the analysis crate owns matching and presentation. The Engine adapter only returns the existing compiled resource. A NULL or omitted SQL analyzer name retains the English word-scanning contract. Merge overlapping spans only at presentation time. Reading conversion highlights the Hanja source; inflection decomposition highlights the shared original inflection; user-defined punctuation-bearing nouns such as `c++` retain their exact span. Character-filter maps preserve the original string in HTML and replacement examples. Scalar-covering UTF-8 spans retain the exact UTF-16 reference coordinates for split surrogate terms, and fragment windows keep a complete selected match. The implementation does not rescan isolated words on this explicit-analyzer path. The complete configured analyzer exposes a budgeted entry point; highlighter caller propagation and rendering cancellation/allocation accounting remain part of the runtime resource gate.
