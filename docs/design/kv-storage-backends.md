...
`SQLiteKeyValueStorage::open_with_options` and `from_connection_with_options` accept `VersionedSessionOptions { retained_bytes }`, with the same default 64 MiB allowance and no spill as redb. Sibling sessions inherit the configured limit. `ManagedConnection::with` and `with_mut` reject a bound logical session; `with_physical` provides explicit diagnostics or physical maintenance outside that session's transaction and does not expose private records. Record-table write guards remain active. See the [unreleased upgrade contract](../manual/reference/10-upgrading.md#040-sqlite-keyvalue-record-format) for the Rust API and one-way file-format boundary. Full concurrent Engine SQL, shared-index merges and provider history reclamation remain unfinished.
