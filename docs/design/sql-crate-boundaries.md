...
SQL owns CREATE TABLE inheritance and CHECK merging, CREATE TABLE AS column declarations, index-key analysis, index naming, and static unique-index restrictions. Execution validates visible index keys with its bounded `ExactRowSet`, using catalog, row-reading, expression, and memory services. Engine no longer constructs a separate SQLite database for that check.
