...
### Building an admitted partition

Storage's `VamanaGraph::build` accepts an already admitted, strictly increasing sequence of `(DocId, ordinal, NavigationVector)` inputs. It validates every identity and dimension, assigns dense local IDs in that order, and returns adjacency with the original logical keys. Its owned key directory, flat edge slots, lengths, frontier, complete visited set, pruning pools and entry work buffers retain the supplied memory allowance. Borrowed navigation vectors retain their existing owner. A failed or cancelled build publishes no partial graph. This primitive requires its admitted partition to fit; streaming global partitioning and external merging remain separate build work.

Algorithm revision 1 initializes each node's `min(max_degree, n - 1)` neighbors with Floyd sampling without replacement from the ordered `n - 1` offsets excluding itself. A SplitMix64 stream starts at `seed`; sources run in ascending node-ID order, each sample uses unbiased bounded draws, and selected offsets are sorted before mapping around the source ID. The visit permutation starts at ascending IDs and uses forward Fisher–Yates swaps with a separate stream seeded by `seed XOR 0x9e3779b97f4a7c15`. Both refinement passes reuse that permutation. Initial edge generation does not rebuild an `n`-element selection pool for every source.

The entry sample uses Floyd sampling of `min(n, 256)` IDs with a stream seeded by `seed XOR 0x94d049bb133111eb`, then sorts those IDs. Compute each centroid coordinate in ascending sample-ID order and choose the closest vector from the entire admitted partition, with the lowest node ID on an exact distance tie. This fixes the sample-centroid approximation and its work order; it does not compute the exact minimum-sum-distance medoid. Empty input has no entry, and a singleton has entry 0 with no edges. The final connectivity pass excludes the stable successor from other candidates, prunes those candidates to the remaining degree, adds the successor once, and stores neighbors in node-ID order.

The independent rational fixture predates this graph implementation and fixes both unaugmented passes and the separate connectivity result. `initialization.json` additionally pins declared seed streams, initial edges, permutation, sample identities and capped-centroid entry selection; its Python generator reads no Rust results. Construction uses full normalized vectors and squared Euclidean distance throughout. Neither the bounded frontier nor the full visited set is a posting list or ranked result.
...
A union of overlapping partition edges can exceed the final degree bound. The UQA merge explicitly performs a final bounded pruning pass before adding its reserved connectivity edge; it must not truncate IDs or assume union preserves degree. This differs from an unpruned edge union and therefore has its own recall acceptance. Partition overlap and local-to-global mappings are deterministic. Persist effective partition settings and ordered assignment/leaf digests as compact build provenance; canonical node records retain durable global-to-logical identities. The partition owner returns that typed summary, and the final sealing owner must encode it with the generation.
...
Two transactions changing different documents write distinct logical records and may commit in either order. Exact byte/count maintenance folds the already evaluated immutable mutation records on one retained canonical/journal snapshot; it adds no shared counter or whole-index expected-version condition to document writes. The [maintenance capture contract](diskann-maintenance-capture.md) defines its additive document/vector/raw-byte accounting and consumes that same capture for reconstruction. There is no transaction-lifetime DiskANN writer permit. Same-document conflicts, READ COMMITTED target rechecks, REPEATABLE READ snapshots, savepoints, statement failure, and SERIALIZABLE participation follow common MVCC.
