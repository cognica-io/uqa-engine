...
Status: Proposed. This design was checked against UQA 0.3.6 at `24e464f7709cd5d942df8d0aeef5d522716b1bfb` on 2026-09-16. It does not describe an implemented concurrent-writer capability. The [implementation plan](../plans/0008-concurrent-storage-transactions.md) tracks the work and acceptance evidence; the [storage manual](../manual/internals/03-storage.md) remains the contract for released behavior.
...
| SQLite writer begins with `BEGIN IMMEDIATE` | [`connection.rs`](../../crates/uqa-storage-sqlite/src/connection.rs), `begin_transaction` | A physical writer survives until outer commit/rollback. Ordinary read-first transactions can defer this acquisition. |
| redb session retains `WriteState.transaction` | [`store.rs`](../../crates/uqa-storage-redb/src/store.rs) and [`transaction.rs`](../../crates/uqa-storage-redb/src/transaction.rs) | redb's native MVCC readers do not provide concurrent SQL writers. |
...
| SQLite K/V and redb | [`SQLite key_value.rs`](../../crates/uqa-storage-sqlite/src/key_value.rs), [`redb store.rs`](../../crates/uqa-storage-redb/src/store.rs), [`redb transaction.rs`](../../crates/uqa-storage-redb/src/transaction.rs) | Replace session-long physical transactions and savepoint undo with common logical sessions and short durable operations. |
...
| Sequences and identifiers | [`sequence lifecycle`](../../crates/uqa-execution/src/schema/sequences/lifecycle.rs), [`sequence adapter`](../../crates/uqa-engine/src/capabilities/sequence_values.rs), [`backend promotion`](../../crates/uqa-engine/src/transactions/backend.rs) | Separate transactional definitions from allocation; replace session-local document-ID reservations with coordinated non-reuse. |
...
`Engine::from_persistent_backends` must validate transaction affinity and format/model identity for a manually supplied concurrent catalog/backend pair. It cannot manufacture a correct shared context from unrelated handles. Preserve the existing serialized path for legacy implementations and reject a mismatched concurrent pair before exposing a session. Public Rust construction changes require migration notes and actual downstream compilation with development dependencies excluded.

The common [`VersionedPersistence`](../../crates/uqa-storage/src/mvcc/persistence.rs) interface now defines durable transaction allocation, retained snapshots, conditional commit, receipt lookup and abort. `CommitFailure` separates known rejection from indeterminate native commit outcomes, and `CommitStatus::Unknown` is not treated as proof of rollback. `PreparedRecordCommit` fingerprints its ordered evaluated records with SHA-256 before commit admission; receipt retries accept only the same prepared batch. The [redb record adapter](../../crates/uqa-storage-redb/src/mvcc/mod.rs) implements this contract with short physical transactions and Immediate durability. It currently retains all versions/receipts and does not yet map existing SQL catalog or Key/Value sessions to its records. Logical session routing, bounded reclamation, old-format migration and full SQL capability remain required.
