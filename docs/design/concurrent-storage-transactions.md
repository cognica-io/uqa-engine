...
Status: Implementation in progress. This design was checked against UQA 0.3.6 at `24e464f7709cd5d942df8d0aeef5d522716b1bfb` on 2026-09-16. Direct redb Key/Value sessions now use common MVCC, but the complete concurrent SQL capability described here remains unfinished. The [implementation plan](../plans/0008-concurrent-storage-transactions.md) tracks the work and acceptance evidence; the [storage manual](../manual/internals/03-storage.md) distinguishes released behavior from development changes.
...
| SQLite K/V and redb | [`SQLite key_value.rs`](../../crates/uqa-storage-sqlite/src/key_value.rs), [`common sessions`](../../crates/uqa-storage/src/mvcc/session/mod.rs), [`redb records`](../../crates/uqa-storage-redb/src/mvcc/mod.rs) | redb now uses common logical sessions and short durable operations; route SQLite K/V through the same contract. |
