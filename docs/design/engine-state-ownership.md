...
Event declarations use native SQL analysis over live relation metadata and fresh catalog routine scopes. Native execution owns registration, DROP, rename and enable-mode publication; [event adapters](../../crates/uqa-engine/src/capabilities/event_registry.rs) lend the actual registry guards and pending constraint-event state. The unified dispatcher holds a native event context instead of a whole-command Engine event interface. Pure event validation and partition tests live in SQL; tests of real registration, concurrent routine catalogs and deferred transaction events remain beside Engine state.
