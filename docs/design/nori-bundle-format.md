...
Status: Implemented dictionary codec, offline packer, native tokenizer, compiled resource sharing, and SQL/binding integration. The [implementation plan](../plans/0006-nori-analyzer.md) tracks measured evidence and remaining package/performance acceptance.

The `nori` feature of `uqa-analysis` exposes `NoriDictionary::from_bytes` and immutable dictionary queries. The `nori-tools` feature additionally exposes the offline packer and exhaustive neutral-model verifier. The `nori` feature also exposes the Korean tokenizer and filters and enables the immutable built-in `nori` analyzer. Loading bytes does not read files, resolve URLs, download a model, or execute a JVM. Direct loader callers retain and clone the returned `Arc<NoriDictionary>`; compiled resource resolution shares validated immutable dictionaries by content identity through bounded caches.
...
`uqa-nori-data` contains the verified bytes, the original model manifest, and a resource manifest covering the bundle and all attribution files. It has no runtime dependencies or build script and exposes static bytes in a `no_std` library. The complete Lucene license and notice were extracted from the hash-verified Nori jar; MeCab-ko-dic COPYING was extracted from the hash-verified source archive; the Unicode notice was read from the pinned Docker JDK. The Rust wrapper retains the workspace license and upstream files retain their own notices.

The release-license checker validates all declared resource hashes in source and in the actual `.crate` archive, rejects missing or altered attributions, and enforces an internal 10,000,000-byte archive budget. The measured data archive is 9,682,047 bytes. All 23 publishable workspace crates packaged offline and passed their archive checks. The data archive also passed Cargo's own compilation verification, and its extracted library built for `wasm32-unknown-unknown`. That initial build establishes artifact portability. Subsequent [native/WASM measurements](../../benchmarks/nori/README.md) execute dictionary decoding and analysis, and the [delivery inventory](../../benchmarks/nori/DELIVERY.md) records actual binding artifacts and package checks. Public SQL/session calibration and final remote-head CI remain open.
...
The reviewed model produces 9,829,534 bytes with artifact SHA-256 `0d920523991ed60909972d85df630c65747ff5d49e1138e4998ca39f0079538d` and semantic identity `ee85ee5796c706ea7288e44e64c7f58ba7bd22cea5683c1f4196c29a2435d74a`. Independent neutral exports from Docker arm64 and amd64 packed to identical bundle bytes and each passed full reconstruction. An initial LZ4 layout with redundant arc/rank and absolute-ID fields occupied 27,648,214 bytes; the final compact representation and zlib framing reduced that measured artifact size while preserving every neutral hash. Repeated [native/WASM measurements](../../benchmarks/nori/README.md) record cold loading, decoded retained memory, tokenizer throughput, and cache sharing with reviewed allocation limits. The implementation plan separately tracks final cross-binding package acceptance.
