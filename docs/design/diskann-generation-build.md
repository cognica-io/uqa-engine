...
The [revision-2 manifest](diskann-generation-format.md#build-provenance) persists work revisions, effective partition/merge/PQ/batch options, counts, entry sample size and assignment/adjacency fingerprints. Revision-1 manifests retain their exact original encoding and validation. Graph and metadata record formats otherwise remain unchanged.

Physical sealing checks ordered complete streams, exact batch boundaries, raw node identities and versions, checksums and artifact digests. For revision 2 it additionally checks global PQ training options, decoded adjacency count/digest and the reserved successor for every non-singleton node. It does not infer SQL publication authority from a coverage fingerprint or successful physical verification.
